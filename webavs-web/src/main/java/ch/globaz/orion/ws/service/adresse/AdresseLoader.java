...
public class AdresseLoader {
    ...
    public Map<String, List<Adresse>> loadLastByIdsTiersAndGroupByIdTiers(List<String> idsTiers, String domaine,
        ...
        try {
            map = searchByLot(idsTiers, domaine, type, LIMIT_SIZE);
        } catch (Exception e) {
    ...
    public Map<String, List<Adresse>> searchByLot(Collection<String> idsTiers, String domaine, String type,
            int limitSize) {
        loadAdresses(idsTiers, limitSize);
        ...
        for (Entry<String, List<TIAbstractAdresseData>> entry : cache.entrySet()) {
            ...
            if (data != null) {
                ...
                map.get(data.getIdTiers()).add(convertToAdresse(data));
            }
        }
    ...
    public Map<String, List<Adresse>> loadLastAllAdresses(Collection<String> idsTiers) {
        loadAdresses(idsTiers, LIMIT_SIZE);
        Map<String, List<Adresse>> map = new HashMap<String, List<Adresse>>();
        for (Entry<String, List<TIAbstractAdresseData>> entry : cache.entrySet()) {
            map.put(entry.getKey(), new ArrayList<Adresse>());
            map.get(entry.getKey()).add(new Adresse());
        }
        return map;
    }

    private void loadAdresses(Collection<String> idsTiers, int limitSize) {
        try {
            ...
            if (!isAllIdsInCache(idsTiers)) {
                for (List<String> ids : idsSplited) {
                    AdresseLoaderManager mgr = new AdresseLoaderManager();
            ...
            throw new CommonTechnicalException("Impossible de charger l'adresse ids:" + Joiner.on(",").join(idsTiers),
                    e);
        }
    }

    private Adresse convertToAdresse(TIAbstractAdresseData data) {
        AdresseType type = AdresseType.INDEFINI;
        if (AdresseService.CS_TYPE_DOMICILE.equals(data.getTypeAdresse())) {
            type = AdresseType.DOMICILE;
        } else if (AdresseService.CS_TYPE_COURRIER.equals(data.getTypeAdresse())) {
            type = AdresseType.COURRIER;
        }

        return new Adresse(data.getLocalite(), data.getCasePostale(), data.getAttention(), data.getNpa(),
                data.getPaysIso(), data.getIdCanton(), data.getTitre_adr(), data.getRue(), data.getNumero(),
