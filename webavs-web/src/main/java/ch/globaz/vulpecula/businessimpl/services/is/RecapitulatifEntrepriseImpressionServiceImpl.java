...
public class RecapitulatifEntrepriseImpressionServiceImpl extends ALAbstractBusinessServiceImpl implements
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(RecapitulatifEntrepriseImpressionServiceImpl.class);
    ...
    /* JadePrintDocumentContainer conteneurAgriInd, */) throws JadeApplicationException, JadePersistenceException {
        ...
        setTexteLibre(docRecapAffilie, numAffilie);
    ...
    private void setTexteLibre(DocumentData docData, String numAffilie) {
        Employeur employeur = VulpeculaRepositoryLocator.getEmployeurRepository().findByNumAffilie(numAffilie);
        if (employeur == null) {
            LOGGER.error("L'employeur " + numAffilie
                    + " ne possède pas de type de facturation (versement ou note de crédit)");
            return;
        }

        BSession session = BSessionUtil.getSessionFromThreadContext();
        if (TypeFacturation.NOTE_DE_CREDIT.equals(employeur.getTypeFacturation())) {
            docData.addData("recapitulatifEntreprise_texteLibre",
                    session.getLabel("DOCUMENT_TYPE_FACTURATION_NOTE_DE_CREDIT"));
        } else if (TypeFacturation.VERSEMENT.equals(employeur.getTypeFacturation())) {
            docData.addData("recapitulatifEntreprise_texteLibre",
                    session.getLabel("DOCUMENT_TYPE_FACTURATION_VERSEMENT"));
        }
    }
