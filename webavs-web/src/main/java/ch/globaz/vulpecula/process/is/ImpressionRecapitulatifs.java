 ...
 */
public class ImpressionRecapitulatifs extends ALAbsrtactProcess {

    private RecapitulatifEntrepriseImpressionServiceImpl recapitulatifEntrepriseImpressionServiceImpl = new RecapitulatifEntrepriseImpressionServiceImpl();
    ...
    protected void process() {
        ...
        try {
            ...
            } else if (JadeStringUtil.equals(getTypeTraitRecapImpr(), ALConstPrestations.TRAITEMENT_NUM_PROCESSUS,
                    false)) {
                listResultatRecherche = ALServiceLocator.getRecapitulatifEntrepriseBusinessService()
                        .resultSearchRecapNumProcessus(getNumProcessus());
            } else {
        ...
        try {
            ...
            // mail en .pdf
            listRecapPdf = recapitulatifEntrepriseImpressionServiceImpl.loadArrayListDocData(listResultatRecherche);
            if (listRecapPdf.size() != 0) {
                listConteneurRecapPublicationPdf = (recapitulatifEntrepriseImpressionServiceImpl.loadDocuments(
                        listRecapPdf, getDateImpression(), envoiGED));
            }
        ...
        try {
            ...
            if (listRecapCsv.size() != 0) {
                listConteneurRecapNonPublicationPdf = recapitulatifEntrepriseImpressionServiceImpl.loadDocuments(
                        listRecapCsv, dateImpression, envoiGED);
            }
