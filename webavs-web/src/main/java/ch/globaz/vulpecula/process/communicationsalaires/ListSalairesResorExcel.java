...
public class ListSalairesResorExcel extends PrestationsListExcel {
    private Annee annee;
    ...
    public ListSalairesResorExcel(BSession session, String filenameRoot, String documentTitle) {
        ...
        for (SalairesAAnnoncer salaires : listDecompteSalaire) {
            if (salaires.getListeDecomptes() != null && salaires.getListeDecomptes().size() > 0) {
                for (DecompteSalaire decompteSalaire : salaires.getListeDecomptes()) {
                    createRow();
                    createCell(decompteSalaire.getEmployeur().getRaisonSociale(), getStyleListLeft());
