...
public class AnnoncesChangeChecker {
    ...
    public static boolean hasChanged(AnnonceRafamModel annonce, AnnonceRafamModel lastAnnonce,
            DossierComplexModel dossier, DroitComplexModel droit) throws JadeApplicationException,
    ...
    private static boolean statutFamilialHasChanged(AnnonceRafamModel lastAnnonce, DroitComplexModel droit)
            throws JadeApplicationException {
        return !RafamFamilyStatus.getFamilyStatus(lastAnnonce.getCodeStatutFamilial()).equals(
                RafamFamilyStatus.getFamilyStatusCS(droit.getDroitModel().getStatutFamilial()));
    }
    ...
    private static boolean echeanceDroitHasChanged(AnnonceRafamModel annonce, AnnonceRafamModel lastAnnonce,
        ...
        return !lastAnnonce.getEcheanceDroit().equals(
                (annonce == null ? droit.getDroitModel().getFinDroitForcee() : annonce.getEcheanceDroit()));
    }

    private static boolean debutDroitHasChanged(AnnonceRafamModel annonce, AnnonceRafamModel lastAnnonce,
            DroitComplexModel droit) {
        return !lastAnnonce.getDebutDroit().equals(
                (annonce == null ? droit.getDroitModel().getDebutDroit() : annonce.getDebutDroit()));
    }
    ...
    private static boolean baseLegalHasChanged(AnnonceRafamModel lastAnnonce, DossierComplexModel dossier,
        ...
        return !RafamLegalBasis.getLegalBasis(lastAnnonce.getBaseLegale()).equals(
                initAnnonceService.getBaseLegale(dossier, droit));
    }
