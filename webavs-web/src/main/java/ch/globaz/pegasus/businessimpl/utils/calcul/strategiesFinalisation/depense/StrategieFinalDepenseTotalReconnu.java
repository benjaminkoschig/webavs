...
public class StrategieFinalDepenseTotalReconnu implements StrategieCalculFinalisation {
    ...
    private float calculeDepensesPersonnelles(CalculContext context, TupleDonneeRapport donnee) throws CalculException {
        ...
        try {
            ...
            // garde valeur nulle par defaut, signifiant que la personne n'est
            // pas dans un home
        }
        ...
        if (IPCTaxeJournaliere.CS_CATEGORIE_CHAMBRE_MEDICALISE.equals(csTypeChambre)) {
            depensesPersonnelles = Float.parseFloat(((ControlleurVariablesMetier) context
                    .get(Attribut.CS_ARGENT_POCHE_MEDICALISE)).getValeurCourante());
        } else if (IPCTaxeJournaliere.CS_CATEGORIE_CHAMBRE_NON_MEDICALISE.equals(csTypeChambre)) {
            depensesPersonnelles = Float.parseFloat(((ControlleurVariablesMetier) context
                    .get(Attribut.CS_ARGENT_POCHE_NON_MEDICALISE)).getValeurCourante());
        } else if (csTypeChambre != null) {
