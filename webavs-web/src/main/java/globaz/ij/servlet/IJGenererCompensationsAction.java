 ...
 * @author dvh
 */
public class IJGenererCompensationsAction extends IJDefaultProcessAction {

    // ~ Constructors
    // ---------------------------------------------------------------------------------------------------

    /**
     * Crée une nouvelle instance de la classe IJGenererCompensationsAction.
     * 
     * @param servlet
     *            DOCUMENT ME!
     */
    public IJGenererCompensationsAction(FWServlet servlet) {
        super(servlet);
    }
    ...
    protected void actionAfficher(HttpSession session, HttpServletRequest request, HttpServletResponse response,
        ...
        try {
            ...
            IJGenererCompensationsViewBean gcViewBean = (IJGenererCompensationsViewBean) viewBean; // pas
            // d'exception
            // possible
            ...
            gcViewBean.setMoisPeriodeFacturation(mois);
            ...
            if (selectedId != null) { // l'id sera null si on arrive du menu
                ...
                if (JadeStringUtil.isIntegerEmpty(gcViewBean.getCsEtatLot())) { // si
                    ...
                    if ((viewBeanDansSession != null) && (viewBeanDansSession instanceof IJLotViewBean)
                            && ((IJLotViewBean) viewBeanDansSession).getIdLot().equals(selectedId)) {
            ...
            if (!(gcViewBean.getCsEtatLot().equals(IIJLot.CS_OUVERT)
                    || gcViewBean.getCsEtatLot().equals(IIJLot.CS_COMPENSE) || JadeStringUtil.isIntegerEmpty(gcViewBean
                    .getCsEtatLot()))) {
                gcViewBean._addError("GENERATION_COMPENSATIONS_IMPOSSIBLE");
            }

            session.setAttribute(FWServlet.VIEWBEAN, gcViewBean);
        } catch (Exception e) {
