...
/**
 * <H1>Ne doit pas être instancieé !! Passer par le manager.</H1>
 * 
 * @author dvh
 */
public class IJFactureJointCompensation extends IJFactureACompenser implements PRHierarchique {
    ...
    protected void _readProperties(globaz.globall.db.BStatement statement) throws Exception {
        ...
        try {
            idCompensation = statement.dbReadNumeric(IJCompensation.FIELDNAME_IDCOMPENSATION);
            idLot = statement.dbReadNumeric(IJCompensation.FIELDNAME_IDLOT);
            idTiersCompensation = statement.dbReadNumeric(IJCompensation.FIELDNAME_IDTIERS);
            idAffilieCompensation = statement.dbReadNumeric(IJCompensation.FIELDNAME_IDAFFILIE);
            montantTotal = statement.dbReadNumeric(IJCompensation.FIELDNAME_MONTANTTOTAL, 2);
            dettes = statement.dbReadNumeric(IJCompensation.FIELDNAME_DETTES, 2);
            prenomTiers = statement.dbReadString(FIELDNAME_PRENOMTIERS);
            nomTiers = statement.dbReadString(FIELDNAME_NOMTIERS);
            isLigneCompensation = statement.dbReadBoolean(FIELDNAME_ISLIGNECOMPENSATION);
        } catch (Exception e) {
            e.printStackTrace();
