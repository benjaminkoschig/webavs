...
//            PRACORAdapter adapter = IJAdapterFactory.getInstance(session).createAdapter(session, prononce);
            ...
            inHost.getAssure()
                  .addAll(assureTypeAcorMapper.map((membreFamilleRequerant, assureType) -> {
                      ...
                      BasesCalculCouranteIJ basesCalculCouranteIJ = mapToBaseCalculCourante(session, prononce, indemniteJournaliereIJ);
    ...
    private BasesCalculCouranteIJ mapToBaseCalculCourante(final BSession session, final IJPrononce prononce, final IndemniteJournaliereIJ indemniteJournaliereIJ) {
