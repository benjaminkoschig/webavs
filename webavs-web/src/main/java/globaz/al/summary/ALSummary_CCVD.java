...
public class ALSummary_CCVD implements ITISummarizable {
    String element = "";
    String icon = "";
    ...
    private TISummaryInfo handleDossierAF(DossierComplexModel dossier, BSession userSession) throws Exception {
        ...
        if (!dossier.isNew()) {
            EntetePrestationSearchModel entetePrestationSearchModel = new EntetePrestationSearchModel();
            entetePrestationSearchModel.setForIdDossier(dossier.getId());
            entetePrestationSearchModel.setOrderKey("periode");
            entetePrestationSearchModel = ALServiceLocator.getEntetePrestationModelService().search(
                    entetePrestationSearchModel);

            EntetePrestationModel entetePrestationModel = new EntetePrestationModel();
            if (entetePrestationSearchModel.getSize() > 0) {
                entetePrestationModel = (EntetePrestationModel) entetePrestationSearchModel.getSearchResults()[0];
            }

            StringBuffer s = new StringBuffer();
            s.append("<table>");

            s.append("<tr><td style='font-weight:bold;width:200px'>");
            s.append(userSession.getLabel("VG_AF_TEXT_ETAT"));
            s.append("</td><td style='font-weight:bold;width:100px'>");
            s.append("Dossier No");
            s.append("</td><td style='font-weight:bold;width:150px'>");
            s.append("Type allocataire");
            s.append("</td><td style='font-weight:bold;width:80px'>");
            s.append("Montant");
            s.append("</td><td style='font-weight:bold;width:150px'>");
            s.append("Période");
            s.append("</td></tr>");
            ...
            if (ALCSDossier.ETAT_RADIE.equals(dossier.getDossierModel().getEtatDossier())) {
                ...
                s.append(dossier.getDossierModel().getDebutValidite());
            }
            s.append("</td><td>");
            s.append("<a href='al?userAction=al.dossier.dossierMain.afficher&selectedId="
                    + dossier.getDossierModel().getIdDossier() + "'>" + dossier.getDossierModel().getIdDossier()
                    + "</a>");
            s.append("</td><td>");
            s.append(userSession.getCodeLibelle(dossier.getDossierModel().getActiviteAllocataire()));
            s.append("</td><td>");
            s.append(entetePrestationModel.getMontantTotal());
            s.append("</td><td>");
            s.append(entetePrestationModel.getPeriodeDe() + " - " + entetePrestationModel.getPeriodeA());
            s.append("</td></tr>");
            s.append("</table>");
            ...
            if (hasDroitDossier) {
                // this.setUrlTitle("al?userAction=al.dossier.dossierMain.afficher&selectedId="
