...
public class ALDecisionsMasseProcess extends ALDecisionsMasseAbstractProcess {
     ...
     */
    private void processDossiers() throws IOException, JadeApplicationException, JadePersistenceException,
     ...
     * @throws JadePersistenceException
     */
    private void addDecision(DossierComplexModel dossierComplex) throws JadeApplicationException,
            JadePersistenceException {

        ALServiceLocator.getCopiesBusinessService().createDefaultCopies(dossierComplex, ALCSCopie.TYPE_DECISION);

        containerDecisions = serviceDecision.getDecision(containerDecisions, dossierComplex, getEmail(), getSession()
                .getUserFullName(), getSession().getUserInfo().getPhone(), getSession().getUserInfo().getVisa(),
                getDateImpression(), getInsertionGED(), getGestionCopie(), getTexteLibre(), getGestionTexteLibre());

        JadeBusinessMessage message = new JadeBusinessMessage(JadeBusinessMessageLevels.INFO,
                ALDecisionsMasseProcess.class.getName(), "al.protocoles.impressionDecisionsMasse.info.decisionImprimee");
        addProtocoleMessage(dossierComplex, message);
    }
     ...
     * @throws JadeNoBusinessLogSessionError
     */
    private void journaliser(DossierComplexModel dossierComplex) throws JadeNoBusinessLogSessionError {
     ...
     */
    private void printProtocole() throws JadePersistenceException, JadeApplicationException,
        ...
        publishDocInfo.setDocumentTitle(JadeThread
                .getMessage("al.protocoles.impressionDecisionsMasse.info.processus.val"));
        publishDocInfo.setDocumentSubject(JadeThread
                .getMessage("al.protocoles.impressionDecisionsMasse.info.processus.val"));
     ...
     * @throws JadeClassCastException
     */
    private void printDecisions() throws JadeServiceLocatorException, JadeServiceActivatorException,
            JadeClassCastException {
        containerDecisions.setMergedDocDestination(serviceDecision.getMergedDocumentInfos(getEmail(),
                getDateImpression()));
