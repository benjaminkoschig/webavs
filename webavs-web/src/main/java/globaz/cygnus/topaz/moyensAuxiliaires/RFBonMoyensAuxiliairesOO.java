 ...
 * author fha
 */
public class RFBonMoyensAuxiliairesOO extends RFAbstractDocumentOO {

    public static final String FICHIER_MODELE_DOCUMENTS_RFM = "RF_LETTRE_TYPE_MOYENS_AUX_BON_REMISE_PRET";
    ...
    public void chargerCatalogueTexte() throws Exception {
        ...
        documentHelper.setCsTypeDocument(IRFCatalogueTexte.CS_BON_REMISE_PRET);
    ...
    public void chargerDonneesEnTete() throws Exception {
        ...
        try {
            ...
            caisseHelper = CaisseHelperFactory.getInstance().getCaisseReportHelperOO(getSession().getApplication(),
                    codeIsoLangue);
            caisseHelper.setTemplateName(RFBonMoyensAuxiliairesOO.FICHIER_MODELE_DOCUMENTS_RFM);
    ...
    public void generationLettre(RFCopieDecisionsValidationData... copie) throws Exception {
        ...
        data.addData("idProcess", "RFBonMoyensAuxiliairesOO");
        ...
        if (null != tiersWrapper) {
            ...
            throw new Exception("Erreur : Pas d'adresse tiers (RFBonMoyensAuxiliairesOO.generationLettre())");
        }
    ...
    public JadePrintDocumentContainer remplir(JadePrintDocumentContainer documentContainer, RFDocumentsProcess process,
        ...
        return super.remplir(documentContainer, process, getIdTiers(),
                IPRConstantesExternes.RFM_LETTRE_TYPE_MOYENS_AUXILIAIRES_BON_REMISE_EN_PRET, isSendToGed);
    }
    ...
    public void remplirDocument() throws Exception {
        ...
        try {
            // %=TITRE_BON=%
            data.addData("TITRE_BON", documentPrincipale.getTextes(2).getTexte(1).getDescription());
            // %=OBJET=%
            ...
            // %=DISPO=%
            data.addData("DISPO", documentPrincipale.getTextes(3).getTexte(3).getDescription());
            // %=ENTRETIEN=%
            data.addData("ENTRETIEN",
                    PRStringUtils.replaceString(documentPrincipale.getTextes(3).getTexte(4).getDescription(), DATE, ""));
            // %=LIVRER=%
            data.addData("LIVRER", documentPrincipale.getTextes(3).getTexte(5).getDescription());
            // %=REPRESENTANT=%
            data.addData("REPRESENTANT", documentPrincipale.getTextes(3).getTexte(6).getDescription());
            // %=COPIE=%
            data.addData("COPIE", documentPrincipale.getTextes(3).getTexte(7).getDescription());
            // %=ACCUSE=%
            data.addData("ACCUSE", documentPrincipale.getTextes(4).getTexte(1).getDescription());
            // %=REPRES=%
            data.addData("REPRES", documentPrincipale.getTextes(4).getTexte(2).getDescription());
            // %=CONFIRMATION=%
            data.addData("CONFIRMATION", documentPrincipale.getTextes(4).getTexte(3).getDescription());
            // %=SALUTATIONS=%
            data.addData("SALUTATIONS", PRStringUtils.replaceString(documentPrincipale.getTextes(5).getTexte(1)
                    .getDescription(), TITRE, sexe.equals("516001") ? "Monsieur" : "Madame"));
            // %=ANNEXE=%
            data.addData("ANNEXE", documentPrincipale.getTextes(6).getTexte(1).getDescription());
            // %=MENTIONNE=%
            data.addData("MENTIONNE", documentPrincipale.getTextes(6).getTexte(2).getDescription());
            // %=COPIE=%
            data.addData("COPIE", documentPrincipale.getTextes(6).getTexte(3).getDescription());
            // %=BENEF_PC=%
            data.addData("BENEF_PC", documentPrincipale.getTextes(6).getTexte(4).getDescription());
