 ...
 * author fha
 */
public class RFRappelQuestionnaireRevisionsOO extends RFAbstractDocumentOO {

    public static final String FICHIER_MODELE_DOCUMENTS_RFM = "RF_LETTRE_TYPE_REGIME_RAPPEL_QUESTIONNAIRE_REVISION";

    protected final String DATE = "{date}";
    protected final String DATE_COURRIER_PRECEDENT = "{dateCourrierPrecedent}";
    private FWMemoryLog memoryLog;
    protected final String MONTANT = "{montantRFM}";
    protected String regimeDateCourrierPrecedent = "";
    protected String regimeMontantAllocationMensuelleRappel = "";
    ...
    private void ajouterSignature() throws Exception {
        ...
        try {
            ...
            addErreurMail(memoryLog, e.getMessage(), "RFRappelQuestionnaireRevisionsOO:ajouterSignature");
    ...
    public void chargerCatalogueTexte() throws Exception {
        ...
        documentHelper.setCsTypeDocument(IRFCatalogueTexte.CS_RAPPEL_QUESTIONNAIRE_REVISION);
    ...
    public void chargerDonneesEnTete() throws Exception {
        ...
        try {
            ...
            caisseHelper = CaisseHelperFactory.getInstance().getCaisseReportHelperOO(getSession().getApplication(),
                    codeIsoLangue);
            caisseHelper.setTemplateName(RFRappelQuestionnaireRevisionsOO.FICHIER_MODELE_DOCUMENTS_RFM);
    ...
    public void generationLettre(RFCopieDecisionsValidationData... copie) throws Exception {
        ...
        // Chargement template
        data.addData("idProcess", "RFRappelQuestionnaireRevisionsOO");
        ...
        if (null != tiersWrapper) {
            ...
            throw new Exception("Erreur : Pas d'adresse tiers (RFRappelQuestionnaireRevisionsOO.generationLettre())");
        }
    ...
    public JadePrintDocumentContainer remplir(JadePrintDocumentContainer documentContainer, RFDocumentsProcess process,
        ...
        setRegimeMontantAllocationMensuelleRappel(process.getRegimeMontantAllocationMensuelleRappel());
        setRegimeDateCourrierPrecedent(process.getRegimeDateCourrierPrecedent());

        return super.remplir(documentContainer, process, process.getIdTiers(),
                RFDocumentEnum.RFM_LETTRE_TYPE_REGIME_RAPPEL_QUESTIONNAIRE_REVISION.getNoInforom(), miseEnGed);
    }
    ...
    public void remplirDocument() throws Exception {
        ...
        try {
            ...
            // %=COURRIER=%
            data.addData("COURRIER", PRStringUtils.replaceString(documentPrincipale.getTextes(2).getTexte(2)
                    .getDescription(), DATE_COURRIER_PRECEDENT, getRegimeDateCourrierPrecedent()));
            // %=MANQUANT=%
            data.addData("MANQUANT", documentPrincipale.getTextes(2).getTexte(3).getDescription());
            // %=REMBOURSEMENT=%
            data.addData("REMBOURSEMENT", PRStringUtils.replaceString(documentPrincipale.getTextes(2).getTexte(4)
                    .getDescription(), MONTANT, getRegimeMontantAllocationMensuelleRappel()));
            // %=CONSEQUENCE=%
            data.addData("CONSEQUENCE", PRStringUtils.replaceString(documentPrincipale.getTextes(2).getTexte(5)
                    .getDescription(), DATE, JadeDateUtil.addMonths(JadeDateUtil.getFormattedDate(new Date()), 1)));
            // %=SALUTATIONS=%
