 ...
 * author fha
 */
public class RFDentistesEnvoiDossierMedecinOO extends RFAbstractDocumentOO {

    public static final String FICHIER_MODELE_DOCUMENTS_RFM = "RF_LETTRE_TYPE_DENTISTE_ENVOI_DOSSIER_MEDECIN";
    ...
    public void chargerCatalogueTexte() throws Exception {
        ...
        documentHelper.setCsTypeDocument(IRFCatalogueTexte.CS_DOSSIER_MEDECIN);
    ...
    public void chargerDonneesEnTete() throws Exception {
        ...
        try {
            ...
            caisseHelper = CaisseHelperFactory.getInstance().getCaisseReportHelperOO(getSession().getApplication(),
                    codeIsoLangue);
            caisseHelper.setTemplateName(RFDentistesEnvoiDossierMedecinOO.FICHIER_MODELE_DOCUMENTS_RFM);
    ...
    public void generationLettre(RFCopieDecisionsValidationData... copie) throws Exception {
        ...
        data.addData("idProcess", "RFDentistesEnvoiDossierMedecinOO");
        ...
        if (null != tiersWrapper) {
            ...
            throw new Exception("Erreur : Pas d'adresse tiers (RFDentistesEnvoiDossierMedecinOO.generationLettre())");
        }
    ...
    public JadePrintDocumentContainer remplir(JadePrintDocumentContainer documentContainer, RFDocumentsProcess process,
        ...
        return super.remplir(documentContainer, process, getIdTiers(),
                IPRConstantesExternes.RFM_LETTRE_TYPE_DENTISTE_ENVOIS_DOSSIER_COMPLET_AU_MEDECIN_CONSEIL, isSendToGed);
    }
    ...
    public void remplirDocument() throws Exception {
        ...
        try {
            ...
            // %=DEVIS_FACTURE=%
            data.addData("DEVIS_FACTURE", documentPrincipale.getTextes(3).getTexte(1).getDescription());
            // %=DENTISTE=%
            ...
            // %=DEMANDE=%
            data.addData("DEMANDE", documentPrincipale.getTextes(4).getTexte(2).getDescription());
            // %=ENVOI=%
            data.addData("ENVOI", documentPrincipale.getTextes(4).getTexte(3).getDescription());
            // %=PARTICIPATION=%
            data.addData("PARTICIPATION", documentPrincipale.getTextes(4).getTexte(4).getDescription());
            // %=SALUTATIONS=%
            ...
            // %=TRAITEMENT=%
            data.addData("TRAITEMENT", documentPrincipale.getTextes(6).getTexte(4).getDescription());
            // %=SEA=%
            data.addData("SEA", documentPrincipale.getTextes(6).getTexte(5).getDescription());
            // %=LOI=%
            data.addData("LOI", documentPrincipale.getTextes(6).getTexte(6).getDescription());
            // %=SA=%
            data.addData("SA", documentPrincipale.getTextes(6).getTexte(7).getDescription());
            // %=RESTE_MONTANT=%
            data.addData("RESTE_MONTANT", documentPrincipale.getTextes(6).getTexte(8).getDescription());
            // %=DETAILS=%
            data.addData("DETAILS", documentPrincipale.getTextes(6).getTexte(9).getDescription());
            // %=ANNEXE=%
