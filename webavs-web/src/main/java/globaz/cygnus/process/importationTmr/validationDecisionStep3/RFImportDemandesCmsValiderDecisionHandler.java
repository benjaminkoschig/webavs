...
import globaz.cygnus.process.importationTmr.RFImportDemandesCmsPopulation;
...
public class RFImportDemandesCmsValiderDecisionHandler implements JadeProcessEntityInterface,
        JadeProcessEntityNeedProperties, JadeProcessEntityDataFind<RFProcessImportationTmrEnum>,
        ...
        if (rfAssMotRefDemMgr.size() > 0) {
            ...
            while (rfAssMotRefItr.hasNext()) {
                ...
                if (rfAssMotRefDem != null) {
                    ...
                    throw new Exception("[RFPreparerDemandesTmrService.getMontantMotifDeRefus()] Demande null");
                }
            }
            return null;
        } else {
        ...
        if (rfAssMotRefDemMgr.size() > 0) {
            ...
            while (rfAssMotRefItr.hasNext()) {
                ...
                if (rfAssMotRefDem != null) {
                    ...
                    throw new Exception(
                            "[RFPreparerDemandesTmrService.isCodeErreurSelonIdMotifDeRefus()] Demande null");
                }
            }
        ...
        try {
            ...
            if (!rfDemande.isNew()) {
                ...
                throw new IllegalArgumentException(
                        "[RFPreparerDemandesTmrService.loadDemandeParId()]  Impossible de retrouver la demande avec son id");
            }
    ...
    private void retrieveCodeTraitementAvecIdDemandeForcePaiementDecoche(RFImportDemandesCmsData dataDemande,
        ...
        if (JadeStringUtil.isBlankOrZero(rfDemande.getMontantAPayer())) {
            ...
            addMessageCodeTraitement(RFCodeTraitementDemandeTmrCleEnum.REFUS_ASV.getCode(),
                    "RF_IMPORT_TMR_REFUS_FORTUNE_PLUS_GRANDE_AVS");
            ...
            if (JadeStringUtil.isBlankOrZero(rfDemande.getMontantFacture())) {
                throw new JadePersistenceException(
                        "[RFPreparerDemandesTmrService.retrieveCodeTraitementAvecIdDemandeForcePaiementDecoche()] Montant facture introuvable");
            }
            ...
            if (isMntFactureEqualMntAPayer(rfDemande.getMontantFacture(), rfDemande.getMontantAPayer())) {
                dataDemande.setCodeTraitement(RFCodeTraitementDemandeTmrCleEnum.PAIEMENT_TOTAL.getCode());
                // Sinon c'est une partielle
            } else {
                addMessageCodeTraitement(RFCodeTraitementDemandeTmrCleEnum.PAIEMENT_PARTIEL.getCode(),
                        "RF_IMPORT_TMR_PAIEMENT_PARTIEL");
            }
        }
    }
    ...
    private void retrieveCodeTraitementAvecIdDemandeForcerPaiementCoche(RFImportDemandesCmsData dataDemande,
        ...
        if (JadeStringUtil.isBlankOrZero(rfDemande.getMontantAPayer())) {
            ...
            if (isCodeErreurSelonIdMotifDeRefus(dataDemande.getIdDemandeValiderDecisionStep(), session,
                ...
                addMessageCodeTraitement(RFCodeTraitementDemandeTmrCleEnum.QD_DANS_HOME.getCode(),
                        "RF_IMPORT_TMR_QD_DANS_HOME");
                ...
                addMessageCodeTraitement(RFCodeTraitementDemandeTmrCleEnum.REFUS_PC.getCode(),
                        "RF_IMPORT_TMR_REFUS_PC_COMPENSATION_EXCEDENT_REVENU");
                ...
                addMessageCodeTraitement(RFCodeTraitementDemandeTmrCleEnum.NSS_PAS_TROUVE.getCode(),
                        "RF_IMPORT_TMR_PAS_DE_QD_VALABLE_DATE_DEBUT_TRAITEMENT", pDate);

            } else {
                addMessageCodeTraitement(RFCodeTraitementDemandeTmrCleEnum.PAIEMENT_PARTIEL.getCode(),
                        "IMPORT_TMR_PAIEMENT_PARTIEL");
            }
        ...
        // Si montants présent
            ...
            if (JadeStringUtil.isBlankOrZero(rfDemande.getMontantFacture())) {
                throw new JadePersistenceException(
                        "[RFPreparerDemandesTmrService.retrieveCodeTraitementAvecIdDemandeForcerPaiementCoche()] Montant facture introuvable");
            }
    ...
    private void retrieveCodeTraitementSansIdDemande(RFImportDemandesCmsData dataDemande)
        ...
        if (dataDemande.getMessagesErreursImportationList().size() > 0) {
            ...
            String codeErreur = RFCodeTraitementDemandeTmrCleEnum.DONNEES_INCOHERENTES.getCode();
            for (String[] messageCourant : dataDemande.getMessagesErreursImportationList()) {
                if (messageCourant[1].equals(RFCodeTraitementDemandeTmrCleEnum.NSS_PAS_TROUVE.getCode())) {
                    codeErreur = RFCodeTraitementDemandeTmrCleEnum.NSS_PAS_TROUVE.getCode();
                }
            }
            ...
            throw new JadePersistenceException(
                    "[RFPreparerDemandesTmrService.attribuerCodeTraitement()] Liste d'erreurs vide");
        }
    }
    ...
    public void setData(Map<RFProcessImportationTmrEnum, String> hashMap) {
        if (null != hashMap) {
            idDemande = hashMap.get(RFProcessImportationTmrEnum.ID_DEMANDE);
            messagesErreurs = hashMap.get(RFProcessImportationTmrEnum.MESSAGES_ERREUR_IMPORTATION);
        }
    }

    private void setDataToSave(String codeErreur) {
        if (!JadeStringUtil.isBlankOrZero(codeErreur)) {
            dataToSave.put(RFProcessImportationTmrEnum.CODE_ERREUR, codeErreur);
        }
    }
