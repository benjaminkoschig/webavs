...
public class RFValiderDecisionSecutelProcess extends AbstractJadeJob {
     ...
     */
    public RFValiderDecisionSecutelProcess() {
        super();
    }
    ...
    public String getObjetMail() {
        ...
        if (getMemoryLog().hasErrors()) {
            return getSession().getLabel("RF_IMPORT_SECUTEL_PROCESS_VALIDATION_DECISION_DOCUMENT_FAILED");
        } else {
            return getSession().getLabel("RF_IMPORT_SECUTEL_PROCESS_VALIDATION_DECISION_DOCUMENT_SUCCESS");
        }
    }
    ...
    public void run() {
        ...
        try {
            ...
            RFValiderDecisionServiceFactory.getRFValiderDecisionSecutelService().recupererDecisions(getIdGestionnaire(),
                    getIdExecutionProcess(), getSession(), decisionArray);

            // Validation des décisions
            RFValiderDecisionServiceFactory.getRFValiderDecisionSecutelService().miseAJourEntites(decisionArray,
                    getDateSurDocument(), false, getIdGestionnaire(), getSession(), getTransaction());

        } catch (Exception e1) {
            RFValiderDecisionServiceFactory.getRFValiderDecisionSecutelService().logErreurs(getEmailAdress(),
                    getMemoryLog(), e1, this, getSession(), getTransaction());

        } finally {

            RFValiderDecisionServiceFactory.getRFValiderDecisionSecutelService().commitTransaction(getTransaction(),
                    false);
            try {
