...
public class RFImportDemandesCmsCreation implements JadeProcessStepInterface, JadeProcessStepBeforable,
    ...
    public void before(JadeProcessStep step, Map<Enum<?>, String> map) throws JadeApplicationException,
        ...
        if (errors.size() > 0) {
            ...
            throw new RFSecutelException("Secutel step 1. " + message, null);
        }
        secutelDebugLogger = new RFLogToDB(BSessionUtil.getSessionFromThreadContext());
        secutelDebugLogger.logInfoToDB("starting Secutel step", "Secutel - step 1 - before");
        idTiersAf = getAdministrationAfParCode(JadeProcessServiceLocator.getPropertiesService()
                .findAllProperties(step.getIdExecutionProcess()).get(RFProcessImportationSecutelEnum.NUMERO_AF));
        secutelDebugLogger.logInfoToDB(
                String.format(
                        "Adresse de paiement pour importation Secutel %s : %s",
                        JadeProcessServiceLocator.getPropertiesService()
                                .findAllProperties(step.getIdExecutionProcess())
                                .get(RFProcessImportationSecutelEnum.PATH_FILE_USER), idTiersAf),
                "Secutel - step 1 - before");
        JadeLogger.info(this, String.format(
                "Adresse de paiement pour importation du fichier Secutel %s : %s",
                JadeProcessServiceLocator.getPropertiesService().findAllProperties(step.getIdExecutionProcess())
                        .get(RFProcessImportationSecutelEnum.FILE_PATH_FOR_POPULATION), idTiersAf));
    ...
    private void envoyerMail(Map<Enum<?>, String> map, String docPath) throws Exception {
        JadeSmtpClient.getInstance().sendMail(
                ...
                BSessionUtil.getSessionFromThreadContext()
                        .getLabel("RF_IMPORT_SECUTEL_PROCESS_CREATION_DEMANDE_SUCCESS") /*
                                                                                        * : BSessionUtil
                                                                                        * .getSessionFromThreadContext().
                                                                                        * getLabel(
                                                                                        * "RF_IMPORT_SECUTEL_PROCESS_CREATION_DEMANDE_FAILED"
                                                                                        * )
                                                                                        */, new String[] { docPath });
    }
    ...
    private String generateDocumentLog(Map<Enum<?>, String> map) throws RFXmlmlException, Exception {
        ...
        String nomDoc = BSessionUtil.getSessionFromThreadContext().getLabel("RF_IMPORT_SECUTEL_NOM_LOG_ETAPE_UNE");
    ...
    private String getAdministrationAfParCode(String code) throws JadePersistenceException {
        ...
        if (JadeStringUtil.isBlankOrZero(code)) {
            logErreurs("getAdministrationAfParCode()",
                    BSessionUtil.getSessionFromThreadContext().getLabel("RF_IMPORT_SECUTEL_NUMERO_AF_VIDE"), true);
        ...
        if ((administrationCms == null) || (administrationCms.length == 0)) {
            logErreurs("getAdministrationAfParCode()",
                    BSessionUtil.getSessionFromThreadContext().getLabel("RF_IMPORT_SECUTEL_ADMINISTRATION_AF_INEXISANT")
                            + " n°:" + code, false);
    ...
    public void init(JadeProcessExecut jadeProcessExecut) {
        ...
        for (Entry<String, Map<Enum<?>, String>> entry : jadeProcessExecut.getPropertiesEntities().entrySet()) {
            idsIdEntityIdDemande.put(entry.getKey(), entry.getValue().get(RFProcessImportationSecutelEnum.ID_DEMANDE));
        }
    ...
    private void logErreurs(String source, String message, boolean isErreurImportation)
        ...
        RFUtils.ajouterLogImportationsSecutel(JadeBusinessMessageLevels.ERROR, "", "", message, logsList);
    }
