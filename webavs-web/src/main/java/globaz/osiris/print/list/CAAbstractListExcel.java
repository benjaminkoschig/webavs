 ...
 * @author SEL
 */
public abstract class CAAbstractListExcel {
    /**
    ...
    protected static final short COLUMN_WIDTH_BLOQUE = 8000;
    protected static final short COLUMN_WIDTH_COMPTEANNEXE = 6400;
    ...
    protected static final short COLUMN_WIDTH_ROLE = 4000;
    protected static final short COLUMN_WIDTH_SECTION = 2800;
     ...
     * @param documentTitle
     */
    public CAAbstractListExcel(BSession session, String filenameRoot, String documentTitle) {
        setSession(session);
    ...
    protected HSSFFooter createFooter(String refInforom) {
        ...
        footer.setRight(getSession().getLabel("PAGE") + HSSFFooter.page() + CAAbstractListExcel.PAGE_SEPARATOR
                + HSSFFooter.numPages());
    ...
    protected int createMergedRegion(int colFrom, int colTo, int rowFrom, int rowTo, String value, HSSFCellStyle style) {
        return this.createMergedRegion(getRegion(colFrom, colTo, rowFrom, rowTo), this.createCell(value, style));
    }
    ...
    protected int createRow() {
        colNum = 0;
        if (currentSheet.getPhysicalNumberOfRows() >= CAAbstractListExcel.NUMBER_MAX_ROW) {
            cptPage++;
            createSheet(CAAbstractListExcel.SUITE + cptPage);
        }
    ...
    protected HSSFSheet createSheet(String title) {
        ...
        if (title.length() > CAAbstractListExcel.SHEET_MAX_LENGTH) {
            title = title.substring(0, CAAbstractListExcel.SHEET_MAX_LENGTH);
        }
        currentSheet = wb.createSheet(title);

        // Marges (unité en pouces)
        currentSheet.setMargin(HSSFSheet.LeftMargin, CAAbstractListExcel.LEFT_MARGIN);
        currentSheet.setMargin(HSSFSheet.RightMargin, CAAbstractListExcel.RIGHT_MARGIN);
        currentSheet.setMargin(HSSFSheet.TopMargin, CAAbstractListExcel.TOP_MARGIN);
        currentSheet.setMargin(HSSFSheet.BottomMargin, CAAbstractListExcel.BOTTOM_MARGIN);
    ...
    public String getOutputFile() {
        try {
            String filePath = Jade.getInstance().getPersistenceDir()
                    + (filenameRoot == null ? CAAbstractListExcel.DEFAULT_FILE_NAME : filenameRoot)
                    + CAAbstractListExcel.FILE_TYPE;
    ...
    protected HSSFCellStyle getStyleMontantTotal() {
        if (styleTotal == null) {
            ...
            styleTotal.setDataFormat(CAAbstractListExcel.FORMAT_MONTANT);
    ...
    protected HSSFCellStyle getStylePourcent() {
        if (stylePourcent == null) {
            ...
            stylePourcent.setDataFormat(wb.createDataFormat().getFormat(CAAbstractListExcel.FORMAT_POURCENT));
