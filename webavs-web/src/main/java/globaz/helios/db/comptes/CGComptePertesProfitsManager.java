...
public class CGComptePertesProfitsManager extends BManager implements Serializable {
    /**
     * 
     */
    private static final long serialVersionUID = 1L;
    public final static String CG_COMPTE_PERTES_PROFITS_ORDER_BY = "IDEXTERNE";
     ...
     */
    public CGComptePertesProfitsManager() {
        super();
    ...
    protected void _beforeFind(globaz.globall.db.BTransaction transaction) {
        ...
        setOrderBy(CG_COMPTE_PERTES_PROFITS_ORDER_BY);
    }
    ...
    protected String _getFrom(globaz.globall.db.BStatement statement) {
        String from = " " + _getCollection() + "CGPLANP";
        from += " INNER JOIN " + _getCollection() + "CGCOMTP" + " ON (" + _getCollection() + "CGPLANP" + ".IDCOMPTE="
                + _getCollection() + "CGCOMTP" + ".IDCOMPTE)";
        from += " LEFT OUTER JOIN " + _getCollection() + "CGSOLDP" + " ON (" + _getCollection() + "CGPLANP"
                + ".IDEXERCOMPTABLE = " + _getCollection() + "CGSOLDP" + ".IDEXERCOMPTABLE and " + _getCollection()
                + "CGSOLDP" + ".ESTPERIODE="
                + _dbWriteBoolean(statement.getTransaction(), getForEstPeriode(), BConstants.DB_TYPE_BOOLEAN_CHAR)
                + " and " + _getCollection() + "CGPLANP" + ".IDCOMPTE=" + _getCollection() + "CGSOLDP" + ".IDCOMPTE )";
    ...
    protected String _getWhere(globaz.globall.db.BStatement statement) {
        ...
        if (!JadeStringUtil.isIntegerEmpty(getForIdCentreCharge())) {
            ...
            sqlWhere += _getCollection() + "CGSOLDP.IDCENTRECHARGE=" + _dbWriteNumeric(statement.getTransaction(), "0");
        }
        ...
        /*
         * //traitement du positionnement pour ReqComptabilite if(getReqComptabilite!=null &&
         * getReqComptabilite.length() != 0) { if (sqlWhere.length() != 0) { sqlWhere += " AND "; } sqlWhere += "?=" +
         * _dbWrite?(statement.getTransaction(), getReqComptabilite()); }
         */

        if (sqlWhere.length() != 0) {
            sqlWhere += " AND ";
        }
        sqlWhere += "IDDOMAINE<>" + _dbWriteNumeric(statement.getTransaction(), CGCompte.CS_COMPTE_BILAN);
    ...
    public void setBeginWithIdExterneListIn(ArrayList beginWithIdExterneListIn) {
        this.beginWithIdExterneListIn = beginWithIdExterneListIn;
    }
