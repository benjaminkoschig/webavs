...
public class PFValidationAjaxViewBean extends JadeAbstractAjaxListFindViewBean {
    ...
    public PFValidationAjaxViewBean() {
        initList();
    }
    ...
    private String sqlFiltre(List<String> inIdDossier) {
        ...
        sql.append("      from schema.PFDOSSI as dossier");
        sql.append("      left join ");
        sql.append("        (select  max(schema.PFDEMPCF.IBIDPF) as id, IBIDOS as idDossier");
        sql.append("           from schema.PFDEMPCF");
        sql.append("          where (schema.PFDEMPCF.IBDFIN is null or schema.PFDEMPCF.IBDFIN = 0) ");
        sql.append("            and schema.PFDEMPCF.IBTETD = 55008003 ");
        sql.append("          group by schema.PFDEMPCF.IBIDOS");
        sql.append("          having count(schema.PFDEMPCF.IBIDPF) = 1   ");
        ...
        sql.append("         from schema.PFDEMPCF as dt");
        sql.append("        where dt.IBTIMEDV = (");
        sql.append("            select max(d.IBTIMEDV)");
        sql.append("             from schema.PFDEMPCF as d");
        sql.append("            where d.IBDFIN = (");
        sql.append("                select max(schema.PFDEMPCF.IBDFIN ) as dateFin");
        sql.append("                   from schema.PFDEMPCF ");
        sql.append("                  where (schema.PFDEMPCF.IBDFIN is not null and schema.PFDEMPCF.IBDFIN <> 0) ");
        sql.append("                    and schema.PFDEMPCF.IBTETD = 55008003 ");
        sql.append("                    and schema.PFDEMPCF.IBIDOS = d.IBIDOS");
        sql.append("                  group by schema.PFDEMPCF.IBIDOS   ");
        sql.append("              )");
