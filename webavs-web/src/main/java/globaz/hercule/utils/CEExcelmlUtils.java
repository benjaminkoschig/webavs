...
import globaz.hercule.mappingXmlml.line.CEXmlmlLine;
...
public class CEExcelmlUtils {
    ...
    public static String createDocumentExcel(String modelName, String nomDoc, CommonExcelmlContainer container)
        ...
        wk = CEExcelmlUtils.load(modelName);
    ...
    public static ExcelmlWorkbook load(String model) throws Exception {
        String sourceFile = Jade.getInstance().getExternalModelDir() + CEApplication.APPLICATION_HERCULE_ROOT + "/"
                + CEApplication.MODELS_EXCELML_HERCULES + "/" + model;
        ExcelmlWorkbook wk = null;
     ...
     * @throws Exception
     */
    public static void renseigneAdresse(BSession session, String typeAdresse, CEXmlmlLine xmlLine, String idTiers,
            String numAffilie) throws HerculeException {
        ...
        // Rue
        xmlLine.setRue(rue + " " + numRue);
        // Case postale
        xmlLine.setCasePostale(casePostale);
        // NPA
        xmlLine.setNpa(npa);
        // Localité
        xmlLine.setLocalite(ville);
    }
