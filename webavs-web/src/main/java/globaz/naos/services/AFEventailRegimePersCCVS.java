...
public class AFEventailRegimePersCCVS extends AFAbstractEventailRegimeParCCVS {
    ...
    public String getTitle() {
        try {
            return CodeSystem.getLibelle(getLocalSession(), CodeSystem.GENRE_ASS_PERSONNEL);
        } catch (Exception e) {
            return "";
        }
    }
    ...
    public TISummaryInfo[] handleAffiliation(AFAffiliation crt) throws Exception {
        ...
        if (crt != null) {
            ...
            cotisations.setForGenreAssurance(CodeSystem.GENRE_ASS_PERSONNEL);
            cotisations.find();
            if (cotisations.getSize() > 0) {
