...
public class AFIdeAnnonceManager extends BManager implements Serializable {
    ...
    private void createCondition(BTransaction transaction, String typeCondition, StringBuffer sqlCondition,
            String column, String column2, String operation, String valeur) {
        ...
        if (!JadeStringUtil.isBlankOrZero(valeur)) {
            ...
            createCondition(transaction, typeCondition, foo, column, operation, valeur);
            StringBuffer bar = new StringBuffer();
            createCondition(transaction, typeCondition, bar, column2, operation, valeur);

            sqlCondition.append(foo + " OR " + bar + ")");
        }
    ...
    protected String _getWhere(BStatement statement) {
        ...
        if (!JadeStringUtil.isBlankOrZero(likeIdAffiliationLiee)) {
            ...
            String likeIdAffiliationLieeMod = "%" + likeIdAffiliationLiee + "%";
            likeIdAffiliationLieeMod = _dbWriteString(theTransaction, likeIdAffiliationLieeMod);

            sqlWhere.append(ALIAS_TABLE_ANNONCE + AFIdeAnnonce.IDE_ANNONCE_FIELD_LIST_ID_AFFILIATION_LIEE + " LIKE "
                    + likeIdAffiliationLieeMod);

        }

        createCondition(theTransaction, CREATE_CONDITION_STRING, sqlWhere, ALIAS_TABLE_ANNONCE
                + AFIdeAnnonce.IDE_ANNONCE_FIELD_HISTORIQUE_RAISON_SOCIALE, ALIAS_TABLE_AFFILIATION
                + AFAffiliation.FIELDNAME_RAISON_SOCIALE, "LIKE_WITHOUT_CASSE", likeRaisonSociale);

        createCondition(theTransaction, CREATE_CONDITION_STRING, sqlWhere, ALIAS_TABLE_ANNONCE
                + AFIdeAnnonce.IDE_ANNONCE_FIELD_HISTORIQUE_STATUT_IDE, ALIAS_TABLE_AFFILIATION
                + AFAffiliation.FIELDNAME_STATUT_IDE, "=", forStatut);
