...
import globaz.apg.vb.droits.APDroitPaiPViewBean;
 ...
 * @author vre
 */
public class APDroitPaiPHelper extends APAbstractDroitPHelper {
    ...
    protected void _init(FWViewBeanInterface viewBean, FWAction action, BISession session) throws Exception {
        ...
        ((APDroitPaiPViewBean) viewBean).setModeEditionDroit(APModeEditionDroit.CREATION);
    }

    @Override
    protected void _add(FWViewBeanInterface vb, FWAction action, BISession iSession) throws Exception {
        APDroitPaiPViewBean viewBean = null;
        if (!(vb instanceof APDroitPaiPViewBean)) {
            throw new APWrongViewBeanTypeException("Wrong viewBean type for action APDroitPatPHelper._delete ["
                    + vb.getClass().getName() + "]. it must be from type APDroitPaiPViewBean");
        }
        viewBean = (APDroitPaiPViewBean) vb;
        ...
        try {
            ...
            APDroitProcheAidant droit = ApgServiceLocator.getEntityService()
                    .creerDroitPaiComplet(session, transaction, viewBean);
    ...
    protected void _update(FWViewBeanInterface vb, FWAction action, BISession iSession) throws Exception {
        ...
        try {
            ...
            APDroitProcheAidant droit = ApgServiceLocator.getEntityService().miseAjourDroitPai(session, transaction, viewBean);
            viewBean.setDroit(droit);
    ...
    protected void _retrieve(FWViewBeanInterface vb, FWAction action, BISession iSession) throws Exception {
        ...
        try {
            ...
            APDroitProcheAidant droit = ApgServiceLocator.getEntityService().getDroitProcheAidant(session, transaction,
                    viewBean.getIdDroit());
            viewBean.setDroit(droit);
