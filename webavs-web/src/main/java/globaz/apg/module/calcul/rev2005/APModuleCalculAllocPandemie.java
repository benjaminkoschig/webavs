 ...
 *
 */
public class APModuleCalculAllocPandemie extends AAPModuleCalculSalaireJournalier implements IAPModuleCalcul {
    ...
    public APResultatCalcul calculerMontantAllocation(APBaseCalcul baseCalcul, IAPReferenceDataPrestation refData)
        ...
        if (baseCalcul.isSansActiviteLucrative()) {
            throw new APCalculException("Activité lucrative obligatoire pour bénéficier d'une allocation.");
        }

        APResultatCalcul result = super.calculerMontantAllocation(baseCalcul, refData);
        result.setCsGenrePrestion(APTypeDePrestation.PANDEMIE.getCodesystemString());
        // Salaire journalier
