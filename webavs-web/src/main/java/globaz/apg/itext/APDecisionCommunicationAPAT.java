 ...
 * @author vre
 */
public class APDecisionCommunicationAPAT extends FWIDocumentManager {
    ...
    // TODO A remplacer par apat
    private static final String DOC_DEC_APAT_COPIE_ASS = "documents.decision.apat.copie.assure";
    private static final String FICHIER_MODELE = "AP_DECISION_PATERNITE";
    ...
    private static final int STATE_LAPAT = 12;
    ...
    private APDroitPaternite droit;
    ...
    private int state = APDecisionCommunicationAPAT.STATE_ASSURES;

    private int state_dec = APDecisionCommunicationAPAT.STATE_STANDARD;

    public APDecisionCommunicationAPAT() throws FWIException {
        super();
     ...
     *             DOCUMENT ME!
     */
    public APDecisionCommunicationAPAT(final BProcess parent) throws FWIException {
        super(parent, APApplication.APPLICATION_APG_REP, APDecisionCommunicationAPAT.FICHIER_RESULTAT);
    }
     ...
     *             DOCUMENT ME!
     */
    public APDecisionCommunicationAPAT(final BSession session) throws FWIException {
        super(session, APApplication.APPLICATION_APG_REP, APDecisionCommunicationAPAT.FICHIER_RESULTAT);
    }
    ...
    public void beforeBuildReport() {
        try {
            ...
            if (APDecisionCommunicationAPAT.STATE_ASSURES == state) {
                ...
                if (repartitionsEmployeur.hasNext()) {
                    ...
                    state = APDecisionCommunicationAPAT.STATE_EMPLOYEURS;
                } else {
                    state = APDecisionCommunicationAPAT.STATE_FIN;
                }

            } else if (APDecisionCommunicationAPAT.STATE_EMPLOYEURS == state) {
            ...
            getMemoryLog().logMessage(e.getMessage(), FWMessage.ERREUR, "APDecisionCommunicationAPAT");
            abort();
        }
    }
    ...
    public void beforeExecuteReport() {
        try {
            ...
            if (!JadeStringUtil.isEmpty(extensionModelCaisse)) {
                setTemplateFile(APDecisionCommunicationAPAT.FICHIER_MODELE + extensionModelCaisse);
                ...
                if ((sourceFile != null) && sourceFile.exists()) {
                    ...
                    setTemplateFile(APDecisionCommunicationAPAT.FICHIER_MODELE);
                }
            } else {
                setTemplateFile(APDecisionCommunicationAPAT.FICHIER_MODELE);
            }
        ...
        if ((documentAssures == null) || (documentEmployeurs == null)) {
            getMemoryLog()
                    .logMessage("catalogue de texte introuvable", FWMessage.ERREUR, "APDecisionCommunicationAPAT");
            abort();
        }
    }
    ...
    private void buildDecision(ICTDocument document) throws Exception {
        ...
        if (document.equals(documentEmployeurs)) {
            ...
            if (state_dec == APDecisionCommunicationAPAT.STATE_STANDARD) {
                helper.setNom("normal");
            } else if (state_dec == APDecisionCommunicationAPAT.STATE_ACM) {
                helper.setNom("ACM");
            } else if (state_dec == APDecisionCommunicationAPAT.STATE_LAPAT) {
                helper.setNom("Lapat");
            }
        ...
        if (getCsTypeDocument().equalsIgnoreCase(IAPCatalogueTexte.CS_DECISION_PAT)) {
        ...
        final Boolean paramCopie = new Boolean(getSession().getApplication().getProperty(
                APDecisionCommunicationAPAT.DOC_DEC_APAT_COPIE_ASS));
    ...
    private String traitementLevel4Pos3ACMEmployeur(ICTTexte texte) throws Exception {
        ...
        // TODO Vérifier nombre de jour paternité
    ...
    public void chargerCatalogueTexte() {
        ...
        try {
            ...
            helper.setCsTypeDocument(IAPCatalogueTexte.CS_DECISION_PAT); // decision
            // et
            // communication,
            // meme
            // catalogue
            helper.setCsDomaine(IAPCatalogueTexte.CS_PATERNITE);
    ...
    private Object[] completeCorpsAssures(final StringBuffer buffer, final ICTListeTextes textes) throws Exception {
        ...
        } else if (state_dec == APDecisionCommunicationAPAT.STATE_STANDARD) {
            buffer.append(" "); // espace
        ...
        // Si c'est une décision standard (Paternité fédérale)
        if (state_dec == APDecisionCommunicationAPAT.STATE_STANDARD) {
            ...
            // Si c'est une décision Lapat (Paternité cantonale)
        } else if (state_dec == APDecisionCommunicationAPAT.STATE_LAPAT) {
    ...
    private void completePiedAssures(final Map parametres, final StringBuffer buffer, final ICTListeTextes textes)
        ...
        if (IAPCatalogueTexte.CS_DECISION_PAT.equals(csTypeDocument)) {
    ...
    public void createDataSource() {
        try {
             ...
             */
            if ((state == APDecisionCommunicationAPAT.STATE_ASSURES) || !isEmployeursMultiples()) {
                ...
                for (int idPrestation = 0; idPrestation < loadPrestations().size(); ++idPrestation) {
                    ...
                    if (isGenreSameAsStateDecision(prestation)) {
                        ...
                        for (int idRP = 0; idRP < repartitionPaiementsManager.size(); ++idRP) {
                            ...
                            if (state == APDecisionCommunicationAPAT.STATE_ASSURES) {
            ...
            } else if (state == APDecisionCommunicationAPAT.STATE_EMPLOYEURS) {
            ...
            // une copie
            // pas de copie pour les independants
            if ((state == APDecisionCommunicationAPAT.STATE_EMPLOYEURS) && isDecompteRempli && !isIndependant) {
    ...
    private boolean isGenreSameAsStateDecision(final APPrestation prestation) {
        ...
        isGoodStateAndGenre |= APTypeDePrestation.STANDARD.isCodeSystemEqual(prestation.getGenre())
                && state_dec == APDecisionCommunicationAPAT.STATE_STANDARD;
        ...
        isGoodStateAndGenre |= (APTypeDePrestation.ACM_ALFA.isCodeSystemEqual(prestation.getGenre()) || APTypeDePrestation.ACM2_ALFA
                .isCodeSystemEqual(prestation.getGenre())) && state_dec == APDecisionCommunicationAPAT.STATE_ACM;

        // OU
        // Genre de prestation LAPAT ET décision à l'état LAPAT
        isGoodStateAndGenre |= APTypeDePrestation.LAPAT.isCodeSystemEqual(prestation.getGenre())
                && state_dec == APDecisionCommunicationAPAT.STATE_LAPAT;
    ...
    public void createDocInfo() {
        ...
        docInfo.setDocumentType("globaz.apg.itext.APDecisionCommunicationAPAT");
        docInfo.setDocumentTypeNumber(IPRConstantesExternes.DECISION_PATERNITE);
        docInfo.setDocumentTitle(getSession().getLabel("DOC_DEC_COMM_PAT_TITLE"));
        ...
        try {
            ...
            if (!JadeStringUtil.isBlankOrZero(docinfo_affilie_tiers_id)) {
                ...
                if (!JadeStringUtil.isIntegerEmpty(docinfo_affilie_id)) {
                    ...
                    final APDroitPaternite droit = new APDroitPaternite();
                    droit.setSession(getSession());
                    droit.setIdDroit(getIdDroit());
                    droit.retrieve();
            ...
            if (!JadeStringUtil.isBlankOrZero(docinfo_affilie_tiers_id)) {
                ...
                // ORDER_PRINTING_BY
                docInfo.setDocumentProperty(APDecisionCommunicationAPAT.ORDER_PRINTING_BY,
            ...
            e.printStackTrace();
            getMemoryLog().logMessage("APDecisionCommunicationAPAT.afterPrintDocument():" + e.toString(),
                    FWMessage.ERREUR, "APDecisionCommunicationAPAT");
        }
    ...
    public boolean next() throws FWIException {
        ...
        switch (state_dec) {
            ...
            case STATE_STANDARD:
                if (state != APDecisionCommunicationAPAT.STATE_FIN) {
                    suite = true;
                } else {
                    state = APDecisionCommunicationAPAT.STATE_ASSURES;
                    state_dec = APDecisionCommunicationAPAT.STATE_ACM;
                    suite = true;
                }
                break;
            ...
            case STATE_LAPAT:
                if (state != APDecisionCommunicationAPAT.STATE_FIN) {
                    ...
                    state = APDecisionCommunicationAPAT.STATE_ASSURES;
                    state_dec = APDecisionCommunicationAPAT.STATE_FINDEC;
                    suite = false;
                }
                break;
