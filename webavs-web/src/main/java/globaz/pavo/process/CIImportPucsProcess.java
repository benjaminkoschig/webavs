...
public class CIImportPucsProcess extends BProcess {

    private static final long serialVersionUID = -314714724478084902L;

    CIImportPucsDetailResultBean detailResultBean;
    ...
    public static void initEbusinessAccessInstance(EBEbusinessInterface instance) {
        if (CIImportPucsProcess.ebusinessAccessInstance == null) {
            CIImportPucsProcess.ebusinessAccessInstance = instance;
        }
    }
    ...
    private void createInscription(String numeroAVS, String nom, String prenom, PeriodeSalary periode,
        ...
        CIImportPucsDetailResultInscriptionBean aDetailResultInscriptionBean = new CIImportPucsDetailResultInscriptionBean();
        ...
        if (journal == null && !declarationSalaire.isAfSeul()) {
            ...
            if (!JadeStringUtil.isBlankOrZero(montantAVS.getValue())
                ...
                if (mntAf.compareTo(mntAvs) != 0) {
                    if (appCI != null && appCI.getPucs4AutoriseAvsDifferentAf()) {
                        info.add(getSession().getLabel("DT_AF_DIFF_AVS"));
                    } else {
            ...
            List<CIImportPucsDetailResultInscriptionBean> listInscriptionsBean = detailResultBean
                    .getMapAnneeListInscriptions().get(annee);
            if (listInscriptionsBean == null) {
                listInscriptionsBean = new ArrayList<CIImportPucsDetailResultInscriptionBean>();
            }
        ...
        if (modeInscription && !declarationSalaire.isAfSeul() && !isAborted()) {
            ...
            if (!"true".equals(getAccepteLienDraco())) {
                if (!JadeStringUtil.isBlankOrZero(idsPucsFile)
                        && (CIImportPucsProcess.ebusinessAccessInstance != null)) {
                    try {

                        List<String> ids = Splitter.on(";").trimResults().splitToList(idsPucsFile);
                        for (String id : ids) {
                            CIImportPucsProcess.ebusinessAccessInstance.notifyFinishedPucsFile(id, provenance,
                                    getSession());
    ...
    private void initResultBean() {
        detailResultBean = new CIImportPucsDetailResultBean();
