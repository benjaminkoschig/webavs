...
public class CIDeclarationPUCS4 extends BProcess {
    ...
    public CIDeclarationPUCS4() {
        super();
    }

    public CIDeclarationPUCS4(BProcess parent) {
        super(parent);
    }

    public CIDeclarationPUCS4(globaz.globall.db.BSession session) {
        super(session);
    }
        ...
        if (CIDeclaration.CS_AC.equals(getType()) || CIDeclaration.CS_AMI.equals(getType())) {
            ...
            try {
                ...
                while (itDec.hasNext() && !getTransaction().hasErrors()) {
                    ...
                    if ((JadeStringUtil.isEmpty(getAnneeCotisation())) || (rec.getAnnee().equals(getAnneeCotisation()))) {
                        ...
                        if ((JadeStringUtil.isEmpty(forNumeroAffilieSansPoint))
                            ...
                            if (traitementAFSeul
                                    && rec.getNomAffilie().equalsIgnoreCase(
                            ...
                            if (!traitementAFSeul) {
                                if (!CIDeclaration.CS_COT_PERS.equals(Type)) {
                                    ...
                                    if (Integer.parseInt(rec.getAnnee()) == JACalendar.getYear(JACalendar
                                            .todayJJsMMsAAAA())) {
                            ...
                            if (journal == null && !traitementAFSeul) {
                                 ...
                                 */
                                hJournalExisteDeja.put(key,
                                        getSession().getLabel("DT_JOURNAL_NON_CREE") + " " + rec.getNumeroAffilie()
                                                + "/" + rec.getAnnee());
                            } else {
                                ...
                                if ((anneeNaissance + 1918) > Integer.parseInt(rec.getAnnee())) {
                                    ...
                                    if (CIDeclarationPUCS4.isAvs0(noAvs)) {
                                    ...
                                    if (CICompteIndividuel.CS_REGISTRE_PROVISOIRE.equals(ecriture.getCI(
                                        ...
                                        for (int i = 0; i < ecrMgr.size(); i++) {
                                            ...
                                            if ((ecr.getMoisDebut().equals(rec.getMoisDebut() + ""))
                                                ...
                                                if ((JadeStringUtil.isEmpty(ecriture.getExtourne()) && "0".equals(ecr
                                                        .getExtourne()))
                ...
                if (modeInscription && !traitementAFSeul && !isAborted()) {
                    ...
                    if (!"true".equals(getAccepteLienDraco())) {
                        if (!JadeStringUtil.isBlankOrZero(idsPucsFile)
                                && (CIDeclarationPUCS4.ebusinessAccessInstance != null)) {
                            try {

                                List<String> ids = Splitter.on(";").trimResults().splitToList(idsPucsFile);
                                for (String id : ids) {
                                    CIDeclarationPUCS4.ebusinessAccessInstance.notifyFinishedPucsFile(id, provenance,
                                            getSession());
                                }
    ...
    private void preparerDonnneeRapportExcelMlAFSeule(CIDeclarationRecord rec, Map<String, String> sommeParCanton) {
        for (String mapKey : sommeParCanton.keySet()) {
            ...
            try {
                ...
                theContainerRapportExcelmlImportedFileAFSeule.put(
                        "COL_INFO_AFFILIE_TRANSMIS",
                        rec.getAffilieFichierNom() + "\n" + rec.getAffilieFichierStreet() + "\n"
                                + rec.getAffilieFichierNpa() + " " + rec.getAffilieFichierCity());
                theContainerRapportExcelmlImportedFileAFSeule.put(
                    ...
                    if (JadeStringUtil.isEmpty(csCanton)) {
                        _addError(
                                ...
                                getSession().getLabel("AFSEUL_CANTON_ERRONE") + " (" + rec.getCodeCanton() + ")"
                                        + " - " + getSession().getLabel("DEC_AFFILIE") + " "
        ...
        while (iter.hasNext()) {
            ...
            if (chaine.length() > 5) {
                ...
                _updateSummary(hNbrInscriptionsTraites, hMontantInscritionsTraites, hNbrInscriptionsErreur,
                        ...
                        hMontantInscriptionsNegatives, 0, new FWCurrency(0), 0, new FWCurrency(0), 0,
                        new FWCurrency(0), 0, new FWCurrency(0), 0, new FWCurrency(0), hNbrInscriptionsTotalControle,
                        (0), hMontantTotalControle, new FWCurrency(0), _getKey(rec));
