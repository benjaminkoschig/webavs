...
public abstract class CIAnnonceCollaborateurService {
    ...
    public static CITreatAclSaisieResult treatAclSaisie(BTransaction transaction, String idAnnonceCollaborateur,
            ...
            String noSuccursale, String nomEmploye, String prenomEmploye, String dateNaissanceEmploye,
            String sexeEmploye, String nationaliteEmploye) throws Exception {
        try {
            ...
            if (CIAnnonceCollaborateurService.isNavs13(nss)) {
                if (duplicata.booleanValue()) {
                    ...
                    if (compteIndividuel != null) {
                        ...
                        CIAnnonceCollaborateurService.genererArc(transaction.getSession(), nss, numeroAffilie,
                                dateEngagement, CIAnnonceCollaborateurService.ARC_MOTIF_31,
                                refInterne + CIAnnonceCollaborateurService.REFINTERNE_EXCEPTION, noEmploye,
                                noSuccursale);
                    } else {
                        NSSinfo nssInfo = null;
                        if (infosAlreadyExistFromUPI(nss, nomEmploye, prenomEmploye, dateNaissanceEmploye, sexeEmploye,
                                nationaliteEmploye)) {
                            nssInfo = buildNSSInfosFromUPI(nss, nomEmploye, prenomEmploye, dateNaissanceEmploye,
                                    sexeEmploye, nationaliteEmploye);
                        } else {
                            nssInfo = CIAnnonceCollaborateurService.findInNssra(transaction.getSession(), nss);
                        }
                        if (nssInfo != null) {
                ...
                if (infosAlreadyExistFromUPI(nss, nomEmploye, prenomEmploye, dateNaissanceEmploye, sexeEmploye,
                        nationaliteEmploye)) {
                    nssInfo = buildNSSInfosFromUPI(nss, nomEmploye, prenomEmploye, dateNaissanceEmploye, sexeEmploye,
                            nationaliteEmploye);
                } else {
                    nssInfo = CIAnnonceCollaborateurService.findInNssra(transaction.getSession(), nss);
                }
    ...
    private static NSSinfo buildNSSInfosFromUPI(String nss, String nomEmploye, String prenomEmploye,
            String dateNaissanceEmploye, String sexeEmploye, String nationaliteEmploye) {
        NSSinfo nssInfo = new NSSinfo();
        nssInfo.setNNSS(nss);
        nssInfo.setNomPrenom(nomEmploye + "," + prenomEmploye);
        nssInfo.setDateNaissance(dateNaissanceEmploye);
        nssInfo.setSexe(sexeEmploye);
        nssInfo.setCodeNationnalite(nationaliteEmploye);
        return nssInfo;
    }

    private static boolean infosAlreadyExistFromUPI(String nss, String nomEmploye, String prenomEmploye,
            String dateNaissanceEmploye, String sexeEmploye, String nationaliteEmploye) {
        return !JadeStringUtil.isBlankOrZero(nss) && !JadeStringUtil.isBlankOrZero(nomEmploye)
                && !JadeStringUtil.isBlankOrZero(prenomEmploye) && !JadeStringUtil.isBlankOrZero(dateNaissanceEmploye)
                && !JadeStringUtil.isBlankOrZero(sexeEmploye) && !JadeStringUtil.isBlankOrZero(nationaliteEmploye);
    }
}
