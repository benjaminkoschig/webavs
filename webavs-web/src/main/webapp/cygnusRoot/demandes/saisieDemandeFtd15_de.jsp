...
<script language="JavaScript">
	...
	function ajoutDevisListIdDevis(){
		var selectedDevis=new Array();
		$("input[type=checkbox][name=devis]:checked").each(function() {
			var numeroDevis = parseInt($(this).attr('value'));
			selectedDevis.push($(this).attr('value')+"-"+$("#idMontantAssocie_"+numeroDevis).val());
		});
		
		$("[name=listIdDevis]").val(selectedDevis.join(','));
		
		//majMontantFacture();
	}
	
	function majMontantFacture(){
		...
		$("input[type=checkbox][name=devis]:checked").each(function() {
			var numeroDevis = parseInt($(this).attr('value'));
			total = total + parseFloat($("#idMontantAssocie_"+numeroDevis).val().replace(decote,""));
		});
	
		$('[name="montantFacture"]').val(demandeUtils.formatMnt(parseFloat(total).toFixed(2)));
...
<%-- tpl:put name="zoneMain" --%>
				...
				<TR style="vertical-align:top" >
					...
					<TD colspan="5">
						...
						<INPUT type="hidden" id="typeDemande" name="typeDemande" value="<%=RFSaisieDemandeAbstractViewBean.TYPE_DEMANDE_FTD15%>" />
              ...
              <%}%>
                ...
                <TR valign="top">
                	<!-- Liste des devis -->
					<TD><ct:FWLabel key="JSP_RF_DEM_S_LISTE_DEVIS"/></TD>
	                <TD colspan="5">
							<TABLE bgcolor="white" cellpadding="0" cellspacing="0" style="vertical-align: top">
								<TR>
									<TH>&nbsp;</TH>
									<TH><font size="2"><ct:FWLabel key="JSP_RF_DEM_S_LISTE_DEVIS_NO"/></font></TH>
									<TH><font size="2"><ct:FWLabel key="JSP_RF_DEM_S_LISTE_DEVIS_DATE_DEVIS"/></font></TH>
									<TH><font size="2"><ct:FWLabel key="JSP_RF_DEM_S_LISTE_DEVIS_DATE_DE_RECEPTION_PRE_MDC"/></font></TH>
									<TH><font size="2"><ct:FWLabel key="JSP_RF_DEM_S_LISTE_DEVIS_MONTANT_DEVIS"/></font></TH>
									<TH><font size="2"><ct:FWLabel key="JSP_RF_DEM_S_LISTE_DEVIS_MONTANT_ACCEPTE"/></font></TH>
									<TH><font size="2"><ct:FWLabel key="JSP_RF_DEM_S_LISTE_DEVIS_MONTANT_SOLDE"/></font></TH>
									<TH><font size="2"><ct:FWLabel key="JSP_RF_DEM_S_LISTE_DEVIS_MONTANT_DEMANDE"/></font></TH>
								</TR>
								<!-- pour chaque ligne de la liste de Devis -->
								<%
									Vector<String[]> listDevisVec = viewBean.getListDevisData();
									for (int i=0;i<listDevisVec.size();i++){
										String[] devis = (String[]) listDevisVec.get(i);%>
										<TR>
											<TD style="border-right=1px solid #C0C0C0">
												<%if(!JadeStringUtil.isBlankOrZero(viewBean.getMontantAssocieListIdDevis(devis[0]))){%>
													<INPUT type="checkbox" name="devis" value="<%=devis[0]%>" onclick="ajoutDevisListIdDevis();" checked/>
												<%}else{%>
													<INPUT type="checkbox" name="devis" value="<%=devis[0]%>" onclick="ajoutDevisListIdDevis();" />
												<%}%>
											</TD>
											<TD style="border-right=1px solid #C0C0C0;text-align:center;"><%=devis[0]%></TD>
											<TD style="border-right=1px solid #C0C0C0;text-align:center;"><%=devis[1]%></TD>
											<TD style="border-right=1px solid #C0C0C0;text-align:center;"><%=devis[2]%></TD>
											<TD style="border-right=1px solid #C0C0C0;text-align:right;"><%=devis[3]%></TD>
											<TD style="border-right=1px solid #C0C0C0;text-align:right;"><%=devis[4]%></TD>
											<TD style="border-right=1px solid #C0C0C0;text-align:right;"><%=devis[5]%></TD>
											<TD style="border-right=1px solid #C0C0C0;text-align:center;">
												 <INPUT type="text" name="montantAssocieDevis" id="idMontantAssocie_<%=devis[0]%>" value="<%=viewBean.getMontantAssocieListIdDevis(devis[0])%>" class="montant" onchange="ajoutDevisListIdDevis();" onkeypress="return filterCharForFloat(window.event);" />
											</TD>
										</TR>
								<%}%>
							</TABLE>
							<INPUT type="hidden" name="listIdDevis" value="<%=viewBean.getListIdDevis()%>"/>
					</TD> 
                	<!-- Fin liste des devis -->
                </TR>
                <TR><TD colspan="6">&nbsp;</TD></TR>
