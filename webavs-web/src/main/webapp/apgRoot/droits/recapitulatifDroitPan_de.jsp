...
<%
    ...
    globaz.apg.vb.droits.APRecapitulatifDroitPanViewBean viewBean = (globaz.apg.vb.droits.APRecapitulatifDroitPanViewBean) session.getAttribute("viewBean");
...
<ct:menuChange displayId="options" menuId="ap-optionmenudroitpan" showTab="options">
    ...
    <%
        if (!(Arrays.asList(IAPDroitLAPG.CS_ETAT_DROIT_DEFINITIF, IAPDroitLAPG.CS_ETAT_DROIT_PARTIEL).contains(viewBean.getEtat()) && APGUtils.isGenreServiceAvecDateFin(viewBean.getGenreService()) && viewBean.getDateFinDroit().isEmpty())) {
    %>
    <ct:menuActivateNode nodeId="finDeDroit" active="no"/>
    <%
    } else {
    %>
    <ct:menuActivateNode nodeId="finDeDroit" active="yes"/>
    <%
        }
    %>
    <%
        if (Arrays.asList(IAPDroitLAPG.DROITS_MODIFIABLES).contains(viewBean.getEtat()) && !viewBean.getEtat().equals(IAPDroitLAPG.CS_ETAT_DROIT_ATTENTE_REPONSE)) {
    %>
    <ct:menuActivateNode nodeId="attenteReponse" active="yes"/>
    <%
    } else {
    %>
    <ct:menuActivateNode nodeId="attenteReponse" active="no"/>
    <%
        }
    %>
    <%
    ...
    %>
    <ct:menuActivateNode nodeId="refuser" active="yes"/>
    <%
    } else {
    %>
    <ct:menuActivateNode nodeId="refuser" active="no"/>
    <%
        }
    %>
</ct:menuChange>
...
<SCRIPT>
    ...
    function upd() {
        document.forms[0].elements('userAction').value = "apg.droits.droitPan.afficher";
    ...
    function del() {
        if (window.confirm("<ct:FWLabel key='JSP_DELETE_MESSAGE_INFO'/>")) {
            document.forms[0].elements('userAction').value = "apg.droits.droitPan.supprimer";
            document.forms[0].submit();
        }
    }
...
<span class="postItIcon"><ct:FWNote sourceId="<%=viewBean.getIdDroit()%>"
                                    tableSource="<%=APApplication.KEY_POSTIT_DROIT_APG_MAT%>"/></span>
<ct:FWLabel key="JSP_TITRE_RECAPITULATIF_PAN"/>
<%-- /tpl:put --%>
...
<TR>
    ...
    <TD COLSPAN="5">
        ...
        <A href="#" onclick="window.open('<%=servletContext%>
            <%=("/apg")%>?userAction=<%=globaz.apg.servlet.IAPActions.ACTION_DROIT_LAPG%>.actionAfficherDossierGed&amp;noAVSId=<%=viewBean.getNoAVS()%>&amp;serviceNameId=<%=viewBean.getSession().getApplication().getProperty(globaz.externe.IPRConstantesExternes.PROPERTY_AFFICHAGE_DOSSIER_GED)%>')"><ct:FWLabel
                key="JSP_GED"/></A>
        &nbsp;/&nbsp;<A
            href="<%=request.getContextPath()%>\pyxis?userAction=pyxis.tiers.tiers.diriger&selectedId=<%=viewBean.getTiers()%>"><ct:FWLabel
            key="JSP_TIERS"/></A>
    </TD>
</TR>
<TR>
...
<TR>
    ...
    <TD COLSPAN="5">
        <TABLE BORDER="3">
            ...
            %>
            <TR>
                <TD><%=employeur.getNoAffilie()%>
                </TD>
                <TD><%=employeur.getNom()%>
                </TD>
                <TD style="text-align: center">
                    <IMG SRC="<%=request.getContextPath()+(!employeur.getIsVersementEmployeur().booleanValue()?"/images/ok.gif":"/images/erreur.gif")%>"
                         ALT="">
                </TD>
                <TD><A href="#" onclick="window.open('<%=servletContext%>
                    <%=("/apg")%>?userAction=<%=globaz.apg.servlet.IAPActions.ACTION_DROIT_LAPG%>.actionAfficherDossierGed&amp;noAffiliationId=<%=employeur.getNoAffilie()%>&amp;noAVSId=<%=employeur.retrieveNoAVS()%>&amp;serviceNameId=<%=viewBean.getSession().getApplication().getProperty(globaz.externe.IPRConstantesExternes.PROPERTY_AFFICHAGE_DOSSIER_GED_COTI)%>')"><ct:FWLabel
                        key="JSP_GED"/></A>&nbsp;/&nbsp;
                    <A href="<%=request.getContextPath()%>\pyxis?userAction=pyxis.tiers.tiers.diriger&selectedId=<%=employeur.getIdTiers()%>"><ct:FWLabel
                            key="JSP_TIERS"/></A></TD>
            </TR>
            <% }
            } %>
    ...
    <TD COLSPAN="5">
        <TABLE BORDER="3">
            ...
            <%
                ...
                globaz.apg.db.droits.APEnfantPan enfant = (globaz.apg.db.droits.APEnfantPan) enfants.next();
            %>
            <TR>
                <TD><%=enfant.getNoAVS()%>
                </TD>
