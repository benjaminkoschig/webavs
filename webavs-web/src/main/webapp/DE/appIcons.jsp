...
<table class="menutable">
    <tr>
        <TD id="tdApp" width="30" class="selectable" onmouseover="chgCellStyle(this,'Over');"
            onmouseout="chgCellStyle(this,'Out');" onclick="javascript:showMainMenu();" nowrap
            style="background-color: <%=Jade.getInstance().getWebappBackgroundColor()%>;">
            <DIV align="CENTER"><B><FONT face="Lucida Sans Unicode"><A
                    href="javascript:showMainMenu();">Anwendungen...</A></FONT></B></DIV>
        </TD>
        <td align='right' nowrap>
            <input style="position: absolute; top: -9999px" type="button" accesskey="q"
                   onclick="top.fr_main.location.href='<%=request.getContextPath()%>/pyxis?userAction=pyxis.tiers.summary.afficher'">
            <a href="#"
               onclick="top.fr_main.location.href='<%=request.getContextPath()%>/pyxis?userAction=pyxis.tiers.summary.afficher'"><IMG
                    id="icon_help" SRC="<%=request.getContextPath()%>/images/evr_find.png" ALT="Überblick [ALT+Q]"
                    border="0"/></a>

            <%
                if (!globaz.jade.client.util.JadeStringUtil.isBlank(globaz.jade.common.Jade.getInstance().getExternalHelpProvider())) {
            %>
            <a href="<%=request.getContextPath() + "/fx"%>?userAction=framework.external.help" target="_blank"><IMG
                    id="icon_help" SRC="<%=request.getContextPath()%>/images/inforom.gif" ALT="InfoRom" border="0"/></a>
            <%
                }
            %>
            <a href="javascript:showProcess()"><IMG id="icon_process"
                                                    SRC="<%=request.getContextPath()%>/images/icon_process.png"
                                                    ALT="Laufende Prozesse" border="0"/></a>
            <a href="javascript:imprime()"><IMG id="icon_print"
                                                SRC="<%=request.getContextPath()%>/images/icon_print.png" ALT="Drucken"
                                                border="0"/></a>
            <a href="javascript:quit()"><IMG id="icon_quit" SRC="<%=request.getContextPath()%>/images/icon_exit.png"
                                             ALT="Beenden" border="0"/></a>
            <a href="javascript:back()"><IMG id="icon_back" SRC="<%=request.getContextPath()%>/images/icon_back.png"
                                             ALT="Zurück" border="0"/></a>
        </td>
        <td>&nbsp;</td>
    </tr>
</table>
...
<div id="appIconsApplicationBodyContent" class="hidden" onmousemove="clearTimeout(hideTimeout)"
     onmouseleave="hideTimeout = setTimeout(hideMainMenu, 500)">
    <div id="outilsMenu">
                    ...
                    <ul>
                        ...
                        <li>
                            <a href="<%=request.getContextPath()%>/eform" target="_top">
                                Verwaltung von SEDEX-Austauschs
                            </a>
                        </li>
                        <%}%>
