  ...
  var autoCompleteElemVal;
  var autoCompleteElemId;
  
  function autoComplete(elem,query,elemVal,elemId) {
	autoCompleteElem = elem;
	autoCompleteElemVal = elemVal;
	autoCompleteElemId = elemId;
  ...
  function handlerAutoComplete() {
	...
	if ((req1.status == 0)||(req1.status == 200)) {
		...
		document.getElementById('zoneAutoComplete').style.width = autoCompleteElem.offsetWidth+200;
  ...
  function autoCompleteAction(elem) {
  	...
  	if (event.keyCode!= 27) { 
			autoCompleteElemVal.value = elem.options[elem.selectedIndex].name;
			autoCompleteElemId.value = elem.options[elem.selectedIndex].value;
