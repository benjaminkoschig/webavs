...
top.document.title = "CI - Détail du compte individuel"
...
//   	mainCompteIndBean.retrieve();	   	
    ...
    String sure = "Etes-vous sur ?";
 	...
 	if("DE".equals(languePage)) {
 		detacher="VNR-Entketten";
 		activer="Aktueller VA";
...
<%@ include file="/theme/detail/bodyStart.jspf" %>
			<%-- tpl:put name="zoneTitle" --%>Détail du compte individuel<%-- /tpl:put --%>
<%@ include file="/theme/detail/bodyStart2.jspf" %>
          ...
          <tr> 
            <td>Dernier motif ARC (ouverture/clôture)</td>
            <td colspan="3"> 
          ...
          <tr> 
            <td>Dernière caisse de compensation</td>
            <td colspan=3> 
              <% if (!isSpecialist) {%>
          ...
          <tr> 
            <td>Compte individuel sécurisé</td>
            <td colspan=3> 
			<% if (isSpecialist) {%> 
          ...
          <tr> 
            ...
            if (isSpecialist) {%>
             ...
             <%if(isReopen){%>
              	<option value="on" <%=(viewBean.isCiOuvert().booleanValue())?"SELECTED":""%>>réouvert</option>
             <%}%>
              <option value="" <%=(!viewBean.isCiOuvert().booleanValue())?"SELECTED":""%>>fermé</option>
              <option value="on"<%="add".equalsIgnoreCase(request.getParameter("_method"))?"SELECTED":""%> <%=(viewBean.isCiOuvert().booleanValue() && !isReopen)?"SELECTED":""%>>ouvert</option>
              </select>
            <%} else {             
          ...
          <tr> 
            <td>CI provenant d'un fusion</td>
            <td colspan=3>
            <%if (isSpecialist) {%>
              <select name="provenanceFusion" style="width:3cm">
              <option value="on" <%=(viewBean.isProvenanceFusion().booleanValue())?"SELECTED":""%>>oui
              <option value="" <%=(!viewBean.isProvenanceFusion().booleanValue())?"SELECTED":""%>>non
              </select>
            <%} else { 
