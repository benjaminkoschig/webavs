...
<%
    ...
    String token = IJAcorTokenServiceImpl.createTokenIJDecompte(bSession,
...
<TR>
    ...
    <TD colspan="3"><INPUT type="text" name="datePrononce" value="<%=viewBean.getDatePrononce()%>" class="date disabled"
                           readonly></TD>
</TR>
<TR>
    <TD><LABEL for="numeroBaseIndemnisation"><ct:FWLabel key="JSP_CAD_D_NO_BASE_INDEMNISATION"/></LABEL></TD>
    <TD><INPUT type="text" name="idBaseIndemnisation" value="<%=viewBean.getIdBaseIndemnisation()%>"
               class="numero disabled" readonly></TD>
    <TD>
        <LABEL for="dateDebutBaseIndemnisation"><ct:FWLabel key="JSP_DU"/></LABEL>&nbsp;
        <INPUT type="text" name="dateDebutBaseIndemnisation" value="<%=viewBean.getDateDebutBaseIndemnisation()%>"
               class="date disabled" readonly>
    </TD>
    <TD>
        <LABEL for="dateDebutBaseIndemnisation"><ct:FWLabel key="JSP_AU"/></LABEL>&nbsp;
        <INPUT type="text" name="dateDebutBaseIndemnisation" value="<%=viewBean.getDateFinBaseIndemnisation()%>"
               class="date disabled" readonly>
    </TD>
</TR>
...
<TR>
    ...
    <TD><INPUT type="text" name="noIJCalculee" value="<%=ijijCalculee.getIdIJCalculee()%>" class="numero disabled"
               readonly></TD>
    <TD>
        <LABEL for="dateDebutIJCalculee"><ct:FWLabel key="JSP_DU"/></LABEL>&nbsp;
        <INPUT type="text" name="dateDebutIJCalculee" value="<%=ijijCalculee.getDateDebutDroit()%>"
               class="date disabled" readonly>
    </TD>
    <TD>
        <LABEL for="dateFinIJCalculee"><ct:FWLabel key="JSP_AU"/></LABEL>&nbsp;
        <INPUT type="text" name="dateFinIJCalculee" value="<%=ijijCalculee.getDateFinDroit()%>" class="date disabled"
               readonly>
    </TD>
</TR>
...
<TR>
    <TD colspan="4"><br/>
        ...
        <SELECT name="noRevisionGaranti">
            ...
            %>
            <OPTION value="" <%=viewBean.getDefaultNoRevision().equals(revs[idx].getNoRevision()) ? "selected" : ""%>>
                <ct:FWLabel key="JSP_CAD_REVISION"/> <%=revs[idx].getNoRevision()%> <ct:FWLabel
                    key="JSP_CAD_MONTANT_INT_EXT"/> <%=revs[idx].getMontant()%>
            </OPTION>
            <%
            } else {
            %>
            <OPTION value="<%=revs[idx].getNoRevision()%>" <%=viewBean.getDefaultNoRevision().equals(revs[idx].getNoRevision()) ? "selected" : ""%>>
                <ct:FWLabel key="JSP_CAD_REVISION"/> <%=revs[idx].getNoRevision()%> <ct:FWLabel
                    key="JSP_CAD_MONTANT_INT_EXT"/> <%=revs[idx].getMontant()%>
            </OPTION>
            <% }
        ...
        <table border="1" width="100%">
            ...
            <tr>
                <td>
                    ...
                    <p>ANCIEN ACOR</p>
                    <p>---------------------------------------------------------------------------------------------</p>
                    <H6><A style="color:black;" href="#" onclick="exporterScriptACOR()"><ct:FWLabel
                            key="JSP_CAD_D_ETAPE_1"/></A></H6>
                    <P><A href="#" onclick="exporterScriptACOR2()"><ct:FWLabel key="JSP_CAD_D_TELECHARGER_SCRIPT"/></A>
                    </P>
                    <H6><u><ct:FWLabel key="JSP_CAD_D_ETAPE_2"/></u></H6>
                    <P><ct:FWLabel key="JSP_CAD_D_CALCULER_ACOR"/></P>
                    <H6><u><ct:FWLabel key="JSP_CAD_D_ETAPE_3"/></u></H6>
...
<INPUT class="btnCtrl" type="button" id="btnPrecedant" value="<ct:FWLabel key="JSP_IJ_PRECEDANTE"/>"
       onclick="ijPrecedante()">
<% } %>
