...
KingTour = (function() {
  ...
  function _transformTourDefToSteps(tourDef) {
    ...
    for (var i = 0; i < children.length; i++) {
      ...
      var url_matches = new RegExp(''+page.url+'', "gi").test(location.href)||page.url=="#";
...
KingTour.Dialog = (function(){
  ...
  */
  ...
  return {
    ...
    nextPrev: function (n_increment){
      // we will not change url
      var callback;
      if (KingTour.__steps[KingTour.__currentStep].pageUrl == KingTour.__steps[KingTour.__currentStep +n_increment].pageUrl) {
        if (callback = KingTour.__steps[KingTour.__currentStep].callback) {
          eval(callback + '(true)');
        }

        KingTour.__currentStep = KingTour.__currentStep+n_increment;
      ...
      var prevStep = KingTour.__currentStep + n_increment,
    ...
    attachToExpose: function(pos, o_element) {
      _pos = pos;
      var dialog = o_element ||jQuery(KingTour.dialogSel)[0],
...
KingTour.Expose = (function() {
  ...
  function _createCover (s_cover) {
        ...
        return ( 0 === $cover.length )
                    ? jQuery('<div id="'+s_cover+'" class="kCover"></div>').appendTo("body")
                    : $cover ;
