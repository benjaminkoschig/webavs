...
top.document.title = "Facturation - Détail d'un journal"
...
<%@ include file="/theme/detail/bodyStart.jspf" %>
			<%-- tpl:put name="zoneTitle" --%>Détail d'un journal de facturation<%-- /tpl:put --%>
<%@ include file="/theme/detail/bodyStart2.jspf" %>
	...
	<TR>
            <TD nowrap width="140">Délai en jours</TD>
         ...
         <TR>
            ...
            <%
	              if (!globaz.jade.client.util.JadeStringUtil.isIntegerEmpty(viewBean.getIdJournal())) {
	              	%>
	              <A href="<%=request.getContextPath()%>/osiris?userAction=osiris.comptes.apercuJournal.afficher&selectedId=<%=viewBean.getIdJournal()%>" class="external_link">Journal associé</A>
	              	<%
	              } else {
	              %>
					Journal associé
   ...
   <TR>
            <TD nowrap width="140">Propriétaire</TD>
...
<%
	if(globaz.musca.db.facturation.FAPassage.CS_ETAT_COMPTABILISE.equalsIgnoreCase(viewBean.getStatus())
		|| globaz.musca.db.facturation.FAPassage.CS_ETAT_ANNULE.equalsIgnoreCase(viewBean.getStatus()) ){
		%> window.alert("Ce passage est soit annulé soit comptabilisé. Impossible de le modifier.");<%
	}
%>
</script>
