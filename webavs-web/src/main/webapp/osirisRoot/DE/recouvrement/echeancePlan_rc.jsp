...
<%@ include file="/theme/find/bodyStart.jspf" %>
				<%-- tpl:put name="zoneTitle" --%>Fälligkeiten<%-- /tpl:put --%>
<%@ include file="/theme/find/bodyStart2.jspf" %>
						<%-- tpl:put name="zoneMain" --%>
						<INPUT type="hidden" name="forIdPlanRecouvrement" value="<%=viewBean.getIdPlanRecouvrement()%>">
						<TR>
							<TD class="label"><A href="osiris?userAction=osiris.comptes.apercuComptes.afficher&selectedId=<%=viewBean.getIdCompteAnnexe()%>">Konto</A></TD>
						...
						<TR>
							<TD>&nbsp;</TD>
							<TD class="label">Saldo des Kontos</TD>
							<TD class="control"><INPUT type="text" value="<%=compteAnnexeSoldeFormate%>" class="montantDisabled" readonly></TD>
						</TR>
						<TR>
							<TD class="label"><a href="osiris?userAction=osiris.recouvrement.planRecouvrement.afficher&selectedId=<%=viewBean.getIdPlanRecouvrement() %>">Zahlungsvereinbarung</a></TD>
							...
							<TD class="label">Saldo der Zahlungsvereinbarung</TD>
...
<%@ include file="/theme/find/bodyButtons.jspf" %>
				...
				<input id="eButton" type="button" value="Fälligkeiten berechnen" onClick="fr_list.location.href='<%=servletContext + mainServletPath %>?userAction=osiris.recouvrement.echeancePlan.calculer&mode=preview&selectedId=<%=viewBean.getIdPlanRecouvrement()%>';">
				<%}%>
				<%-- /tpl:put --%>
