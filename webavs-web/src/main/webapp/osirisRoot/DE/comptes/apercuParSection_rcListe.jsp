...
<%@page import="globaz.osiris.api.APICompteAnnexe" %>
...
<TH align="left">Status eBill</TH>
<TH align="left">Fehler eBill</TH>
...
<TD class="mtd" width="16">
    <ct:menuPopup menu="CA-DetailSection" label="<%=optionsPopupLabel%>" target="top.fr_main"
                  detailLabel="<%=menuDetailLabel%>" detailLink="<%=(detailLink+_ligne.getIdSection())%>">
...
<TD class="mtd" nowrap onClick="<%=actionDetail%>" width="70"><%=_ligne.getIdExterne()%>
</TD>
<TD class="mtd" nowrap onClick="<%=actionDetail%>" nowrap width="90"><%=_ligne.getDateSection()%>
</TD>
<TD class="mtd" nowrap onClick="<%=actionDetail%>">
    <% if (!globaz.jade.client.util.JadeStringUtil.isDecimalEmpty(_ligne.getIdRemarque())) { %>
    <img src="<%=request.getContextPath()%>/images/attach.png" style="float:right;"
         title="<%=_ligne.getRemarque().getTexte()%>">
    <% } %>
...
%>
<TD class="mtd" nowrap onClick="<%=actionDetail%>" width="110"
    align="right"><%=globaz.globall.util.JANumberFormatter.formatNoRound(bSolde.subtract(bPmtCmp).toString(), 2)%>
</TD>
<TD class="mtd" nowrap onClick="<%=actionDetail%>" width="110" align="right"><%=_ligne.getPmtCmpFormate()%>
</TD>
<TD class="mtd" nowrap onClick="<%=actionDetail%>" width="110" align="right"><%=_ligne.getSoldeFormate()%>
</TD>
<TD class="mtd" nowrap onClick="<%=actionDetail%>" width="20"
    align="right"><%if (_ligne.getAttenteLSVDD().booleanValue()) {%> <IMG src="<%=servletContext%>/images/pending.gif"
                                                                          title="Anhängig LSV/DD"><%} else {%>&nbsp;<%}%>
</TD>
