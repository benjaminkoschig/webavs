...
<SCRIPT language="JavaScript">
    ...
    function updateIdCompteAnnexe(tag) {
        if (tag.select && tag.select.selectedIndex !== -1) {
            ...
            document.getElementById('idExterneRoleFromScreen').value = tag.select[tag.select.selectedIndex].value;
...
<TR>
    ...
    <TD class="control">
        ...
        <input type="hidden" name="idExterneRoleFromScreen" value="<%=viewBean.getIdExterneRole()%>">
    ...
    <TD class="control"><input type="text" name="descCompteAnnexe" maxlength="29"
                               value="<%=viewBean.getCompteAnnexeNom()%>" class="libelleLongDisabled" tabindex="-1"
                               readonly></TD>
</TR>
...
<TR>
    ...
    <TD class="control">
        ...
        <ct:FWListSelectTag
                data="<%=CAAvanceViewBean.modesPourUtilisateurCourant(objSession, CAAvanceViewBean.CODES_EXCLUS_POUR_ECRANS, false)%>"
                defaut="<%=viewBean.getIdModeRecouvrement()%>" name="idModeRecouvrement"/>
    </TD>
</TR>
...
<TR>
    ...
    <TD class="control">
        <SELECT id="idTypeEcheance" name="idTypeEcheance">
            <OPTION <%if (viewBean.getIdTypeEcheance().equals(globaz.osiris.db.avance.CAAvanceViewBean.CS_ECHEANCE_ANNUELLE)) {%>selected<%}%>
                    value="<%=globaz.osiris.db.avance.CAAvanceViewBean.CS_ECHEANCE_ANNUELLE%>">Annuelle
            </OPTION>
            <OPTION <%if (viewBean.getIdTypeEcheance().equals(globaz.osiris.db.avance.CAAvanceViewBean.CS_ECHEANCE_MENSUELLE)) {%>selected<%}%>
                    value="<%=globaz.osiris.db.avance.CAAvanceViewBean.CS_ECHEANCE_MENSUELLE%>">Mensuelle
            </OPTION>
            <OPTION <%if (viewBean.getIdTypeEcheance().equals(globaz.osiris.db.avance.CAAvanceViewBean.CS_ECHEANCE_TRIMESTRIELLE)) {%>selected<%}%>
                    value="<%=globaz.osiris.db.avance.CAAvanceViewBean.CS_ECHEANCE_TRIMESTRIELLE%>">Trimestrielle
            </OPTION>
        </SELECT>
    </TD>
...
<TR>
    ...
    <TD class="control"><ct:FWCalendarTag name="date" doClientValidation="CALENDAR"
                                          value="<%=viewBean.getDate()%>"/></TD>
...
<TR>
    ...
    <TD class="control"><INPUT type="text" name="acompte" value="<%=viewBean.getAcompteFormate()%>" class="montant">
    </TD>
    <TD class="label">1er Acompte</TD>
    <TD class="control"><INPUT type="text" name="premierAcompte" value="<%=viewBean.getPremierAcompteFormate()%>"
                               class="montant"></TD>
</TR>
<TR>
    <TD class="label">Montant maximum à couvrir</TD>
    <TD class="control"><INPUT type="text" name="plafond" value="<%=viewBean.getPlafondFormate()%>" class="montant">
    </TD>
    <TD class="label">Date de fin</TD>
    <TD class="control"><ct:FWCalendarTag name="dateMax" doClientValidation="CALENDAR"
                                          value="<%=viewBean.getDateMax()%>"/></TD>
</TR>
<%-- /tpl:put --%>
