...
<script language="JavaScript">
    ...
    adressePmtArray[<%=i%>][4] = "<%=adpmt.isEligibleQrReference()%>";
    ...
    function changeMbrFamille(selectObj) {
        ...
        if (idx == 0) {
            <%
                ...
                if (Objects.nonNull(viewBean.getAdressePaiementData())) {
                    elm.setEligibleQrReference(TIAdressePaiement.isQRIban(viewBean.getAdressePaiementData().getCompte()));
                }
...
<span class="postItIcon">
					<ct:FWNote sourceId="<%=viewBean.getIdTiersBeneficiaire()%>"
                               tableSource="<%=globaz.corvus.application.REApplication.KEY_POSTIT_RENTES%>"/>
				</span>
<% } %>
...
<tr>
    <TD colspan="6">
        <ct:ifhasright element="osiris.comptes.ordresVersement" crud="r">
            <A href="<%=request.getContextPath()%>/osiris?userAction=osiris.comptes.apercuParSection.chercher&selectedId=<%=viewBean.getIdCompteAnnexeIC()%>&id=<%=viewBean.getIdCompteAnnexeIC()%>&idCompteAnnexe=<%=viewBean.getIdCompteAnnexeIC()%>"
               class="external_link">
        ...
        <TABLE>
            ...
            <TR>
                ...
                <TD colspan="2" rowspan="2">
                    <PRE><span class="IJAfficheText"
                               id="ccpOuBanqueF"><%=viewBean.getCcpOuBanqueFormatte()%></span></PRE>
                    <PRE><span class="IJAfficheText" id="adresseF"><%=viewBean.getAdresseFormattee()%></span></PRE>
                    <PRE><span class="IJAfficheText"
                               id="referenceQRF"><%=viewBean.getReferenceQRFormattee()%></span></PRE>
            ...
            <TR>
                ...
                <TD><INPUT type="hidden" name="noDemandeRente" value="<%=noDemandeRente%>">
                    <INPUT type="text" name="dummy" value="<%=viewBean.getIdPrestationAccordee()%>" class="disabled"
                           readonly>
                </TD>
                <TD>&nbsp;</TD>
                <TD>
            ...
            <TR>
                ...
                <TD colspan="1">
                    <select name="selectMbrFam" onchange="changeMbrFamille(this);">
                        ...
                        %>
                        <option value="<%=admpt.getIdTiers()%>"><%=admpt.getNom() + " " + admpt.getPrenom()%>
                        </option>
                        <% }
                        }
                        %>
            ...
            <TR>
                ...
                <TD><INPUT type="text" name="dateDecision" value="<%=viewBean.getDateDecision()%>" class="disabled"
                           readonly></TD>
                <TD>&nbsp;</TD>
                <TD colspan="3">
                    <ct:FWLabel key="JSP_RAC_R_REF_PAIEMENT"/><br>
                    <textarea name="referencePmt" cols="50" rows="2"
                              onKeyDown="limiteur();"><%=viewBean.getReferencePmt()%></textarea>
                </TD>
            </TR>
            <TR>
            ...
            <TR>
                ...
                <TD>
                    <INPUT type="text" id="codePrestation" name="codePrestation" size="8"
                           value="<%=viewBean.getCodePrestation()%>" onkeyup="metAJourCodePrestation()">
                    <IMG src="" alt="" name="imageOKKOCodePrestation">
                </TD>
                <TD><ct:FWLabel key="JSP_RAC_D_CODE_AUXILIAIRE"/></TD>
                <TD>
                    <INPUT type="text" size="1" id="codeAuxilliaire" name="codeAuxilliaire"
                           value="<%=viewBean.getCodeAuxilliaireJSP()%>">
                </TD>
                <TD><ct:FWLabel key="JSP_RAC_D_FRACTION_DE_RENTE"/></TD>
                <TD>
                    <INPUT type="text" size="1" id="fractionRente" name="fractionRente"
                           value="<%=viewBean.getFractionRenteJSP()%>">
                </TD>
            </TR>
            <TR>
                <TD><ct:FWLabel key="JSP_RAC_D_ETAT_CIVIL"/></TD>
                <TD>
                    <ct:select id="csEtatCivil" name="csEtatCivil" defaultValue="<%=viewBean.getCsEtatCivil()%>"
                               disabled="true" wantBlank="true">
                ...
                <TD>
                    <INPUT type="text" size="5" maxlength="6" id="quotiteRente" name="quotiteRente"
            ...
            <TR>
                ...
                <TD><INPUT type="text" name="montantPrestation" value="<%=viewBean.getMontantPrestation()%>"
                           class="montant"></TD>
                <TD><ct:FWLabel key="JSP_RAC_D_MONTANT_RENTE_ORDI_REMPLACEE"/></TD>
                <TD><INPUT type="text" name="montantRenteOrdiRemplacee"
                           value="<%=viewBean.getMontantRenteOrdiRemplaceeJSP()%>" class="montant"></TD>
                <TD colspan="2">&nbsp;</TD>
            </TR>
            <TR>
                <TD><ct:FWLabel key="JSP_RAC_D_CAS_SPECIAUX"/></TD>
                <TD colspan="3">
                    <INPUT type="text" name="codeCasSpeciaux1" size="2" maxlength="2"
                           value="<%=viewBean.getCodeCasSpeciaux1()%>" onchange="metAJourCodeCasSpeciaux()">
                    <INPUT type="text" name="codeCasSpeciaux2" size="2" maxlength="2"
                           value="<%=viewBean.getCodeCasSpeciaux2()%>" onchange="metAJourCodeCasSpeciaux()">
                    <INPUT type="text" name="codeCasSpeciaux3" size="2" maxlength="2"
                           value="<%=viewBean.getCodeCasSpeciaux3()%>" onchange="metAJourCodeCasSpeciaux()">
                    <INPUT type="text" name="codeCasSpeciaux4" size="2" maxlength="2"
                           value="<%=viewBean.getCodeCasSpeciaux4()%>" onchange="metAJourCodeCasSpeciaux()">
                    <INPUT type="text" name="codeCasSpeciaux5" size="2" maxlength="2"
                           value="<%=viewBean.getCodeCasSpeciaux5()%>" onchange="metAJourCodeCasSpeciaux()">
                    <IMG src="" alt="" name="imageOKKOCodeCasSpeciaux">
                </TD>
                <TD><ct:FWLabel key="JSP_RAC_D_REDUCTION_FAUTE_GRAVE"/></TD>
                <TD><INPUT type="text" name="reductionFauteGrave" value="<%=viewBean.getReductionFauteGraveJSP()%>"
                           class="montant"></TD>
            </TR>
            <TR>
            ...
            <TR>
                ...
                <TD><INPUT type="text" name="supplementAjournement" value="<%=viewBean.getSupplementAjournementJSP()%>"
                           class="montant"></TD>
                <TD><ct:FWLabel key="JSP_RAC_D_DATE_REVOCATION_AJOURNEMENT"/></TD>
                <TD><INPUT type="text" name="dateRevocationAjournement"
                           value="<%=viewBean.getDateRevocationAjournementJSP()%>"></TD>
            </TR>
            <TR>
                <TD><ct:FWLabel key="JSP_RAC_D_ANNEE_ANTICIPATION"/></TD>
                <TD>
                    <INPUT type="text" size="3" name="anneeAnticipation"
                           value="<%=viewBean.getAnneeAnticipationJSP()%>">
                </TD>
                <TD><ct:FWLabel key="JSP_RAC_D_MONTANT_REDUCTION_ANTICIPATION"/></TD>
                <TD><INPUT type="text" name="montantReducationAnticipation"
                           value="<%=viewBean.getMontantReducationAnticipationJSP()%>" class="montant"></TD>
                <TD><ct:FWLabel key="JSP_RAC_D_DATE_DEBUT_ANTICIPATION"/></TD>
                <TD><INPUT type="text" name="DateDebutAnticipation" value="<%=viewBean.getDateDebutAnticipationJSP()%>">
                </TD>
            </TR>
            ...
            <TR>
                ...
                <TD>
                    <INPUT type="text" name="tauxReductionAnticipation"
            ...
            <TR>
                ...
                <TD>
                    <INPUT type="text" size="1" name="prescriptionAppliquee"
                           value="<%=viewBean.getPrescriptionAppliqueeJSP()%>">
                </TD>
            ...
            <TR>
                ...
                <TD>
                    <INPUT type="checkbox" name="isPrestationBloquee"
                           value="on" <%=viewBean.getIsPrestationBloquee().booleanValue()?"CHECKED":""%>
                           class="forceDisable" disabled="disabled">
                </TD>
                <TD><ct:FWLabel key="JSP_RAC_L_DATE_ECHEANCE"/></TD>
                <TD>
                    <input type="hidden" value="<%=viewBean.getDateNaissanceTiersBeneficiaire()%>"
...
<ct:ifhasright element="<%=IREActions.ACTION_RENTE_ACCORDEE_JOINT_DEMANDE_RENTE%>" crud="r">
    <INPUT name="boutonDeblocageMontant" type="button"
           value="<%=viewBean.getSession().getLabel("JSP_RAC_D_DEBLOQUER_MNT")%>" onclick="actionDeblocageMontant()">
</ct:ifhasright>
<%} else if (viewBean.isDesactiverBlocageOnly()) {%>
<ct:ifhasright element="<%=IREActions.ACTION_RENTE_ACCORDEE_JOINT_DEMANDE_RENTE%>" crud="u">
    <INPUT name="boutonDeblocageMontant" type="button"
           value="<%=viewBean.getSession().getLabel("JSP_RAC_D_DEBLOQUER")%>" onclick="actionDesactiverBloquage()">
</ct:ifhasright>
<%} %>
<%}%>
<%-- /tpl:put --%>
