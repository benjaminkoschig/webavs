...
<TR>
    ...
    <td colspan="2"> <PRE><span class="IJAfficheText" id="referenceQRF"><%=viewBean.getReferenceQRFormattee()%></span></PRE></td>
    <TD colspan="1">
        <select name="selectMbrFam" onchange="changeMbrFamille(this);">
            ...
            %>
            <option value="<%=admpt.getIdTiers()%>-<%=admpt.getCsDomaine()%>"><%=admpt.getNom() + " " + admpt.getPrenom()%>
            </option>
            <% }
            }
            %>
...
<tr>
    ...
    <TD>
        ...
        <input type="text" name="dateDebutPmt1erAcompte" value="<%=viewBean.getDateDebutPmt1erAcompte()%>"
    ...
    <TD>
        <INPUT type="text" class="disabled" name="dummy2" readonly="readonly"
               value="<%=viewBean.getDatePmt1erAcompte()%>">
    </TD>
</tr>
<TR>
    ...
    <%if (IREAvances.CS_ETAT_1ER_ACOMPTE_TERMINE.equals(viewBean.getCsEtat1erAcompte()) || IREAvances.CS_ETAT_1ER_ACOMPTE_ANNULE.equals(viewBean.getCsEtat1erAcompte())) {%>
    <TD><INPUT type="text" style="text-align:right" name="montant1erAcompte"
               value="<%=new globaz.framework.util.FWCurrency(viewBean.getMontant1erAcompte()).toStringFormat()%>"
               readonly class="disabled"></TD>
    <%} else { %>
    <TD><INPUT type="text" style="text-align:right" name="montant1erAcompte"
               value="<%=new globaz.framework.util.FWCurrency(viewBean.getMontant1erAcompte()).toStringFormat()%>"></TD>
    <%} %>
...
<TR>
    ...
    <TD>
        ...
        <%} else { %>
        <input type="text" name="dateDebutAcompte" value="<%=viewBean.getDateDebutAcompte()%>" readonly
               class="disabled"/>
        <%} %>
...
<TR>
    ...
    <%
        if (IREAvances.CS_ETAT_ACOMPTE_ATTENTE.equals(viewBean.getCsEtatAcomptes()) ||
                JadeStringUtil.isBlankOrZero(viewBean.getCsEtatAcomptes())) {
    %>
    <TD><INPUT type="text" style="text-align:right" name="montantMensuel"
               value="<%=new globaz.framework.util.FWCurrency(viewBean.getMontantMensuel()).toStringFormat()%>"></TD>
    <%} else { %>
    <TD><INPUT type="text" style="text-align:right" name="montantMensuel"
               value="<%=new globaz.framework.util.FWCurrency(viewBean.getMontantMensuel()).toStringFormat()%>" readonly
               class="disabled"></TD>
    <%} %>
