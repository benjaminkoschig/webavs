...
<%@page
	import="globaz.pegasus.vb.fortuneusuelle.PCBienImmobilierNonHabitableViewBean"%>
<%@page import="java.util.Iterator"%>
<%@page
	import="ch.globaz.pegasus.business.models.home.SimplePeriodeServiceEtat"%>
<%@page import="globaz.framework.util.FWCurrency"%>
<%
	//Les labels de cette page commencent par le préfix "JSP_PC_BIEN_IMMOBILIER_NON_HABITABLE"
	idEcran = "PPC0103";

	PCBienImmobilierNonHabitableViewBean viewBean = (PCBienImmobilierNonHabitableViewBean) session
...
<%@page
	import="globaz.pegasus.vb.fortuneusuelle.PCBienImmobilierNonHabitableAjaxViewBean"%>
...
<%@page import="globaz.pegasus.utils.PCCommonHandler"%><script language="JavaScript">
	...
	var ACTION_AJAX_BIEN_IMMOBILIER_NON_HABITABLE="<%=IPCActions.ACTION_DROIT_BIEN_IMMOBILIER_NH_AJAX%>";
	var CS_TYPE_AUTRE = <%=IPCDroits.CS_AUTRE_TYPE_BIEN%>;
	var LANGUAGES = "<%= objSession.getLabel("JSP_PC_BIEN_IMMOBILIER_NON_HABITABLE_MULTIWIDGETS")%>";
...
<script type="text/javascript" src="<%=servletContext%><%=(mainServletPath + "Root")%>/scripts/droit/BienImmobilierNonHabitable_MembrePart.js" /></script>
<script type="text/javascript" src="<%=servletContext%><%=(mainServletPath + "Root")%>/scripts/droit/BienImmobilierNonHabitable_de.js" /></script>
<style>
.noHypotheque{
	width: 210px;
}
...
<TR>
	...
	<table class="areaDFDataTable">
		<thead>
			<tr>
				...
				<th><ct:FWLabel key="JSP_PC_BIEN_IMMOBILIER_NON_HABITABLE_L_PROPRIETE" /></th>
				<th><ct:FWLabel key="JSP_PC_BIEN_IMMOBILIER_NON_HABITABLE_L_PART" /></th>
				<th><ct:FWLabel key="JSP_PC_BIEN_IMMOBILIER_NON_HABITABLE_L_BIEN" /></th>
				<th><ct:FWLabel key="JSP_PC_BIEN_IMMOBILIER_NON_HABITABLE_L_VALEUR_VENALE" /></th>					
				<th><ct:FWLabel key="JSP_PC_BIEN_IMMOBILIER_NON_HABITABLE_L_DETTE" /></th>
				<th><ct:FWLabel key="JSP_PC_BIEN_IMMOBILIER_NON_HABITABLE_L_INTERETS" /></th>
				<th><ct:FWLabel key="JSP_PC_BIEN_IMMOBILIER_NON_HABITABLE_L_MONTANT_RENDEMENT" /></th>
				<th data-g-cellformatter="css:formatCellIcon"><ct:FWLabel key="JSP_PC_BIEN_IMMOBILIER_NON_HABITABLE_L_DF" /></th>
				<th data-g-cellformatter="css:formatCellIcon" ><ct:FWLabel key="JSP_PC_BIEN_IMMOBILIER_NON_HABITABLE_L_DR" /></th>
				<th data-g-periodformatter=" " data-g-deallaterperiod=" "><ct:FWLabel key="JSP_PC_BIEN_IMMOBILIER_NON_HABITABLE_L_PERIODE" /></th>
			</tr>
		</thead>
		<tbody>
			<%								
					...
					for(Iterator itDonnee=viewBean.getDonnees(membreFamille.getId()).iterator();itDonnee.hasNext();){																	
						...
						BienImmobilierNonHabitable donneeALDComplexe=donneeComplexe.getBienImmobilierNonHabitable();			 
						SimpleBienImmobilierNonHabitable donnee = (SimpleBienImmobilierNonHabitable)donneeALDComplexe.getSimpleBienImmobilierNonHabitable();
						...
						if(!donneeALDComplexe.getSimpleBienImmobilierNonHabitable().getIdBienImmobilierNonHabitable().equals(currentId)){
							currentId = donneeALDComplexe.getSimpleBienImmobilierNonHabitable().getIdBienImmobilierNonHabitable();												
			...
			%>		
							<tr idEntity="<%=donnee.getId() %>" idGroup="<%=dfHeader.getIdEntityGroup() %>" header="<%=idGroup==null?"true":"false"%>">
								...
								<td title="<%=donnee.getAutresTypeBien() %>"><%=objSession.getCodeLibelle(donnee.getCsTypeBien()) %></td>
								...
								<td  style="text-align:right;"><%=new FWCurrency(donnee.getMontantRendement()).toStringFormat() %></td>					
	...
	<div class="areaDFDetail">
		<table>
		<tr>
			<td><ct:FWLabel key="JSP_PC_BIEN_IMMOBILIER_NON_HABITABLE_D_PROPRIETE"/></td>
			<td><ct:select styleClass="typePropriete"  name="champTypeDePropriete" notation="data-g-select='mandatory:true'">
			 ...
			 <td><ct:FWLabel key="JSP_PC_BIEN_IMMOBILIER_NON_HABITABLE_D_PART"/></td>
		...
		<tr>
			<td><ct:FWLabel key="JSP_PC_BIEN_IMMOBILIER_NON_HABITABLE_D_BIEN"/></td>
			<td><ct:select styleClass="typeBien"  name="typeBien" notation="data-g-select='mandatory:true'">
					<ct:optionsCodesSystems csFamille="PCTYPBNHA"/>					
				</ct:select>
			 </td>	
			<td class="autreToHiden"><ct:FWLabel key="JSP_PC_BIEN_IMMOBILIER_NON_HABITABLE_D_AUTRE"/></td>
			<td class="autreToHiden">
			  <input type="text" 
			         class="autres"
			         data-g-commutator="context:$(this).parents('.areaDFDetail'),
				   					    master:context.find('.typeBien'),
				   					    condition:context.find('.typeBien').val()==CS_TYPE_AUTRE,
				   					    actionTrue:¦show(context.find('.autreToHiden')),mandatory()¦,
				   					    actionFalse:¦hide(context.find('.autreToHiden')),notMandatory()¦"/>
			  </td>					 						
		</tr>
		
		<tr>
			<td><ct:FWLabel key="JSP_PC_BIEN_IMMOBILIER_NON_HABITABLE_D_RENDEMENTS"/></td>
			<td><input type="text" class="rendements" data-g-amount="mandatory:true, periodicity:Y"/></td>		
			
			<td><ct:FWLabel key="JSP_PC_BIEN_IMMOBILIER_NON_HABITABLE_D_VALEUR_VENALE"/></td>
			<td><input type="text" class="valeurVenale" data-g-amount="mandatory:true, periodicity:Y"/></td>	
																									
		</tr>
		
		<tr class='widgetPays'>
		  <td><ct:FWLabel key="JSP_PC_BIEN_IMMOBILIER_NON_HABITABLE_D_PAYS"/></td>
				...
				<ct:widget id='<%="paysWidget"+membreFamille.getId()%>' name='<%="paysWidget"+membreFamille.getId()%>' styleClass="widgetPays">
					<ct:widgetService methodName="findPays" className="<%=AdresseService.class.getName()%>" defaultSearchSize="">
						...
						if("fr".equals(objSession.getIdLangueISO())){
							%>
								<ct:widgetCriteria criteria="forLibelleFrUpperLike" label="JSP_PC_BIEN_IMMOBILIER_NON_HABITABLE_D_LIBELLE_PAYS"/>
								<ct:widgetCriteria criteria="forCodeIso" label="JSP_PC_BIEN_IMMOBILIER_NON_HABITABLE_D_CODE_ISO_PAYS"/>																
							...
							%>
								<ct:widgetCriteria criteria="forLibelleAlUpperLike" label="JSP_PC_BIEN_IMMOBILIER_NON_HABITABLE_D_LIBELLE_PAYS"/>
								<ct:widgetCriteria criteria="forCodeIso" label="JSP_PC_BIEN_IMMOBILIER_NON_HABITABLE_D_CODE_ISO_PAYS"/>																
							...
							%>
								<ct:widgetCriteria criteria="forLibelleItUpperLike" label="JSP_PC_BIEN_IMMOBILIER_NON_HABITABLE_D_LIBELLE_PAYS"/>
								<ct:widgetCriteria criteria="forCodeIso" label="JSP_PC_BIEN_IMMOBILIER_NON_HABITABLE_D_CODE_ISO_PAYS"/>																
		...
		<tr>
				<td><ct:FWLabel key="JSP_PC_BIEN_IMMOBILIER_NON_HABITABLE_D_COMMUNE"/></td>
			<td>
			...
			<td><ct:FWLabel key="JSP_PC_BIEN_IMMOBILIER_NON_HABITABLE_D_FEUILLET"/></td>
			<td><input type="text" class="numeroFeuillet" /></td>
		</tr>
		<tr>		
			<td><ct:FWLabel key="JSP_PC_BIEN_IMMOBILIER_NON_HABITABLE_D_DETTE"/></td>
			<td><input type="text" class="dette" data-g-amount="periodicity:Y" /></td>
			<td><ct:FWLabel key="JSP_PC_BIEN_IMMOBILIER_NON_HABITABLE_D_INTERETS"/></td>
			<td><input type="text" class="interets" data-g-amount="periodicity:Y" /></td>																														
		</tr>
		<tr>
			<td><ct:FWLabel key="JSP_PC_BIEN_IMMOBILIER_NON_HABITABLE_D_NUMERO"/></td>
			<td><input type="text" class="noHypotheque" /></td>
			<td><ct:FWLabel key="JSP_PC_BIEN_IMMOBILIER_NON_HABITABLE_D_COMPAGNIE"/></td>
			<td colspan="3">	
		...
		<tr>
			<td><ct:FWLabel key="JSP_PC_BIEN_IMMOBILIER_NON_HABITABLE_D_DESSAISISSEMENT_FORTUNE"/></td>
			<td><input type="checkbox" class="dessaisissementFortune" /></td>
			<td><ct:FWLabel key="JSP_PC_BIEN_IMMOBILIER_NON_HABITABLE_D_DESSAISISSEMENT_REVENUS"/></td>
			<td><input type="checkbox" class="dessaisissementRevenus" /></td>					
		</tr>		
		<tr>
			<td><ct:FWLabel key="JSP_PC_BIEN_IMMOBILIER_NON_HABITABLE_D_DATE_DEBUT"/></td>
			<td><input type="text" name="dateDebut" value="" data-g-calendar="mandatory:true,type:month"/></td>
			<td><ct:FWLabel key="JSP_PC_BIEN_IMMOBILIER_NON_HABITABLE_D_DATE_FIN"/></td>
			<td><input type="text" name="dateFin" value=""  data-g-calendar="type:month"/></td>
		</tr>
	</table>	
	<ct:ifhasright element="<%=IPCActions.ACTION_DROIT_BIEN_IMMOBILIER_NH_AJAX%>" crud="cud">
