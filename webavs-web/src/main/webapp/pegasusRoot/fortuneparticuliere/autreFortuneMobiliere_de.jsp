...
<%@page import="globaz.pegasus.vb.fortuneparticuliere.PCAutreFortuneMobiliereViewBean"%>
...
<%
	//Les labels de cette page commencent par le préfix "JSP_PC_AUTRE_FORTUNE_MOBILIERE_D"
	idEcran="PPC0023";

	PCAutreFortuneMobiliereViewBean viewBean = (PCAutreFortuneMobiliereViewBean) session.getAttribute("viewBean");
...
<%@page import="ch.globaz.pegasus.business.constantes.donneesfinancieres.IPCAutreFortuneMobiliere"%><link rel="stylesheet" type="text/css" href="<%=servletContext%><%=(mainServletPath+"Root")%>/css/droit/fortuneParticuliere_de.css"/>
...
<script language="JavaScript">
	...
	var ACTION_AJAX_DONNEE_FINANCIERE="<%=IPCActions.ACTION_DROIT_AUTRE_FORTUNE_MOBILIERE_AJAX%>";
	var CS_TYPE_AUTRE ="<%=IPCDroits.CS_TYPE_AUTRE%>";
	var CS_TYPE_FORTUNE_AUTRE ="<%=IPCAutreFortuneMobiliere.CS_TYPE_FORTUNE_AUTRES%>";
...
<script type="text/javascript" src="<%=servletContext%><%=(mainServletPath+"Root")%>/scripts/droit/AutreFortuneMobiliere_MembrePart.js"/></script>
<script type="text/javascript" src="<%=servletContext%><%=(mainServletPath+"Root")%>/scripts/droit/AutreFortuneMobiliere_de.js"/></script>
						...
						<div class="conteneurMembres">
								...
								<table class="areaDFDataTable">
									<thead>
										<tr>
											...
											<th><ct:FWLabel key="JSP_PC_AUTRE_FORTUNE_MOBILIERE_L_TYPE_PROPRIETE"/></th>
											<th><ct:FWLabel key="JSP_PC_AUTRE_FORTUNE_MOBILIERE_L_PART"/></th>
											<th><ct:FWLabel key="JSP_PC_AUTRE_FORTUNE_MOBILIERE_L_MONTANT"/></th>
											<th><ct:FWLabel key="JSP_PC_AUTRE_FORTUNE_MOBILIERE_L_TYPE_FORTUNE"/></th>
											<th data-g-cellformatter="css:formatCellIcon"><ct:FWLabel key="JSP_PC_AUTRE_FORTUNE_MOBILIERE_L_DESSAISISSEMENT_FORTUNE"/></th>
											<th data-g-periodformatter=" " data-g-deallaterperiod=" "><ct:FWLabel key="JSP_PC_AUTRE_FORTUNE_MOBILIERE_L_PERIODE"/></th>
										</tr>
									</thead>
									<tbody>
							<%
									...
									for(Iterator itDonnee=viewBean.getDonnees(membreFamille.getId()).iterator();itDonnee.hasNext();){
										...
										SimpleAutreFortuneMobiliere donnee=(SimpleAutreFortuneMobiliere)donneeComplexe.getDonneeFinanciere();
							...
							%>
										<tr idEntity="<%=donnee.getId() %>" idGroup="<%=dfHeader.getIdEntityGroup() %>" header="<%=idGroup==null?"true":"false"%>">
											...
											<td><%=objSession.getCodeLibelle(donnee.getCsTypeFortune()) %></td>
							...
							%>
								...
								<div class="areaDFDetail">
									<table>
										<tr>
											<td><ct:FWLabel key="JSP_PC_AUTRE_FORTUNE_MOBILIERE_D_TYPE_PROPRIETE"/></td>
											...
											<td><ct:FWLabel key="JSP_PC_AUTRE_FORTUNE_MOBILIERE_D_PART"/></td>
											<td><input type="text" class="part" value="1/1" data-g-string="mandatory:true"/></td>
										</tr>
										<tr>
											<td><ct:FWLabel key="JSP_PC_AUTRE_FORTUNE_MOBILIERE_D_MONTANT"/></td>
											<td><input type="text" class="montant" data-g-amount="mandatory:true, periodicity:Y"></td>
											<td><ct:FWLabel key="JSP_PC_AUTRE_FORTUNE_MOBILIERE_D_TYPE_FORTUNE"/></td>
											<td><ct:select styleClass="csTypeFortune"  name="champTypeDeFortune" notation="data-g-select='mandatory:true'">
													<ct:optionsCodesSystems csFamille="PCTYFOR"/>
												</ct:select>
											</td>
											<td class="autreToHiden"><ct:FWLabel key="JSP_PC_AUTRE_FORTUNE_MOBILIERE_D_AUTRES"/></td>
											<td class="autreToHiden"><input type="text" class="autres" data-g-string="mandatory:false" 										 	    
												data-g-commutator="context:$(this).parents('.areaDFDetail'),
											 					   master:context.find('[name=champTypeDeFortune]'),
											 		               condition:context.find('[name=champTypeDeFortune]').val()==CS_TYPE_FORTUNE_AUTRE,
											 		               actionTrue:¦mandatory(),show(context.find('.autreToHiden'))¦,
											 		               actionFalse:¦notMandatory(),hide(context.find('.autreToHiden'))¦"/></td>									</tr>
										<tr>
											<td><ct:FWLabel key="JSP_PC_AUTRE_FORTUNE_MOBILIERE_D_DESSAISISSEMENT_FORTUNE"/></td>
											<td><input type="checkbox" class="dessaisissementFortune" /></td>
										</tr>
										<tr>
											<td><ct:FWLabel key="JSP_PC_AUTRE_FORTUNE_MOBILIERE_D_DATE_DEBUT"/></td>
											<td><input type="text" name="dateDebut" value="" data-g-calendar="mandatory:true,type:month"/></td>
											<td><ct:FWLabel key="JSP_PC_AUTRE_FORTUNE_MOBILIERE_D_DATE_FIN"/></td>
											<td><input type="text" name="dateFin" value="" data-g-calendar="type:month"/></td>
										</tr>
									</table>
									<ct:ifhasright element="<%=IPCActions.ACTION_DROIT_AUTRE_FORTUNE_MOBILIERE_AJAX%>" crud="cud">
