...
<%@page import="globaz.pegasus.vb.fortuneparticuliere.PCPretEnversTiersViewBean"%>
...
<%
	...
	PCPretEnversTiersViewBean viewBean = (PCPretEnversTiersViewBean) session.getAttribute("viewBean");
...
<script language="JavaScript">
	...
	var ACTION_AJAX_PRET_ENVERS_TIERS="<%=IPCActions.ACTION_DROIT_PRET_ENVERS_TIERS_AJAX%>";
...
<script type="text/javascript" src="<%=servletContext%><%=(mainServletPath+"Root")%>/scripts/droit/PretEnversTiers_MembrePart.js"/></script>
<script type="text/javascript" src="<%=servletContext%><%=(mainServletPath+"Root")%>/scripts/droit/PretEnversTiers_de.js"/></script>
						...
						<div class="conteneurMembres">
								...
								<table class="areaDFDataTable">
									<thead>
										<tr>
											...
											<th><ct:FWLabel key="JSP_PC_PRET_TIERS_L_TYPE_PROPRIETE"/></th>
											<th><ct:FWLabel key="JSP_PC_PRET_TIERS_L_PART"/></th>
											<th><ct:FWLabel key="JSP_PC_PRET_TIERS_L_TYPE_PRET"/></th>
											<th><ct:FWLabel key="JSP_PC_PRET_TIERS_L_MONTANT"/></th>
											<th><ct:FWLabel key="JSP_PC_PRET_TIERS_L_BENEFICIAIRE"/></th>
											<th><ct:FWLabel key="JSP_PC_PRET_TIERS_L_DATE_ECHEANCE"/></th>
											<th data-g-cellformatter="css:formatCellIcon"><ct:FWLabel key="JSP_PC_PRET_TIERS_L_DESSAISISSEMENT_FORTUNE"/></th>
											<th data-g-cellformatter="css:formatCellIcon"><ct:FWLabel key="JSP_PC_PRET_TIERS_L_DESSAISISSEMENT_REVENU"/></th>
											<th data-g-periodformatter=" " data-g-deallaterperiod=" "><ct:FWLabel key="JSP_PC_PRET_TIERS_L_PERIODE"/></th>
										</tr>
									</thead>
									<tbody>
							<%
									...
									for(Iterator itDonnee=viewBean.getDonnees(membreFamille.getId()).iterator();itDonnee.hasNext();){
										...
										SimplePretEnversTiers donnee=(SimplePretEnversTiers)donneeComplexe.getDonneeFinanciere();
							...
							%>
										<tr idEntity="<%=donnee.getId() %>" idGroup="<%=dfHeader.getIdEntityGroup() %>" header="<%=idGroup==null?"true":"false"%>">
											...
											<td><%=donnee.getTypePret() %></td>
											<td><%=new FWCurrency(donnee.getMontantPret()).toStringFormat() %></td>
											<td><%=donnee.getNomPrenomBeneficiaire() %></td>
							...
							%>
								...
								<div class="areaDFDetail">
									<table>
										<tr>
											<td><ct:FWLabel key="JSP_PC_PRET_TIERS_D_TYPE_PROPRIETE"/></td>
											...
											<td><ct:FWLabel key="JSP_PC_PRET_TIERS_D_PART"/></td>
											<td><input type="text" class="part" value="1/1" data-g-string="mandatory:true"/></td>
											<td><ct:FWLabel key="JSP_PC_PRET_TIERS_D_TYPE_PRET"/></td>
											<td><input type="text" class="typePret"/></td>
										</tr>
										<tr>
											<td><ct:FWLabel key="JSP_PC_PRET_TIERS_D_MONTANT"/></td>
											<td><input type="text" class="montant" data-g-amount="mandatory:true,periodicity:Y"></td>
											<td><ct:FWLabel key="JSP_PC_PRET_TIERS_D_BENEFICIAIRE"/></td>
											<td colspan="3"><input type="text" class="beneficiaire" data-g-string="mandatory:true"/></td>
										</tr>
										<tr>
											<td><ct:FWLabel key="JSP_PC_PRET_TIERS_D_DATE_ECHEANCE"/></td>
										...
										<tr>		
											<td><ct:FWLabel key="JSP_PC_PRET_TIERS_D_INTERET"/></td>
											...
											<td class="montantInteretField"><ct:FWLabel key="JSP_PC_PRET_TIERS_D_MONTANT_INTERETS"/></td>
											<td class="montantInteretField"><input type="text" class="montantInteret" data-g-amount="mandatory:true, periodicity:Y"/></td>	
										</tr>									
										<tr>
											<td><ct:FWLabel key="JSP_PC_PRET_TIERS_D_DESSAISISSEMENT_FORTUNE"/></td>
											<td><input type="checkbox" class="dessaisissementFortune" /></td>
											<td><ct:FWLabel key="JSP_PC_PRET_TIERS_D_DESSAISISSEMENT_REVENU"/></td>
									...
									<ct:ifhasright element="<%=IPCActions.ACTION_DROIT_PRET_ENVERS_TIERS_AJAX%>" crud="cud">
