            ...
            $("#dialog-date-reduction").dialog({
                ...
                buttons: {
                    ...
                    "<ct:FWLabel key='PROCESS_ADAPTATION_PC_NON'/>": function () {
                        ...
                        $('#comptabilisationAuto').val("false");
                        var dateTemp = document.getElementById("forDateFin").value;
                        var dateReduc = document.getElementById("dateReduc").value;
                        $('#forDateFin').val(dateReduc);
...
%>
    ...
    <script type="text/javascript"
            src="<%=servletContext%><%=(mainServletPath+"Root")%>/scripts/demande/demandeDetail_de.js"/>
    </script>
    ...
    <ct:menuChange displayId="options" menuId="pegasus-optionsdemandes">
        ...
        <ct:menuSetAllParams key="idTiers"
                             value="<%=viewBean.getDemande().getDossier().getDemandePrestation().getDemandePrestation().getIdTiers()%>"/>
        <ct:menuSetAllParams key="isFratrie"
                             value="<%=viewBean.getDemande().getSimpleDemande().getIsFratrie().booleanValue()?\"1\":\"0\"%>"/>
        <ct:menuSetAllParams key="idGestionnaire"
                             value="<%=viewBean.getDemande().getSimpleDemande().getIdGestionnaire()%>"/>
        <ct:menuSetAllParams key="idDecision"
                             value="<%=viewBean.getDemande().getSimpleDecisionRefus()!=null?viewBean.getDemande().getSimpleDecisionRefus().getIdDecisionRefus():\"0\"%>"/>
...
// attribue une id à tous les champs ayant un nom mais pas encore d'id
        ...
        function validate() {
            ...
            var dateReduc = document.getElementById("dateReduc");
            if (dateReduc != null && dateReduc.value != "") {
                var dateReducValue = dateReduc.value;
                if (dateReducValue != "") {
                    state = false;
                    parts = dateReducValue.split('.');
            ...
            var dateFin =  document.getElementById("forDateFin").value;
            if(dateFin != null && dateFin.value != ""){
                $('#dateFinViewBean').val( dateFin);
            }
    ...
    <span class="postItIcon"
          data-g-note="idExterne:<%=viewBean.getDemande().getSimpleDemande().getIdDemande()%>, tableSource:PCDEMPC"></span>
        ...
        <TD colspan="6" align="center">
            ...
            <table width="90%">
                ...
                <TR>
                    ...
                    <td colspan="5">
                        <%=affichePersonnne %>
                        <a class="lienTiers external_link"
                           href="./pyxis?userAction=pyxis.tiers.tiers.afficher&selectedId=<%=personne.getTiers().getIdTiers()%>">
                ...
                <tr>
                    <td><label class="standardLabel" for="forIdDemande"><ct:FWLabel
                            key="JSP_PC_DEM_R_DEMANDE_PUR_RETROACTIF"/></label></td>
                    ...
                    <td><label class="standardLabel" for="forFocerCalcul"><ct:FWLabel
                            key="JSP_PC_DEM_R_FOCER_CALCUL"/></label></td>
                    <td>
                ...
                <tr id="forDisplayDateReto">
                    <td><label class="standardLabel" for="forDateDebut"><ct:FWLabel
                            key="JSP_PC_DEM_R_DATE_DEBUT"/></label></td>
                    <td><input calss="clear"
                               value="<%=JadeStringUtil.toNotNullString(viewBean.getDemande().getSimpleDemande().getDateDebut())%>"
                               type="text"
                    ...
                    <td><label class="standardLabel " for="forDateFin"><ct:FWLabel key="JSP_PC_DEM_R_DATE_FIN"/></label>
                    </td>
                    <td><input calss="clear"
                               value="<%=JadeStringUtil.toNotNullString(viewBean.getDemande().getSimpleDemande().getDateFin())%>"
                               type="text"
                               data-g-calendar="mandatory:true,type:month"
                ...
                <tr>
                    ...
                    <td colspan="5"><%= viewBean.getPeriode() %>
                    </td>
                </tr>
                <tr>
                    <td class="standardLabel"><ct:FWLabel key="JSP_PC_DOS_D_ETAT_DEMANDE"/></td>
                    <td colspan="5"><%= objSession.getCodeLibelle(viewBean.getDemande().getSimpleDemande().getCsEtatDemande()) %>
                    </td>
                </tr>
                <TR>
                ...
                <tr>
                    ...
                    <td id="dateReducTD">
                        <input type="text" name="dateReduc" id="dateReduc" onchange="doDisableCheckbox(this)"
                               data-g-calendar="mandatory:false,type:month"
                               value="<%=!JadeStringUtil.isBlank(viewBean.getDemande().getSimpleDemande().getDateFinInitial())?viewBean.getDemande().getSimpleDemande().getDateFin():""%>"/>
                    </td>
                </tr>
                <tr id="montantRestitutionTr">
                    <TD><ct:FWLabel key="JSP_PC_DRO_D_MONTANT_RESTITUTION"/></TD>
                    <TD>
                        <input type="text" value=" " name="montantRestitution" id="montantRestitution"
                               readonly="readonly" disabled="disabled" data-g-amount=" "/>
                    </TD>
                </tr>
                <TR>
                    <TD colspan="6">&nbsp;<HR class="separator">
                ...
                <TR>
                    ...
                    <TD>
                        <ct:FWCodeSelectTag codeType="<%=IPCDemandes.CS_TYPE_DEMANDE%>"
                                            name="demande.simpleDemande.typeDemande" wantBlank="true"
                                            defaut="<%=viewBean.getDemande().getSimpleDemande().getTypeDemande()%>"/>
                    </TD>
                    <td class="standardLabel"><ct:FWLabel key="JSP_PC_DEM_D_ISENFANT"/></td>
                    <td>
                        <input type="checkbox"
                               name="demande.simpleDemande.isFratrie" <%=viewBean.isFratrie() ? "checked" : "" %>
                               value="on"/>
                    </td>
                    <TD colspan="2">&nbsp;</TD>
                </TR>
                <TR>
                    ...
                    <TD>
                        <input type="text" name="dateDepot"
                               value="<%=JadeStringUtil.toNotNullString(viewBean.getDemande().getSimpleDemande().getDateDepot())%>"
                               data-g-calendar="mandatory:true"/>
                    </TD>
                    <TD class="standardLabel"><ct:FWLabel key="JSP_PC_DEM_D_DATE_ARRIVEE_CC"/></TD>
                    <TD>
                        <input type="text" name="dateArrivee"
                               value="<%=JadeStringUtil.toNotNullString(viewBean.getDemande().getSimpleDemande().getDateArrivee())%>"
                ...
                <TR>
                    ...
                    <TD>
                        <input type="text" name="demande.simpleDemande.dateProchaineRevision"
                               value="<%=JadeStringUtil.toNotNullString(viewBean.getDemande().getSimpleDemande().getDateProchaineRevision())%>"
                               data-g-calendar="mandatory:false, type:month"/>
                    </TD>
                ...
                <tr>
                    ...
                    <td colspan="5"><textarea rows="5" cols="100" name="demande.simpleDemande.motifProchaineRevision"
                                              value="<%=JadeStringUtil.toNotNullString(viewBean.getDemande().getSimpleDemande().getMotifProchaineRevision())%>"><%=JadeStringUtil.toNotNullString(viewBean.getDemande().getSimpleDemande().getMotifProchaineRevision())%></textarea>
                    </td>
                </tr>
                ...
                <TR>
                    ...
                    <TD>
                        <%if (viewBeanIsNew) { %>
                        <ct:FWListSelectTag name="demande.simpleDemande.idGestionnaire"
                                            data="<%=PCGestionnaireHelper.getResponsableData(objSession)%>"
                                            defaut="<%=objSession.getUserId()%>"/>
                        <%} else {%>
                        <ct:FWListSelectTag name="demande.simpleDemande.idGestionnaire"
                                            data="<%=PCGestionnaireHelper.getResponsableData(objSession)%>"
                                            defaut="<%=viewBean.getDemande().getSimpleDemande().getIdGestionnaire()%>"/>
                        <%}%>
                    </TD>
                </TR>
            </TABLE>
            ...
            <div id="dialog-confirm-annulation"
                 title="<%= objSession.getLabel("JSP_PC_DECALCUL_D_CONFIRMATION_COMPTA_AUTO_TITRE")%>">
                <p><span class="ui-icon ui-icon-alert"
                         style="float: left; margin: 0 7px 20px 0;"></span><%= objSession.getLabel("JSP_PC_DEM_D_FORCER_CONFIRMATION_COMPTA_AUTO")%>
                </p>
            </div>
            <div id="dialog-error-date-reduction" title="<%= objSession.getLabel("JSP_PC_DEM_D_DATE_REDUCTION_FIN")%>">
                <p><span class="ui-icon ui-icon-alert"
                         style="float: left; margin: 0 7px 20px 0;"></span><%=objSession.getLabel("JSP_PC_DEM_D_DATE_REDUCTION_ERROR")%>
                </p>
            </div>
            <div id="dialog-date-reduction" title="<%= objSession.getLabel("JSP_PC_DEM_D_DATE_REDUCTION_FIN")%>">
                <p><span class="ui-icon ui-icon-alert"
                         style="float: left; margin: 0 7px 20px 0;"></span><%= objSession.getLabel("JSP_PC_DEM_D_DATE_REDUCTION")%>
                </p>
            </div>
            <div id="dialog-error-date-reduction-debut"
                 title="<%= objSession.getLabel("JSP_PC_DEM_D_DATE_REDUCTION_FIN")%>">
                <p><span class="ui-icon ui-icon-alert"
                         style="float: left; margin: 0 7px 20px 0;"></span><%= objSession.getLabel("JSP_PC_DEM_D_DATE_REDUCTION_ERROR_DATE_DEBUT")%>
                </p>
            </div>
