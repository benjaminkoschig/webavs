...
<%@page import="globaz.phenix.db.communications.CPCommunicationFiscaleRetourSEDEXViewBean"%>
...
bButtonDelete = false;
idEcran="CCP1017";
CPCommunicationFiscaleRetourSEDEXViewBean viewBean = (globaz.phenix.db.communications.CPCommunicationFiscaleRetourSEDEXViewBean)session.getAttribute ("viewBean");
...
<%@ include file="/theme/detail/bodyStart2.jspf" %>
		...
		String infoRevenu = "(revenu AVS sans le revenu agricole)";
		...
		if(CPDecision.CS_NON_ACTIF.equalsIgnoreCase(viewBean.getGenreAffilie()) && CPDecision.CS_NON_ACTIF.equalsIgnoreCase(viewBean.getGenreConjoint())){
			coupleNonActif = true;
			infoRevenu = "(y compris celle du conjoint)";
		}
		%>
		<div id="tabs"> 
		<ul>
		...
		<INPUT type="text" name="numAffilie" tabindex="-1" value="<%=viewBean.getNumAffilie()%>" class="libelleLongDisabled" readonly>
         ...
         <TR>
       	...
       	<TR>
            ...
            <TD nowrap><INPUT type="text" name="fortune" value="<%=viewBean.getFortune()%>" class="montantDisabled" readonly="readonly" style="width : 2.45cm;"> (contribuable + conjoint)</TD>
            <TD></TD>
            <TD nowrap></TD>
       	</TR>
 	    <TR>
        ...
        <TR>
            ...
            <TD nowrap><INPUT type="text" name="dateRetour" value="<%=viewBean.getDateRetour()%>" class="dateDisabled" readonly="readonly" style="width : 2.45cm;"></TD>
        </TR>
        <TR>
            <TD nowrap>&nbsp;</TD>
            <TD> </TD>
        ...
        <TR>
			...
			<% if (!JadeStringUtil.isIntegerEmpty(viewBean.getIdAffiliation())) {%> 
				...
				<A href="<%=request.getContextPath()%>\phenix?userAction=phenix.principale.decision.chercher&selectedId=<%=viewBean.getIdTiers()%>&selectedId2=<%=viewBean.getIdAffiliation()%>" class="external_link">Décisions du contribuable</A>
				</ct:ifhasright>
			<%}%>&nbsp;
			<% if (!JadeStringUtil.isIntegerEmpty(viewBean.getIdAffiliationConjoint())) {%> 
				<ct:ifhasright element="phenix.principale.decision.chercher" crud="r">
				<A href="<%=request.getContextPath()%>\phenix?userAction=phenix.principale.decision.chercher&selectedId=<%=viewBean.getIdConjoint()%>&selectedId2=<%=viewBean.getIdAffiliationConjoint()%>" class="external_link">Décisions du conjoint</A>
				</ct:ifhasright>
			<%}%>
			</TD>
			<TD></TD>
