...
<script type="text/javascript" src="${rootPath}/scripts/monthDateUtil.js"></script>
...
globazGlobal.CS_SANS_TRAVAILLEUR = ${viewBean.csSansTravailleur};
...
jsManager.executeAfter = function () {
	...
	function isSP() {
		...
		return typeDecompte==globazGlobal.CS_SPECIAL;		
	}
	...
	function isST() {
		var typeDecompte = getTypeDecompte();
		return typeDecompte==globazGlobal.CS_SANS_TRAVAILLEUR;
	}
...
<div>
	<table>
		<tr>
			<td width="240px;"><label><ct:FWLabel key='JSP_PROCESS_DECOMPTE_TYPE'/></label></td>
			<td>
				<select id="typeDecompte" name="typeDecompte" />
					...
					<option value="-1"><ct:FWLabel key="JSP_ST_SANS_TRAVAILLEUR"/></option>
				</select>
			</td>
		</tr>
		<tr>
			<td><label><ct:FWLabel key='JSP_PROCESS_DECOMPTE_DATE'/></label></td>
			<td><input name="dateDecompte" data-g-calendar="mandatory:true" value="" /></td>
		</tr>
		<tr class="hideable toHideST">
			<td><label><ct:FWLabel key='JSP_PROCESS_DECOMPTE_EMPLOYEUR_SPECIAL'/></label></td>
			<td>
			<input class="jadeAutocompleteAjax libelleLong" type="text" id="employeurNumeroLibelle" value="${viewBean.designationEmployeur}"
				data-g-autocomplete="
				...
				<input type="hidden" id="employeurNumero" name="idEmployeur" value="${viewBean.idEmployeur}">
				</td>
		</tr>
		<tr>
			<td><label><ct:FWLabel key='JSP_PROCESS_DECOMPTE_PERIODE'/></label></td>
			<td><input name="periodeDebut" id="DATE_PERIODE_DE" data-g-calendar="type:month,mandatory:true" value="" />
				&nbsp;<ct:FWLabel key='JSP_PROCESS_DECOMPTE_A'/>
				&nbsp;<input name="periodeFin" id="DATE_PERIODE_A" data-g-calendar="type:month,mandatory:true" value="" />
			</td>
		</tr>
		<tr id="trNumeroDecompte" style="display:none;">
			<td><label><ct:FWLabel key='JSP_DECOMPTE_NUM'/></label></td>
			<td><input id="numeroDecompte" name="numeroDecompte" data-g-integer="mandatory:true,autoFit:true,sizeMax:10" value="" /></td>
		</tr>
		<tr class="hideable toHideCT toHideSP toHideST">
			<td><label><ct:FWLabel key='JSP_PROCESS_DECOMPTE_AVEC_POSTES_ET_DOCUMENTS'/></label></td>
			<td><input type="checkbox" name="withPostes" id="withPostes" checked="checked"/></td>
		</tr>
