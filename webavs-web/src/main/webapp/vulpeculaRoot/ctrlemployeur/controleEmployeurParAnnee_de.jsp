...
<c:set var="labelTitreEcran" value="JSP_LISTE_CONTROLE_EMPLOYEUR_PAR_ANNEE"/>
...
<c:set var="userActionListe" value="vulpecula.ctrlemployeur" />
...
$(function () {
	$("#launchListe").click(function () {
		...
		if(globazGlobal.checkFields()) {
			document.forms[0].elements('userAction').value="vulpecula.ctrlemployeur.controleEmployeurParAnnee.executer";
...
globazGlobal.checkFields = function () {
	...
	function isAnneeSaisie(mainForm) {
		if (mainForm.annee.length > 0) {
			return true;
		} else {
			showErrorDialog("L'année est obligatoire !");
			return false;
		}
	}
	
	if (isAnneeSaisie(mainForm)) {
		return true;	
	} else {
		return false;
	} 
...
<div style="width: 100%;text-align: center;">
	<div>
		<table>			
			...
			<tr>
				<td><label for="idEmployeur"><ct:FWLabel key='JSP_LISTE_CONTROLE_EMPLOYEUR_PAR_ANNEE_ANNEE'/></label></td>
				<td>
					<input id="annee" name="annee" maxlength="4" type="number" value="${viewBean.annee}" />
				</td>
			</tr>
			<tr>
