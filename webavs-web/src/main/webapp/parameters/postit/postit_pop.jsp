...
String formAction = servletContext + "/vela";
...
function validate() {
	...
	var exit = true;
	var message = "FEHLER: Felden sind nicht alle richtig ergänzt !!";
...
function del() {
	if (window.confirm("Wollen Sie wirklich die PosIt löschen ?"))
	{
...
<BR>
	<FORM name="mainForm" action="<%=formAction%>">
		...
		<BR>
		<BUTTON name="btnClose" id="btnClose" onclick="window.close()">Schliessen</BUTTON>
		<BUTTON name="btnCancel" id="btnCancel" onclick="mainForm.reset()" style="display:none">Annullieren</BUTTON>
		<BUTTON name="btnSave" id="btnSave" onclick="if(validate()){mainForm.submit();}" style="display:none">Speichern</BUTTON>
		<BUTTON name="btnDelete" id="btnDelete" onclick="del()" style="display:none">Löschen</BUTTON>
