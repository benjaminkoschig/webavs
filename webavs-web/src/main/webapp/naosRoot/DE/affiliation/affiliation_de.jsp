...
<SCRIPT language="JavaScript">
    ...
    function del() {
        if (window.confirm("Sie sind dabei, das Detail der ausgewählten Erfassungsperiode zu löschen! Wollen Sie fortfahren?")) {
    ...
    function popupConfirmation() {
        $("#dialog_confirm_debut_affiliation").dialog({
            resizable: false,
            height: 140,
            modal: true,
            buttons: [{
                text: "<ct:FWLabel key='NAOS_DIALOG_LINK_OR_CREATE_ANNONCE_BOUTON_OUI'/>",
                click: function () {
                    $(this).dialog("close");
                    if (proceedValidation()) action(COMMIT);
                }
...
//setCtrl("envoiAutomatiqueLAA",false);
    ...
    optionsList[<%=i%>] = {
        name: "<%=code.getIdCode()%>",
        text: "<%=code.getCurrentCodeUtilisateur().getCodeUtilisateur()+ " - " + code.getCurrentCodeUtilisateur().getLibelle()%>",
        link: "<%= code.getIdSelection()%>"
    };
    <%  } %>
...
<span class="postItIcon"><ct:FWNote sourceId="<%=viewBean.getAffiliationId()%>"
                                    tableSource="globaz.naos.db.affiliation.AFAffiliationViewBean"/></span>
Erfassung - Detail
<%-- /tpl:put --%>
<%@ include file="/theme/detail/bodyStart2.jspf" %>
<%-- tpl:put name="zoneMain" --%>

<div style="display:none" id="dialog_link_or_create_annonce"
     title="<ct:FWLabel key='NAOS_DIALOG_LINK_OR_CREATE_ANNONCE_TITRE'/>">
    <p>
        <ct:FWLabel key="NAOS_DIALOG_LINK_OR_CREATE_ANNONCE_TEXTE"/>
    </p>
</div>
<div style="display:none" id="dialog_confirm_mutation_activite"
     title="<ct:FWLabel key='NAOS_DIALOG_CONFIRM_MUTATION_ACTIVITE_TITRE'/>">
    <p>
        <ct:FWLabel key="NAOS_DIALOG_CONFIRM_MUTATION_ACTIVITE_TEXTE"/>
    </p>
</div>

<div style="display:none" id="dialog_confirm_debut_affiliation"
     title="<ct:FWLabel key='NAOS_DIALOG_CONFIRM_DATE_AFFILIAITON_TITRE'/>">
    <p>
        <%= viewBean.getSession().getLabel("NAOS_DIALOG_CONFIRM_DATE_AFFILIAITON_TEXTE").replace("{0}", viewBean.getDateDebut()) %>
    </p>
</div>

<INPUT type="hidden" id="idAnnonceIdeCreationLiee" name="idAnnonceIdeCreationLiee"
       value="<%=viewBean.getIdAnnonceIdeCreationLiee()%>">
...
<TR>
    <TD>
        <TABLE border="0" cellspacing="0" cellpadding="0">
            <TBODY>
            <TR>
                <TD nowrap width="161"><A
                        href="<%=request.getContextPath()%>\pyxis?userAction=pyxis.tiers.tiers.diriger&selectedId=<%=viewBean.getIdTiers()%>"
                        class="external_link">Partner</A>
                </TD>
                <TD nowrap colspan="3">
                    <INPUT type="hidden" name="selectedId" value="<%=viewBean.getAffiliationId()%>">
                    <% if (viewBean.getTiers().idTiersExterneFormate().length() != 0) { %>
                    <input type="text" name="idExterne" size="16" maxlength="16"
                           value="<%=viewBean.getTiers().idTiersExterneFormate()%>" readOnly tabindex="-1"
                           class="disabled">
                    <% } %>
                    <input type="text" name="nom" size="60" maxlength="60"
                           value="<%=globaz.jade.client.util.JadeStringUtil.change(viewBean.getTiers().getNom(),"\"","&quot;")%>"
                           readOnly tabindex="-1" class="disabled">
                </TD>
            </TR>
            <TR>
            ...
            <TR>
                <TD>Firma (lang)</TD>
                <TD colspan="3"><input type="text" id="raisonSociale" name="raisonSociale" size="100" maxlength="100"
                                       value="<%=globaz.jade.client.util.JadeStringUtil.change(designationLong,"\"","&quot;")%>"
                                       tabindex="-1"></TD>
            </TR>
            ...
            <TR>
                <TD nowrap>Abr.-Nr.</TD>
                <TD nowrap>
                    <INPUT name="affilieNumero" size="20" maxlength="20" type="text"
                           value="<%=viewBean.getAffilieNumero()%>">
                </TD>
                <TD nowrap>Alte Abr.-Nr.</TD>
                <TD nowrap>
                    <INPUT name="ancienAffilieNumero" type="text" size="20" maxlength="20"
                           value="<%=viewBean.getAncienAffilieNumero()%>">
                </TD>
            </TR>
            <TR>
                <TD nowrap height="31" width="161">Periode<%=libDateDebutReelle%>
                </TD>
                <TD nowrap>
                    <ct:FWCalendarTag name="dateDebut" doClientValidation="CALENDAR"
        ...
        <TABLE border="0" cellspacing="0" cellpadding="0" id="tPartie1">
            ...
            <TR>
                ...
                <TD nowrap colspan="2">
                    <input type="text" name="raisonSocialeCourt" size="30" maxlength="30"
                           value="<%=globaz.jade.client.util.JadeStringUtil.change(designationCourt,"\"","&quot;")%>">
                </TD>
            ...
            <TR>
                <TD nowrap height="31" width="161">Erfassungsgrund</TD>
                <TD nowrap colspan="2">
                    <ct:FWCodeSelectTag name="motifCreation" defaut="<%=viewBean.getMotifCreation()%>"
                                        codeType="VEMOTIFAFF"/>
                </TD>
                <TD nowrap>Abgangsgrund</TD>
                <TD nowrap>
                    <ct:FWCodeSelectTag
                            name="motifFin"
            ...
            <TR>
                <TD nowrap height="31" width="161">Rechtsform</TD>
                <TD nowrap colspan="2">
                    <ct:FWCodeSelectTag
                ...
                <TD nowrap>Geschäftspartnertyp</TD>
                <TD nowrap>
                    ...
                    <TABLE border="0" cellspacing="0" cellpadding="0" id="nonTypeAssocieDisplay">
                        ...
                        <TR>
                            <TD>
                                <INPUT type="text" name="nontypeAssocie" size="17" tabindex="-1" class="disabled"
                                       readOnly>
                            </TD>
                        </TR>
                        </TBODY>
            ...
            <TR>
                ...
                <TD nowrap width="100">Lohnbescheinigung</TD>
                <TD nowrap>
                    <ct:FWCodeSelectTag
            ...
            <TR>
                <TD nowrap height="31" width="161">Erwerbszweig</TD>
                <TD nowrap colspan="3">
                    <ct:FWCodeSelectTag
            ...
            <TR>
                ...
                <TD nowrap colspan="2">
                    ...
                    <a href="<%= linkNoga %>" target="new"><ct:FWLabel
                            key="NAOS_JSP_NOGA_LISTE_AFFILIES_CODE_NOGA_LIEN"/></a>
                </TD>
                <TD nowrap>&nbsp;Aktivit&auml;t(en)</TD>
                <td>
                    <TEXTAREA name="activite" rows="5" cols="40"
                              onkeyup="maxLength(this, 254);"><%=viewBean.getActivite()%></TEXTAREA>
                    <INPUT type="hidden" name="confirmerAnnonceActivite"
                           value="<%=viewBean.getConfirmerAnnonceActivite()%>"/>
                </td>
            </TR>
            <TR>
                ...
                <TD nowrap width="100">
                    <INPUT type="checkbox" name="releveParitaire"
                           onclick="resetEAAVisibility()" <%=(viewBean.isReleveParitaire().booleanValue())? "checked" : ""%> >
                </TD>
                <TD rowspan="2" colspan="2">
                    <DIV id="eaa" style="display: none">
                        <INPUT type="checkbox"
                               name="envoiAutomatiqueAnnonceSalaires" <%=(viewBean.getEnvoiAutomatiqueAnnonceSalaires().booleanValue())? "checked" : ""%> >&nbsp;
                        Automatischer Versand
                        der Arbeitgeberauszüge
                    </DIV>
                </TD>
            </TR>
            <TR>
                <TD nowrap width="161">&nbsp;</TD>
                <TD nowrap width="210">für die persönlichen Beiträge</TD>
                <TD nowrap width="100">
                    <INPUT type="checkbox" name="relevePersonnel"
                           onclick="compteaLaCarte()" <%=(viewBean.isRelevePersonnel().booleanValue())? "checked" : ""%> >
                </TD>
            </TR>
            <TR>
                <TD nowrap width="161">&nbsp;</TD>
            ...
            <TR>
                ...
                <TD nowrap colspan="2">
                    ...
                    <% if (method != null && method.equals("add")) { %>
                    <INPUT type="checkbox"
                           name="envoiAutomatiqueLAA" <%=(viewBean.getEnvoiAutomatiqueLAA().booleanValue())? "checked" : ""%> >UVG
                    &nbsp;
                    <INPUT type="checkbox"
                           name="envoiAutomatiqueLPP" <%=(viewBean.getEnvoiAutomatiqueLPP().booleanValue())? "checked" : ""%> >BVG
                    <% } else {
                        if (viewBean.isEnvoiAutomatiqueLAA().booleanValue()) {
                    %>
                    <INPUT type="checkbox" name="envoiAutomatiqueLAAReadOnly" checked tabindex="-1" readonly disabled>UVG
                    &nbsp;
                    <% } else { %>
                    <INPUT type="checkbox" name="envoiAutomatiqueLAA">UVG &nbsp;
                    <% }
                        if (viewBean.hasLPPetMotif() == true) {

                            if (viewBean.isEnvoiAutomatiqueLPP().booleanValue()) { %>
                    <INPUT type="checkbox" name="envoiAutomatiqueLPPReadOnly" checked tabindex="-1" readonly disabled>BVG
                    <% } else { %>
                    <INPUT type="checkbox" name="envoiAutomatiqueLPPReadOnly" tabindex="-1" readonly disabled>BVG
                    <% } %>
                    <% } else {
                        if (viewBean.isEnvoiAutomatiqueLPP().booleanValue()) {%>
                    <INPUT type="checkbox" name="envoiAutomatiqueLPPReadOnly" checked tabindex="-1" readonly disabled>BVG
                    <% } else { %>
                    <INPUT type="checkbox" name="envoiAutomatiqueLPP">BVG
                    <% }
                    }
                    }%>
                </TD>
                ...
                <TD nowrap colspan="3">
                    <INPUT type="checkbox"
                           name="occasionnel" <%=(viewBean.isOccasionnel().booleanValue())? "checked" : ""%> >
            ...
            <TR>
                ...
                <TD nowrap colspan="2">
                    <INPUT type="checkbox"
                           name="exonerationGenerale" <%=(viewBean.isExonerationGenerale().booleanValue())? "checked" : ""%> >
                </TD>
                <!--TD nowrap></TD-->
                <TD nowrap colspan="3">
                    <INPUT type="checkbox"
                           name="traitement" <%=(viewBean.isTraitement().booleanValue())? "checked" : ""%>
                           onclick="displayFieldIDEPassif()">
            ...
            <TR id="isEBusiness">
                ...
                <TD nowrap colspan="2">
                    <INPUT id="isActivAffilieEBusiness" type="checkbox"
                           name="isActivAffilieEBusiness" <%=(viewBean.isActivAffilieEBusiness())? "checked" : ""%>
            ...
            <TR>
                ...
                <TD nowrap width="310">
                    <INPUT name="prefixeNumeroIDE" size="4" maxlength="4" type="text" value="CHE-" readOnly
                           tabindex="-1" class="prefixeIDEDisable" disabled="disabled">
                    ...
                    <INPUT type="text" id="numeroIDESansCHE" name="numeroIDESansCHE" onchange="displayFieldIDEPassif();"
                ...
                <TD>
                    ...
                    <div id="lienRegistreIde">
                        <a href="<%=AFIDEUtil.giveMeLienRegistreIde(viewBean.getSession(),viewBean.getNumeroIDE())%>"
                           target="_blank"><ct:FWLabel key="NAOS_JSP_AFFILIATION_IDE_LIEN_REGISTRE"/></a>
                    </div>
                    <%}%>
                </TD>
            </TR>
            ...
            <tr>
                ...
                <td colspan="2">
                    <INPUT type="text" id="ideRaisonSociale" name="ideRaisonSociale" class="libelleLong10Disabled"
                           readonly value="<%=viewBean.getIdeRaisonSociale()%>">
                </td>
            </tr>
            <TR>
                ...
                <TD colspan="2">
                    <INPUT type="text" id="libelleStatutIde" name="libelleStatutIde" class="libelleLong10Disabled"
                           readonly value="<%=CodeSystem.getLibelle(viewBean.getSession(),viewBean.getIdeStatut())%>">
            ...
            <TR>
                ...
                <TD nowrap colspan="2">
                    <INPUT id="ideNonAnnoncante" type="checkbox"
                           name="ideNonAnnoncante" <%=(viewBean.isIdeNonAnnoncante().booleanValue())? "checked" : ""%> >
                </TD>
            </TR>
            <TR id="displayIDEAnnoncePassive">
                <TD nowrap width="161"><ct:FWLabel key="NAOS_JSP_AFFILIATION_IDE_CHECKBOX_PASSIVE"/></TD>
                <TD nowrap colspan="2">
                    <INPUT id="ideAnnoncePassive" type="checkbox"
                           name="ideAnnoncePassive" <%=(viewBean.isIdeAnnoncePassive().booleanValue())? "checked" : ""%>
                           onclick="messageSiPassif()">

                </TD>
                <TD colspan="2">
                    <div id="messageAnnoncePassive" style="visibility:hidden;"><B><ct:FWLabel
                            key="NAOS_JSP_AFFILIATION_MESSAGE_IDE_ANNONCE_PASSIVE"/></B></div>
                </TD>
            </TR>
            ...
            <TR>
                ...
                <TD nowrap colspan="4">
                    ...
                    <ct:ifhasright element="leo.envoi.envoi.chercher" crud="r">
                        --
                        <A href="<%=request.getContextPath()%><%=urlGestionEnvoi%>">Versandverwaltung</A>
                    </ct:ifhasright>
                </TD>
            </TR>
            </TBODY>
        ...
        <TABLE border="0" cellspacing="0" cellpadding="0" id="tPartie2" style="display:none">
                ...
                <TD nowrap colspan="4" width="900">
                    <TABLE border="0" cellspacing="0" cellpadding="0">
                        ...
                        <TR style="font-weight: bold">
                            <TD nowrap width="160" height="14">Nachfolge der Kassen</TD>
                            <TD nowrap width="150">Externe Abr.-Nr.</TD>
                            <TD nowrap width="130">Begindatum</TD>
                            <TD nowrap width="130">Enddatum</TD>
                            <TD nowrap width="*">Nummer und Kassenname</TD>
                        </TR>
                        </TBODY>
                    </TABLE>
                </TD>
            ...
            <TR>
                <TD nowrap colspan="4" width="900">
                    <TABLE border="0" cellspacing="0" cellpadding="0">
                        <TBODY>
                        <TR>
                            <TD nowrap width="160" height="25" <%=style%>><%=libelleGenre%>
                            </TD>
                            <%
                                String libelleAdmin = "";
            ...
            <tr>
                <td width="150" nowrap="nowrap">Geschützte Erfassung (IK)</td>
                <td width="150" nowrap="nowrap">
                    <% if (viewBean.hasRightAccesSecurity()) {%>
                    <ct:FWCodeSelectTag name="accesSecurite" defaut="<%=viewBean.getAccesSecurite()%>"
                                        codeType="CISECURI" wantBlank="false"/>
                    <% } else { %>
                    <input title="Vous n'avez pas les droits requis pour modifier cette valeur" size="13" tabindex="-1"
                           name='accesSecuriteInv' readonly class="disabled"
                           value='<%=globaz.pavo.translation.CodeSystem.getLibelle(viewBean.getAccesSecurite(),session)%>'>
                    <% } %>
                </td>
            </tr>
