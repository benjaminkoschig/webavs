...
function del() {
	if (window.confirm("Sie sind dabei, das Auszugsdetail zu löschen! Wollen Sie fortfahren?")) {
...
<%@ include file="/theme/detail/bodyStart.jspf" %>
			<%-- tpl:put name="zoneTitle" --%>Auszug - Detail<%-- /tpl:put --%>
<%@ include file="/theme/detail/bodyStart2.jspf" %>
						<%-- tpl:put name="zoneMain" --%>
					<TR>
						<TD nowrap height="31">Mitglied</TD>
						<TD nowrap >
					...
					<TR>
						<TD nowrap height="31">Abrechnungstyp</TD>
						<TD nowrap></TD>
						<TD nowrap>
						...
						<TD nowrap colspan="3">
							...
							<TBODY>
							<TR>
						<TD nowrap width="150">Rechnung-Nr.</TD>
						<TD nowrap >
					...
					<TR>
						...
						<TD nowrap colspan="3">
							...
							<TBODY>
							<TR>
						<TD nowrap width="150">Mitarbeiter</TD>
						<TD nowrap>
					...
					<TR>
						...
						<TD nowrap colspan="3">
							...
							<TBODY>
							<TR>
							<TD nowrap width="150">Empfangsdatum</TD>
							<TD nowrap>
					...
					<TR>
						...
						<%
	              		...
	              		<A href="<%=request.getContextPath()%>/musca?userAction=musca.facturation.passage.afficher&selectedId=<%=viewBean.getIdPassage()%>" class="external_link">Fakturierungsjournal</A>
	              		</ct:ifhasright>
	              		<%
	              			} else {
	              		%>
							Fakturierungsjournal
	              		<%
	              		}
	          			%>
						</TD>
					...
					<TR>
						<TD nowrap height="31">Kontrolltotal</TD>
						<TD nowrap></TD>
						<TD nowrap>
							...
							nowrap width="161"><A href="<%=request.getContextPath()%>\musca?userAction=musca.facturation.afact.chercher&idEnteteFacture=<%=viewBean.getIdEntete()%>&idPassage=<%=viewBean.getIdPassage()%>&selectedId=<%=viewBean.getIdTiers()%>" class="external_link">Faktzeilen Erstellung</A>
							 </ct:ifhasright>
							</TD>
							<% } %>
					</TR>
					...
					%>
						<TD nowrap height="31" colspan="3"><b>Versicherung des Mitglieds</b></TD>
						<TD nowrap><b>Lohnsumme/Betrag</b></TD>
						<TD nowrap><b>Beitragsatz</b></TD>
						<TD nowrap><b>Beitragsbetrag</b></TD>
					<%
					...
					<TR>
						...
						<TD nowrap height="31">
						<%=viewBean.isMontantNegatif() ? "<span style='font-weight: bold; color: red;'>Achtung negativ</span>" : ""%><INPUT class="btnCtrl" id="btnCalcul" type="button" value="Berechnen" onclick="calcul();">
						</TD>
						<TD nowrap>Total</TD>
						<TD nowrap>
...
<%@ include file="/theme/detail/bodyButtons.jspf" %>
				...
				<% if (bButtonFacturer) {%><INPUT class="btnCtrl" id="btnFacturer" type="button" value="Fakturieren" onclick="facturer();" ><% } %>
				<% if (bButtonDevalider) {%><INPUT class="btnCtrl" id="btnDevalider" type="button" value=Devalidieren" onclick="devalider();" ><% } %>
				<%}%>
				<%-- /tpl:put --%>
