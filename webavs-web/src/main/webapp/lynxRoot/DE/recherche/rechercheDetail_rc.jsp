...
<%
	...
	String titre = "";
	String numeroInterne = "Interne Rechnung-Nr.";
	
	if(LXSection.CS_TYPE_FACTURE.equals(csTypeSection)) {
		titre = "Detail einer Rechnung";
	}else {
		numeroInterne = "Interne Gutschrift-Nr.";
		titre = "Detail einer Gutschrift";
	}
	...
	<TR>
		...
		<TD>
			<SELECT id="forEtat" name="forEtat"onchange="javascript:changeMontant()" tabindex="1">
				<OPTION value="<%=LXConstants.ETAT_DEFINITIF %>" <%= LXConstants.ETAT_DEFINITIF.equals(request.getParameter("forEtat")) ? "selected=\'selected\'" : "" %> >Definitiv</OPTION>
				<OPTION value="<%=LXConstants.ETAT_PROVISOIRE %>" <%= LXConstants.ETAT_PROVISOIRE.equals(request.getParameter("forEtat")) ? "selected=\'selected\'" : "" %>>Provisorisch</OPTION>
			</SELECT>
		</TD>
      	<TD>&nbsp;</TD>	
	</TR>	
