...
function del() {
	if (window.confirm("Sie sind dabei, die ausgewählte Rechnung zu löschen! Wollen Sie fortfahren?")) {
...
<TABLE border="0" cellspacing="0" cellpadding="0" width="100%">
	<TBODY>
		...
		<%
			if (viewBean.isJournalEditable() && "add".equals(request.getParameter("_method"))) {
		%>
		<INPUT type="button" name="btnNewFactureBvrOrange" value="Oranger ESR" onClick="document.location.href='<%=actionNewFactureBvrOrange%>'">
		<INPUT type="button" name="btnNewFactureBvrRouge" value="Roter ESR" onClick="document.location.href='<%=actionNewFactureBvrRouge%>'">
		<INPUT type="button" name="btnNewFactureVirement" value="Überweisung" onClick="document.location.href='<%=actionNewFactureVirement%>'">
		<INPUT type="button" name="btnNewFactureLsv" value=" LSV " onClick="document.location.href='<%=actionNewFactureLsv%>'">
		<INPUT type="button" name="btnNewFactureCaisse" value="Kasse" onClick="document.location.href='<%=actionNewFactureCaisse%>'">
		<%
		...
		<ct:ifhasright element="lynx.extourne.extourneProcess.extourner" crud="u">
			<INPUT type="button" name="Stornieren" value="Extourner" onClick="document.location.href='<%=request.getContextPath()%>/lynx?userAction=lynx.extourne.extourneProcess.extourner&idJournal=<%=viewBean.getIdJournal()%>&idSociete=<%=viewBean.getIdSociete()%>&idSection=<%=viewBean.getIdSection()%>&idOperation=<%=viewBean.getIdOperation()%>'">
		</ct:ifhasright>
		<% } %>
