...
<%
globaz.hermes.db.parametrage.HEAttenteEnvoiCIViewBean viewBean = (globaz.hermes.db.parametrage.HEAttenteEnvoiCIViewBean)session.getAttribute("viewBean");
...
function validate() {} 
function cancel() {
  document.forms[0].elements('userAction').value="hermes.parametrage.attenteEnvoiCI.afficher";
}
...
<%@ include file="/theme/detail/bodyStart.jspf" %>
			<%-- tpl:put name="zoneTitle" --%>Detail der Periode<%-- /tpl:put --%>
...
%>
          <tr> 
            <td width="40%">&nbsp;<%=viewBean.getFieldLibelle(i)%>&nbsp;:&nbsp;</td>
            <td>&nbsp; 
              <input name="<%=viewBean.getFieldId(i)%>" value="<%=globaz.commons.nss.NSUtil.formatAVSUnknown(viewBean.getFieldValue(i))%>" size="<%=viewBean.getFieldLength(i,5)%>" maxlength="<%=viewBean.getFieldLength(i,3)%>">
            </td>
            <%
