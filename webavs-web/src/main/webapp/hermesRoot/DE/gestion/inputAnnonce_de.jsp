...
{
	while (input.value.search('‰') != -1)
		input.value=input.value.replace('‰','AE');
	while (input.value.search('ˆ') != -1)
		input.value=input.value.replace('ˆ','OE');
	while (input.value.search('¸') != -1)
	...
	while (input.value.search('Î') != -1)
		input.value=input.value.replace('Î','E');
	while (input.value.search('Í') != -1)
		input.value=input.value.replace('Í','E');					
	while (input.value.search('Ù') != -1)
		input.value=input.value.replace('Ù','O');
	while (input.value.search('‡') != -1)
		input.value=input.value.replace('‡','A');
		
	while (input.value.search('ƒ') != -1)
		input.value=input.value.replace('ƒ','AE');
	while (input.value.search('¿') != -1)
		input.value=input.value.replace('¿','A');
	while (input.value.search('¬') != -1)
		input.value=input.value.replace('¬','A');					
	...
	while (input.value.search('À') != -1)
		input.value=input.value.replace('À','E');
	while (input.value.search(' ') != -1)
		input.value=input.value.replace(' ','E');	
	while (input.value.search('÷') != -1)
	...
	while (input.value.search('Á') != -1)
		input.value=input.value.replace('Á','C');	
	while (input.value.search('«') != -1)
...
//	var re = /[^a-zA-Z\-'‰ˆ¸,\s].*/
	var re = /[^a-zA-Z0-9\-.,-/‰ˆ¸ƒ÷‘‹À…» «ÈËÙ‡ÎÁ,\s].*/
...
function trim(valueToTrim)
{
  ...
  // tester si le numÈro avs entrÈ comporte slt des numÈros et/ou des .
  var cre = /((\d|\.)+)/;
  if (!cre.test(valueToTrim)) {
	valueToTrim = "";
  }
  // ContrÙle ‡ chaque touche si le NSS est vide pour activer ou non la case
...
// "surcharge" de la mÈthode doInitThings 
...
function updateJspNameCat()
{
	if(document.getElementById('categorie')!=null){
		...
		var indÈ = '<%=IHEAnnoncesViewBean.CS_CATEGORIE_INDEPENDANT%>';
...
%>
		...
		/* numÈro d'affiliÈ pour le CA ou pour la dÈclaration de salaire */			
		if(
			(
...
<script>
	// on ne veut pas afficher le message d'erreur dans une boÓte de dialogue
