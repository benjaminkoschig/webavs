...
<%@page import="globaz.amal.vb.sedexrp.AMSedexrpViewBean"%>
<%@page import="globaz.amal.vb.sedexrp.AMSedexrpListViewBean"%>
...
<%-- tpl:insert attribute="zoneScripts" --%>
	    <%
	    	AMSedexrpListViewBean viewBean=(AMSedexrpListViewBean)request.getAttribute("viewBean");	    	
   	    	size=viewBean.getSize();
	    	    	    	
   	    	menuName = "amal-menuprincipal";	 
   	    	
   	    	boolean hasRightUpdate = objSession.hasRight("amal.sedexrp.sedexrp", FWSecureConstants.UPDATE);
	    %>
<%-- /tpl:insert --%>
...
<%@ include file="/theme/list/tableHeader.jspf" %>
    ...
    <%
    	AMSedexrpViewBean line = (AMSedexrpViewBean)viewBean.getEntity(i);
    %>
    <%-- /tpl:insert --%>
...
<% String detailSubsideLink = "amal?userAction=amal.detailfamille.detailfamille.afficher&selectedId="+line.getComplexAnnonceSedex().getSimpleDetailFamille().getIdDetailFamille(); %>
		<%-- tpl:insert attribute="zoneList" --%>
			...
			<ct:menuPopup menu="amal-optionsannoncesrp" detailLabelId="MENU_SEDEXRP_SUBSIDE_DETAIL" detailLink="<%=detailSubsideLink%>">
				...
				if (AMMessagesTypesAnnonceSedex.NOUVELLE_DECISION.getValue().equals(msgType)
						|| AMMessagesTypesAnnonceSedex.INTERRUPTION.getValue().equals(msgType)
						|| AMMessagesTypesAnnonceSedex.DEMANDE_RAPPORT_ASSURANCE.getValue().equals(msgType)
						|| AMMessagesTypesAnnonceSedex.ETAT_DECISIONS.getValue().equals(msgType)) {
				%>
					<img
					width="22px"
					height="22px"
