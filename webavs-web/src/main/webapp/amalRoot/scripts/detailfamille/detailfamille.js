...
$(document).ready(function() {		
    ...
    //
    // Action pour afficher la s�lection des templates
    // -----------------------------------------------
    ...
    $('#cmSuggestChoice').keyup(function(e) {
    	// Touche Enter, s�lection et retour dans champ input
    	if(e.which==13){
    		// Get and Set Value
...
//------------------------------------------------------------
//Appel Ajax pour g�n�rer l'annonce 'Demande de rapport d'assurance'
//------------------------------------------------------------
...
//------------------------------------------------------------
//Appel Ajax pour g�n�rer une annonce Decree ou Stop
//------------------------------------------------------------
...
//------------------------------------------------------------
//Appel Ajax pour g�n�rer une annonce demande prime tarofaire
//------------------------------------------------------------
...
/**
 * Cette m�thode rattache les event. N�cessaire car le plugin jquery.contextMenu unbind tout lors de sa destruction
 */
...
//------------------------------------------------------------
// Adapte la liste des caisses maladies � afficher en fonction
...
//------------------------------------------------------------
// Adapte l'�l�ment input cmSuggestChoice avec la nouvelle liste
//------------------------------------------------------------
...
//------------------------------------------------------------
// Action de g�n�ration de document (action du bouton Generate)
//------------------------------------------------------------
function onClickGenerateDocument(){
	...
	if(batchOrNotBatch=="42001100"){
		...
		$('#mask, .window').hide();
		alert("G�n�ration du document impossible, cat�gorie Batch-Direct non sp�cifi�e.");
	}
}

//------------------------------------------------------------
// G�n�ration du document interactif
//------------------------------------------------------------
				...
				if(data!=null && data.length>4 && data.substr(data.length-4,4)==".doc"){
					...
					try{
						...
						if(s_filepath.indexOf('persistence')>=0){
							...
							var s_httpError = 'Le document a �t� g�n�r� sur le serveur web (http).';
							s_httpError+= '\nLa sauvegarde et la mise en GED automatique ne peut �tre assur�e.';
							s_httpError+= '\nVeuillez v�rifier les param�tres de configuration de l\'application';
							alert(s_httpError);
						}
...
//------------------------------------------------------------
// Ajout du document dans la queue des documents � g�n�r�s (TOPAZ)
//------------------------------------------------------------
function onClickQueue(){
	...
	// Get the id of the document to merge
	// il est mentionn� en tant que classe...
...
//------------------------------------------------------------
// trim tous les caract�res qui ne sont pas compris 
// entre 0 et 128
...
// du formulaire de g�n�ration de document
//------------------------------------------------------------
...
//------------------------------------------------------------
// G�n�ration du tableau d'historique
//------------------------------------------------------------
var textToInsert = '';
...
//------------------------------------------------------------
// Appel Ajax pour g�n�rer l'historique des annonces
//------------------------------------------------------------
...
//------------------------------------------------------------
//Appel Ajax pour g�n�rer l'historique des envois
//------------------------------------------------------------
...
//------------------------------------------------------------
// CallBack Ajax pour g�n�rer l'historique des annonces
//------------------------------------------------------------
function createHistoriqueAnnonces(data) {
	...
	if(data.length == 0){
		textToInsert += '<tr class="divInfoSubsideHistoEnvoiExpand_Hidable"><td></td><td colspan="11">Pas d\'annonce caisse-maladie effectu�</td></tr>';
	}
	textToInsert += '<tr class="divInfoSubsideHistoEnvoiExpand_Hidable"><td>&nbsp;</td></tr>';
	generateHistoriqueEnvois(currentIdDetailFamille);
}
//------------------------------------------------------------
//CallBack Ajax pour g�n�rer l'historique des envois
//------------------------------------------------------------
function createHistoriqueEnvois(data) {
	...
	if(documentInProgress == 0){
		textToInsert += '<tr class="divInfoSubsideEnvoiModifExpand_Hidable"><td></td><td colspan="11">Pas d\'�l�ment modifiable actif</td></tr>';
	}
...
//------------------------------------------------------------
//Appel Ajax pour g�n�rer l'historique des annonces SEDEX RP
//------------------------------------------------------------
...
function createHistoriqueSEDEXRP(data) {
	...
	// Fin du traitement, on cache l'ic�ne de chargement
	$('#ajaxLoaderImgLine').hide();
	// On append notre r�sultat
...
function attachContextualMenu() {
    ...
    function(action, el, pos, attrIdClicked) {
    	...
    	if (action == 'viewXml') {
			...
			if (confirm("Cloner l'annonce pour r��mission ?")) {
				reSubmitAnnonce(idAnnonceSedex);
			}
		} else if (action == 'delAnnonce') {
			if (confirm("Supprimer l'annonce (seulement si l'annonce n'a pas �t� envoy�e) ?")) {
...
function showCounters() {
	if (nbElemHistoEnvoi==0) {
		$("#nbElemHistoEnvoi").html("(0 �l�ment)");
		$(".divInfoSubsideHistoEnvoiExpand").click();
	} else if (nbElemHistoEnvoi==1) {
		$("#nbElemHistoEnvoi").html("("+nbElemHistoEnvoi+" �l�ment)");
	} else {
		$("#nbElemHistoEnvoi").html("("+nbElemHistoEnvoi+" �l�ments)");
	}
	
	if (nbElemEnvoiModifs==0) {
		$("#nbElemAnnoncesModifs").html("(0 �l�ment)");
		$(".divInfoSubsideEnvoiModifExpand").click();
	} else if (nbElemEnvoiModifs==1) {
		$("#nbElemAnnoncesModifs").html("("+nbElemEnvoiModifs+" �l�ment)");
	} else {
		$("#nbElemAnnoncesModifs").html("("+nbElemEnvoiModifs+" �l�ments)");
	}
	

	if (nbElemHistoSedex==0) {
		$("#nbElemAnnoncesSedex").html("(0 �l�ment)");
		$(".divInfoSubsideSedexExpand").click();
	} else if (nbElemHistoSedex==1) {
		$("#nbElemAnnoncesSedex").html("("+nbElemHistoSedex+" �l�ment)");
	} else {
		$("#nbElemAnnoncesSedex").html("("+nbElemHistoSedex+" �l�ments)");
	}
}
