...
<div id="conteneurComplexAnnonceSedex">
	<table width="100%" border="0">
		...
		<tr style="height:26px" class="<%=rowStyle%>" onMouseOver="jscss('swap', this, '<%=rowStyle%>', 'amalRowHighligthed')" onMouseOut="jscss('swap', this, 'amalRowHighligthed', '<%=rowStyle%>')">
			...
			<td><%=currentAnnonce.getSimpleAnnonceSedexCO().getMessageSubTypeLibelle()%></td>
			...
			<%
				...
				if(!contribReprise){
				    nomContribuable = viewBean.getContribuable().getPersonneEtendue().getTiers().getDesignation1();
				    nomContribuable += " " + viewBean.getContribuable().getPersonneEtendue().getTiers().getDesignation2();
				}else{
				    nomContribuable = AMContribuableHistoriqueHelper.getContribuableInfos().getNom();
				    nomContribuable += " " + AMContribuableHistoriqueHelper.getContribuableInfos().getPrenom();
				} %>
