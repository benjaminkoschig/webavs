...
<%
	...
	CGImprimerBalanceComptesViewBean viewBean = (CGImprimerBalanceComptesViewBean) session.getAttribute ("viewBean");
	...
	userActionValue = "helios.print.imprimerBalanceComptes.executer";
...
<%-- tpl:put name="zoneTitle" --%><ct:FWLabel key="GCF2002_BALANCECOMPTE_TITRE"/><%-- /tpl:put --%>
...
<%-- tpl:put name="zoneMain" --%>
		...
		<tr>
			...
			<TD><input type="checkbox" name="inclurePeriodePrecedente" <%=(viewBean.isInclurePeriodePrecedente())?"CHECKED":""%>/></TD>
		</tr>

		<tr>
			<td><ct:FWLabel key="GCF20XX_CRITERE_DOMAINECOMPTES"/></td>
			<td> <ct:FWCodeSelectTag name="idDomaine" defaut="<%=viewBean.getIdDomaine()%>" codeType="CGDOMCPT" /></td>
		</tr>
		...
		<tr>
			<td></td>
			<td>
			<iframe name="classeCompteIFrame" id="classeCompteIFrame" src="<%=servletContext%><%=((String)request.getAttribute("mainServletPath")+"Root")%>/<%=globaz.framework.controller.FWDefaultServletAction.getIdLangueIso(session)%>/comptes/classeCompte_select.jsp?idDefinitionListe=<%=viewBean.getIdDefinitionListe()%>" width=500px height=200px></iframe>
			</td>
		</tr>
