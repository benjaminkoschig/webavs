<%-- mtpl:insert page="/theme/list.jtpl" --%><!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN">
<HTML>
<!--# set echo="url" -->
...
<%
		globaz.fx.user.client.bean.FXUserGroupListViewBean viewBean =
		(globaz.fx.user.client.bean.FXUserGroupListViewBean) request.getAttribute("viewBean");
		size= viewBean.getSize();
		target="parent.fr_detail";
		targetLocation = target + ".location.href";
		detailLink ="fx?userAction=fx.user.userGroup.afficher&selectedId=";
...
%>
<%-- mtpl:put name="zoneList" --%>
<%
	actionDetail = targetLocation+ "='" +detailLink+java.net.URLEncoder.encode(viewBean.getGroupIds()[i],"iso-8859-1")+"&idUser="+viewBean.getUserIds()[i]+"&dateStart="+viewBean.getDateStarts()[i]+"&dateEnd="+viewBean.getDateEnds()[i]  +"'";
%>

   <TD class="mtd" width="16" ></TD>
  	<TD class="mtd" onClick="<%=actionDetail%>" ><%=viewBean.getUserIds()[i]%>&nbsp;</TD>
  	<TD class="mtd" onClick="<%=actionDetail%>" ><%=viewBean.getUserVisas()[i]%>&nbsp;</TD>
  	<TD class="mtd" onClick="<%=actionDetail%>" ><%=viewBean.getGroupIds()[i]%>&nbsp;</TD>
	<TD class="mtd" onClick="<%=actionDetail%>" ><%=viewBean.getDescriptions()[i]%>&nbsp;</TD>

<%-- /mtpl:put --%>
    </TR>
  <%	} %>
<%-- mtpl:put name="zoneTableFooter" --%>
<%-- /mtpl:put --%>
  </TBODY>
</TABLE>
