...
public class CIDeclarationsAnciensClientsIterator implements ICIDeclarationIterator {
    ...
    private CIDeclarationRecord _fillBean(String line) {
        ...
        // champs
        String numeroAffilie = line.substring(1, 8);
        String numeroAffilieFormate = new String();
        try {
        ...
        String noAvs = line.substring(8, 19);
        ...
        int moisFin = 12;
        try {
            moisDebut = Integer.parseInt(line.substring(27, 29));
        } catch (Exception e) {
            moisDebut = 1;
        }
        try {
            moisFin = Integer.parseInt(line.substring(29, 31));
        } catch (Exception e) {
            moisFin = 12;
        }

        String annee = line.substring(31, 33);
        if (Integer.parseInt(annee) > 48) {
            annee = "19" + annee;
        } else {
            annee = "20" + annee;
        }
        String montantEcr = line.substring(33, 40) + "." + line.substring(40, 42);
    ...
    private void _init() throws Exception {
        ...
        try {
            if (!isReady) {
                fi = new java.io.FileInputStream(getFilename());
    ...
    public boolean hasNext() throws Exception {
        ...
        if (line != null) {
            ...
            line = br.readLine();
            try {
                while ("".equals(line) || !line.startsWith("B")) {
                    line = br.readLine();
                }
            } catch (Exception e) {
                return false;
    ...
    public int size() throws Exception {
        try {
            if (size == -1) {
                size = 0;
                java.io.FileInputStream sfi = new java.io.FileInputStream(getFilename());
