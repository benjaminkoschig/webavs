...
/**
 * Impression de la Liste A des augmentations d'un mois de rapport
 * 
 * @author BSC
 */
public class REListeAAugmentations extends FWIAbstractDocumentList {
    ...
    public REListeAAugmentations() throws Exception {
    ...
    public void _beforeExecuteReport() {
        ...
        try {
            ...
            getDocumentInfo().setDocumentTypeNumber(IRENoDocumentInfoRom.LISTE_A_AUGMENTATIONS);
    ...
    protected void initializeTable() {
        ...
        _addColumnLeft(getSession().getLabel("LISTE_AAU_CATEGORIE"), 2);
        _addColumnLeft(getSession().getLabel("LISTE_AAU_DETAIL_ASSURE"), 5);
        _addColumnLeft(getSession().getLabel("LISTE_ADI_NSS_COMPLEMENTAIRE"), 2);
        _addColumnLeft(getSession().getLabel("LISTE_ADI_NSS_COMPLEMENTAIRE"), 2);
        _addColumnLeft(getSession().getLabel("LISTE_AAU_GENRE"), 1);
        _addColumnRight(getSession().getLabel("LISTE_AAU_DEBUT"), 1);
    ...
    private void populate() throws Exception {
        ...
        this._addLine(getSession().getLabel("LISTE_AAU_AUGMENTATIONS"), "", "");
        this._addLine("", "", "");
        ...
        for (REAnnonces50Container.KeyLevelNomPrenom key : mapLevelNomPrenom.keySet()) {

            List<REAnnonce50VO> annonces = mapLevelNomPrenom.get(key);
            ...
            for (REAnnonce50VO annonce50 : annonces) {
                ...
                if (!JadeStringUtil.isBlank(annonce50.getNssAssureCompl1())) {
                    _addCell(NSUtil.formatAVSUnknown(annonce50.getNssAssureCompl1()));
                } else {
                    _addCell("");
                }

                if (!JadeStringUtil.isBlank(annonce50.getNssAssureCompl2())) {
                    _addCell(NSUtil.formatAVSUnknown(annonce50.getNssAssureCompl2()));
                } else {
                    _addCell("");
                }

                _addCell(annonce50.getGenrePrestations());
                _addCell(PRDateFormater.convertDate_MMAA_to_MMxAAAA(annonce50.getDebutDroit()));
                _addCell(PRDateFormater.convertDate_MMAA_to_MMxAAAA(annonce50.getFinDroit()));

                // on ne prend pas les annonces du genre
                // "annonces de modification"
                if ("4".equals(annonce50.getGenreAnnonce())) {
                    _addCell("");
                } else {
