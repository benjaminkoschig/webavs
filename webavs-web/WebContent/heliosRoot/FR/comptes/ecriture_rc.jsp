...
<%@ include file="/theme/find/bodyStart.jspf" %>
				<%-- tpl:put name="zoneTitle" --%>Ecriture<%-- /tpl:put --%>
<%@ include file="/theme/find/bodyStart2.jspf" %>
						<%-- tpl:put name="zoneMain" --%>
				<tr>
					<td colspan="4">Mandat <input class='libelleLongDisabled' readonly value='<%=exerciceComptable.getMandat().getLibelle()%>'><input type="hidden" name="forIdJournal" value="<%=viewBean.getIdJournal()%>"> Exercice <input class='disabled' readonly value='<%=exerciceComptable.getFullDescription()%>'> Journal <input class='disabled' readonly value="<%=viewBean.getJournalLibelle()%>"></td>
				</tr>
				<tr>
					<td colspan="4"><hr></td>
				</tr>
...
<%@ include file="/theme/find/bodyButtons.jspf" %>
				...
				if (viewBean.isJournalEditable()) {
				%>
					<INPUT type="button" name="btnNewEcriture" value="Nouveau" onClick="document.location.href='<%=actionNewEcriture%>'" >
				<%
				}
				%>
				<%-- /tpl:put --%>
