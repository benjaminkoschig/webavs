...
top.document.title = "Process - Lettrer - " + top.location.href;
// stop hiding -->
</SCRIPT>
...
function disableAutolist() {
	...
	for (int i = 0; i < sectionListViewBean.size(); i++) {
		...
		if (tmpSection.getAttenteLSVDD().booleanValue()){
			%>document.getElementById("info").value="Attente LSV/DD";<%
		} else {
			%>document.getElementById("info").value="";<%
		}
	}
...
<%@ include file="/theme/process/bodyStart.jspf" %>
			<%-- tpl:put name="zoneTitle" --%>Lettrer<%-- /tpl:put --%>
<%@ include file="/theme/process/bodyStart2.jspf" %>
						<%-- tpl:put name="zoneMain" --%>

					<tr>
						<td nowrap>Débiteur</td>
					...
					<tr>
						<td nowrap>Montant à lettrer</td>
						 <td nowrap>
					...
					<tr>
						<td nowrap>Mode</td>
						<td nowrap class="gText">
							<input type="radio" name="mode" value="<%=CAJournalOperationLettrerViewBean.MODE_AUTOMATIC%>" checked onCLick="javascript:disableAutolist()" />Automatique
							<br/>
							<input type="radio" name="mode" value="<%=CAJournalOperationLettrerViewBean.MODE_MANUAL%>" onCLick="javascript:enableAutolist()"/>Manuel
							...
							<%
								for (int i = 0; i < sectionListViewBean.size(); i++) {
									...
									if (tmpSection.getSoldeToCurrency().isPositive()) {
										if (tmpSection.getAttenteLSVDD().booleanValue()) {
											out.write("<option value=\"" + tmpSection.getIdSection() + "\">" + tmpSection.getIdExterne() + " (" + tmpSection.getSolde()  + " CHF)" + " Attente LSV/DD" + "</option>");
										} else {
