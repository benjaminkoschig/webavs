...
<c:set var="labelTitreEcran" value="JSP_LISTES_COMMUNICATIONSALAIRES"/>
...
$("#launchListe").click(function () {
		...
		if(globazGlobal.checkFields()) {
			document.forms[0].elements('userAction').value="vulpecula.listes.communicationsalaires.executer";
...
<div style="width: 100%;text-align: center;">
	<div>
		<table>			
			...
			<tr>
				<td><label for="typeDecompte"><ct:FWLabel key="JSP_TYPE_DECOMPTE"/></label></td>
				<td>
					<select name="codeTypeDecompte">
						<c:forEach var="typeDecompte" items="${viewBean.typeDecompte}">
							<c:choose>
								<c:when test="${viewBean.typeDecompte==typeDecompte.type.value}">
									<option selected="selected" value="${typeDecompte.type.value}">${typeDecompte.typeDecompteLibelle}</option>
								</c:when>
								<c:otherwise>
									<option value="${typeDecompte.type.value}">${typeDecompte.typeDecompteLibelle}</option>
								</c:otherwise>
							</c:choose>
						</c:forEach>
					</select>
			...
			<c:if test="${hasCreateRightOnListes}">
				<tr>
					<td><label for="miseAJour"><ct:FWLabel key="JSP_MISE_A_JOUR"/></label></td>
					<td>
						<c:choose>
							<c:when test="${viewBean.miseAJour}">
								<input id="miseAJour" name="miseAJour" type="checkbox" checked="checked" />
							</c:when>
							<c:otherwise>
								<input id="miseAJour" name="miseAJour" type="checkbox"  />
							</c:otherwise>
						</c:choose>
					</td>
				</tr>
