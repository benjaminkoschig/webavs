...
globaz.corvus.vb.prestations.REPrestationsJointTiersViewBean viewBean = new globaz.corvus.vb.prestations.REPrestationsJointTiersViewBean();
...
if(provenance.equals(globaz.corvus.vb.prestations.REPrestationsJointTiersViewBean.FROM_ECRAN_LOTS)){
	...
	viewBean.setIdDecision(selectedId);
}

viewBean.setCsTypeLot(csTypeLot);
viewBean.setCsEtatLot(csEtatLot);
viewBean.setProvenance(provenance);
...
<SCRIPT language="javascript">
	bFind = true;
	usrAction = "corvus.prestations.prestationsJointTiers.lister";
</SCRIPT>
<%-- /tpl:put --%>
<%@ include file="/theme/find/bodyStart.jspf" %>
				<%-- tpl:put name="zoneTitle" --%><ct:FWLabel key="JSP_PRE_R_TITRE"/><%-- /tpl:put --%>
<%@ include file="/theme/find/bodyStart2.jspf" %>
						<%-- tpl:put name="zoneMain" --%>
						<%if(viewBean.getProvenance().equals(globaz.corvus.vb.prestations.REPrestationsJointTiersViewBean.FROM_ECRAN_LOTS)){%>
						<TR>
						...
						<TR>
							...
							<TD>
								...
								<INPUT type="hidden" name="csTypeLot" value="<%=viewBean.getCsTypeLot()%>">
							</TD>
						</TR>
						<%}else{%>
						<TR>
							...
							<TD colspan="3">
								<INPUT type="text" name="decisionDescription" value="<%=viewBean.getIdDecision()%>" class="libelleLongDisabled" readonly="readonly">
								<INPUT type="hidden" name="forIdDecision" value="<%=viewBean.getIdDecision()%>">
							</TD>
						</TR>
						<%}%>
						<TR><TD>&nbsp;</TD></TR>
