...
<tr >
	...
	<table >
		<tr>
			...
			<td>Adresse de paiement&nbsp;(<b><%=viewBean.getSession().getCodeLibelle(viewBean.getDomainePaiement())%></b>)
	...
	<td colspan="4" >
		...
		<% if (globaz.pyxis.constantes.IConstantes.CS_APPLICATION_DEFAUT.equals(viewBean.getExtDomaine())) {%>
			/ <input accesskey=" " type="radio" name="extType" <%=((globaz.pyxis.db.adressecourrier.TIAvoirAdresse.CS_DOMICILE.equals(viewBean.getExtType()))?"CHECKED":"")%>
				...
				value="<%=globaz.pyxis.constantes.IConstantes.CS_AVOIR_ADRESSE_DOMICILE%>" ><%=viewBean.getSession().getCodeLibelle(globaz.pyxis.db.adressecourrier.TIAvoirAdresse.CS_DOMICILE)%>
				&nbsp;		
			<input type="radio" name="extType" <%=((globaz.pyxis.constantes.IConstantes.CS_AVOIR_ADRESSE_COURRIER.equals(viewBean.getExtType()))?"CHECKED":"")%>
				onclick="document.getElementById('casePostaleTR').style.visibility='visible';"
				value="<%=globaz.pyxis.constantes.IConstantes.CS_AVOIR_ADRESSE_COURRIER%>"><%=viewBean.getSession().getCodeLibelle(globaz.pyxis.db.adressecourrier.TIAvoirAdresse.CS_COURRIER)%>
		<%} else {%>
			/ <%=viewBean.getSession().getCodeLibelle(globaz.pyxis.constantes.IConstantes.CS_AVOIR_ADRESSE_COURRIER)%><input name="extType" type="hidden" value="<%=globaz.pyxis.constantes.IConstantes.CS_AVOIR_ADRESSE_COURRIER%>" >
		<%}%>
		</b>)<br><br> 
