...
<%
	String idEcran ="GTI4000";
	globaz.pyxis.vb.maintenance.TIMaintenanceViewBean viewBean = (globaz.pyxis.vb.maintenance.TIMaintenanceViewBean) session.getAttribute("viewBean");
	userActionValue = "pyxis.maintenance.maintenance.executer";
	formAction= request.getContextPath()+mainServletPath+"Root/"+languePage+"/maintenance/maintenanceFile_de.jsp";
	
%>
<SCRIPT language="JavaScript">
top.document.title = "Tiers - Maintenance"
...
function validateform() {
	...
	if (document.getElementsByName("filename")[0].value == "") {
		...
		document.getElementById('btnOk').readonly = true;
	...
	<TBODY>
		<TR>
			<TH colspan="3" height="10" class="title">
				<DIV style="width: 100%">
					<SPAN class="idEcran"><%=(null==idEcran)?"":idEcran%></SPAN>Maintenance</DIV>
			</TH>
		</TR>
		<TR>
		...
		<TR>
          ...
          <tr>
          	<td>Simulation</td>
          	<td>
          		<input type="checkBox" CHECKED name="simulation">
          	</td>
          </tr>
		...
		<tr id="message_confirmation" style="display:none">
			<td colspan="3" style="height: 2em; color: white; font-weight: bold; text-align: center;background-color: green">Le fichier est en cours de chargement, veuillez patienter</td>
		</tr>
		<TR>
			<TD bgcolor="#FFFFFF" colspan="3" align="center"><INPUT type="submit" id="btnOk" value="Ok" style="width:60" onClick="if(validateform())document.forms[0].submit();"></TD>
		</TR>
		<TR>
