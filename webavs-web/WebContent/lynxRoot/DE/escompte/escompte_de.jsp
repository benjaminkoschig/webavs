...
function init() {
	top.document.title = "Detail eines Diskonts - " + top.location.href;
		...
		<TR>
			...
			<TD>Diskontsatz &nbsp;
				<% if(!JadeStringUtil.isBlankOrZero(viewBean.getIdOperationLiee())) { %>
					(<A href="<%=request.getContextPath()%>/lynx?userAction=lynx.paiement.paiement.afficher&idOperation=<%=viewBean.getIdOperationLiee()%>" class="link">Verknüpfte Zahlung</A>)
				<% } %>			
			</TD>
