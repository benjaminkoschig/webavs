...
<%@ include file="/theme/detail/bodyStart2.jspf" %>
        ...
        <TR>
            <TD>Total notes de cr&eacute;dits</TD>
            <TD colspan="2">
          ...
          <TR>
			...
			<%
	              if (!globaz.jade.client.util.JadeStringUtil.isIntegerEmpty(viewBean.getIdJournalCG())) {
	              	%>
	              <A href="<%=request.getContextPath()%>/helios?userAction=helios.special.journal.afficher&idJournal=<%=viewBean.getIdJournalCG()%>" class="external_link">Journal comptable associé</A>
	              	<%
	              } else {
	              %>
        ...
        <TR>
			...
			%>
	              <A href="<%=request.getContextPath()%>/lynx?userAction=lynx.ordregroupe.ordreGroupe.afficher&selectedId=<%=viewBean.getIdOrdreGroupeSrc()%>" class="link">Ordre groupé associé</A>
	              	<%
	              		} else {
	              	%>
					Ordre groupé associé
	              <%
	              		}
	              	%>
			</TD>
