...
globaz.apg.vb.lots.APFactureJointCompensationListViewBean viewBean = (globaz.apg.vb.lots.APFactureJointCompensationListViewBean) request.getAttribute("viewBean");
globaz.prestation.tools.PRIterateurHierarchique iterH = viewBean.iterateurHierarchique();

size = viewBean.getSize ();
detailLink = servletContext + mainServletPath + "?userAction=" + globaz.apg.servlet.IAPActions.ACTION_FACTURES_LOT + ".afficher&selectedId=";	
...
<%@ include file="/theme/list/javascripts.jspf" %>
    ...
    <TH><ct:FWLabel key="JSP_MONTANT_APG"/></TH>
...
<%
		...
		globaz.apg.vb.lots.APFactureJointCompensationViewBean courant = null;
		try {
			courant = (globaz.apg.vb.lots.APFactureJointCompensationViewBean) iterH.next();
		} catch (Exception e) {
			break;
		}
...
<%@ include file="/theme/list/lineStyle.jspf" %>
		...
		<TD class="mtd"<% if (!courant.getIsLigneCompensation().booleanValue()) { %> onclick="<%=detailUrl%>"<% } %>>
		...
		<TD class="mtd" nowrap align="left" onclick="<%=detailUrl%>"> <%=courant.getNumEtTypeAffiliation()%>&nbsp;</TD>
		...
		<TD class="mtd" nowrap onclick="<%=detailUrl%>"><%=courant.retrieveGenrePrestationLabel()%>&nbsp;</TD>
