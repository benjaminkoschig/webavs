<%-- tpl:insert page="/theme/detail.jtpl" --%><%@page import="globaz.cygnus.api.TypesDeSoins.IRFCodeTypesDeSoins"%>
...
<script language="JavaScript">
	...
	function postInit(){
		...
		<%}else{%>
			<%if (viewBean.isFocusMotifsDeRefus()){%>
				var motifsRefusId = document.getElementsByName("motifsRefusId");
				if(motifsRefusId != null){
					// FIXME motifsRefusId.focus();
				}
			<%}else if (viewBean.isFocusAdressePaiement()){%>
				var selecteurTiers = document.getElementsByName("selecteurTiersAdressePaiement");
				if(selecteurTiers != null && selecteurTiers[0] != null ){
					selecteurTiers[0].focus();
				}
			<%}%>
		<%}%>
		
		initCalculTypeTransport();
	}
	
	function checkTypeTransport() {
				
		//on cache les champs spécifiques aux transports, et raz
		$donneesVhcKm.hide();
		$donneesVhcKm.val('');
		...
		$tva.hide();
		$tva.removeAttr('checked');
	
		//on affiche les champs spécifiques en fonction du type de transport
		if ($typeTransport.val() === "<%=IRFDemande.CS_TYPE_TRANSPORT_VOITURE_PRIVEE%>"
				|| $typeTransport.val() === "<%=IRFDemande.CS_TYPE_TRANSPORT_VOITURE_PRIVEE_AI%>") {
			$donneesVhcKm.show();
			$donneesVhcPrix.show();
		} else
		if ($typeTransport.val() === "<%=IRFDemande.CS_TYPE_TRANSPORT_CROIX_ROUGE%>") {
			$tva.show();
		} else {
			$tva.val("2");
			$prixKil.val("0.00");
	...
	function initCalculTypeTransport(){
				
		$donneesVhcKm = $('#donneesVhcKm');
		$donneesVhcPrix = $('#donneesVhcPrix');
		$tva = $('#tva');
		$typeTransport = $('[name=csTypeVhc]');
		
		$typeTransport.change(function () {
			checkTypeTransport();
		});

		checkTypeTransport();
		
		$prixKil = $("#prixKilometre");
		$nbrKil = $("#nombreKilometres");

		$montantFacture = $('#montantFacture');
		
		// Fonction pour écouter si un changment de valeur se fait dans le nombre de kilomètres, afin de rappeler la fonction de calculDeplacement
		$nbrKil.change(function () {
			calculDeplacement();
		});

		// Fonction pour écouter si un changment de valeur se fait dans le prix par kilomètre, afin de rappeler la fonction de calculDeplacement
		$prixKil.change(function () {
			calculDeplacement();
		});
	};
	
	// Fonction permettant de faire le calcul des champs nombreKilometres et prixKilometre
	function calculDeplacement (){
		...
		var nbrKm = parseFloat($nbrKil.val().replace(decote,""));
		var prixKm = parseFloat($prixKil.val().replace(decote,""));
		total = nbrKm * prixKm;

		$montantFacture.val(demandeUtils.formatMnt(parseFloat(total).toFixed(2)));
...
<%-- tpl:put name="zoneMain" --%>
				...
				<TR style="vertical-align:top" >
					...
					<TD colspan="5">
						...
						<INPUT type="hidden" id="typeDemande" name="typeDemande" value="<%=RFSaisieDemandeAbstractViewBean.TYPE_DEMANDE_FRA16%>" />
				...
				<TR>
					...
					<TD><ct:FWLabel key="JSP_RF_DEM_S_DATE_FACTURE"/></TD>
					<TD>
				...
				<TR>
					...
					<TD>
						<input data-g-calendar=" "  name="dateDebutTraitement" value="<%=viewBean.getDateDebutTraitement()%>"/>
					</TD>
					
					<TD><ct:FWLabel key="JSP_RF_DEM_S_DATE_FIN_TRAITEMENT"/></TD>
					<TD>
						<input data-g-calendar=" "  name="dateFinTraitement" value="<%=viewBean.getDateFinTraitement()%>"/>
					</TD>
				</TR>
                ...
                <TR>
					...
					<TD colspan="3">
                    	...
                    	<IMG src="<%=request.getContextPath()+viewBean.getImageIsConventionne()%>" alt="" id="isConventionneImg" name="isConventionneImg">
                    </TD>
                </TR>
                <TR><TD colspan="6">&nbsp;</TD></TR>
           		<TR>
           			<%if (!viewBean.getCodeSousTypeDeSoinList().equals(IRFCodeTypesDeSoins.SOUS_TYPE_16_5_AMBULANCE)) { %>
					<TD><ct:FWLabel key="JSP_RF_DEM_S_TYPE_TRANSPORT"/></TD>
					<TD><ct:FWListSelectTag name="csTypeVhc" data="<%=viewBean.getCsTypeVhcData()%>" defaut="<%=viewBean.getCsTypeVhc()%>"/></TD>		
					<%} else {%>
					<TD></TD>
					<TD></TD>
					<%} %>
					<TD colspan="4">
						<div id="tva">
							<ct:FWLabel key="JSP_RF_DEM_S_TVA"/>
        	       			<INPUT type="checkbox" value="on" name="isTVA" <%=viewBean.getIsTVA().booleanValue()?"CHECKED":""%>/>
						</div>
						
						<div id="donneesVhcKm">
							<ct:FWLabel key="JSP_RF_DEM_S_KM_TRANSPORT"/> &nbsp;&nbsp;&nbsp;&nbsp;
							<input type="text" name="nombreKilometres" id="nombreKilometres" value="<%=viewBean.getNombreKilometres()%> " class="montant" onkeypress="return filterCharForFloat(window.event);"/>
						</div>
					</TD>
				</TR>
				<TR>
					<TD colspan="2"></TD>
					<TD colspan="4">
						<div id="donneesVhcPrix">
							<ct:FWLabel key="JSP_RF_DEM_S_PRIX_KM_TRANSPORT"/>
							<input type="text" name="prixKilometre" id="prixKilometre" value="<%=viewBean.getPrixKilometre()%>"  class="montant" onkeypress="return filterCharForFloat(window.event);"/>
						</div>
					</TD>
				</TR>
				<TR><TD colspan="6">&nbsp;</TD></TR>
               ...
               <%} %>  
				...
				<TR><TD colspan="6">&nbsp;</TD></TR>
				<TR>
					<TD><ct:FWLabel key="JSP_RF_DEM_S_MONTANT_FACTURE"/></TD>
					<TD>
						<INPUT type="text" name="montantFacture" id="montantFacture" value="<%=viewBean.getMontantFacture()%>" class="montant" onkeypress="return filterCharForFloat(window.event);"/>
					</TD>
					<TD  align="right">
				...
				<TR>
					<TD><ct:FWLabel key="JSP_RF_DEM_S_MONTANT_A_PAYER"/></TD>
