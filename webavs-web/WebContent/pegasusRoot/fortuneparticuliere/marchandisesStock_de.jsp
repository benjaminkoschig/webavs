...
<%@page import="globaz.pegasus.vb.fortuneparticuliere.PCMarchandisesStockViewBean"%>
<%@page import="ch.globaz.pegasus.business.models.droit.MembreFamilleEtendu"%>
<%@page import="globaz.pegasus.utils.PCDroitHandler"%>
<%
	//Les labels de cette page commencent par le préfix "JSP_PC_MAR_STO_D"
	idEcran="PPC0026";

	PCMarchandisesStockViewBean viewBean = (PCMarchandisesStockViewBean) session.getAttribute("viewBean");
...
<script language="JavaScript">
	...
	var ACTION_AJAX_DONNEE_FINANCIERE="<%=IPCActions.ACTION_DROIT_MARCHANDISES_STOCK_AJAX%>";
</script>

<script type="text/javascript" src="<%=servletContext%><%=(mainServletPath+"Root")%>/scripts/droit/MarchandisesStock_MembrePart.js"/></script>
<script type="text/javascript" src="<%=servletContext%><%=(mainServletPath+"Root")%>/scripts/droit/MarchandisesStock_de.js"/></script>
						...
						<div class="conteneurMembres">
								...
								<table class="areaDFDataTable">
									<thead>
										<tr>
											...
											<th><ct:FWLabel key="JSP_PC_MAR_STO_L_TYPE_PROPRIETE"/></th>
											<th><ct:FWLabel key="JSP_PC_MAR_STO_L_PART"/></th>
											<th><ct:FWLabel key="JSP_PC_MAR_STO_L_MONTANT"/></th>
											<th data-g-cellformatter="css:formatCellIcon"><ct:FWLabel key="JSP_PC_MAR_STO_L_DESSAISISSEMENT_FORTUNE"/></th>
											<th data-g-periodformatter=" " data-g-deallaterperiod=" "><ct:FWLabel key="JSP_PC_MAR_STO_L_PERIODE"/></th>
										</tr>
									</thead>
									<tbody>
							<%
									...
									for(Iterator itDonnee=viewBean.getDonnees(membreFamille.getId()).iterator();itDonnee.hasNext();){
										...
										SimpleMarchandisesStock donnee=(SimpleMarchandisesStock)donneeComplexe.getDonneeFinanciere();
							...
							%>
										<tr idEntity="<%=donnee.getId() %>" idGroup="<%=dfHeader.getIdEntityGroup() %>" header="<%=idGroup==null?"true":"false"%>">
											...
											<td><%=new FWCurrency(donnee.getMontantStock()).toStringFormat() %></td>
							...
							%>
								...
								<div class="areaDFDetail">
									<table>
										<tr>
											<td><ct:FWLabel key="JSP_PC_MAR_STO_D_TYPE_PROPRIETE"/></td>
									...
									<ct:ifhasright element="<%=IPCActions.ACTION_DROIT_MARCHANDISES_STOCK_AJAX%>" crud="cud">
