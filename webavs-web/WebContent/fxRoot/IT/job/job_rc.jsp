...
<%@ include file="/theme/find/bodyStart.jspf" %>
				...
				<DIV style="width: 100%">
					<SPAN class="idEcran">FX0300</SPAN>
					Lista dei jobs (lavori)
				</DIV>
				<%-- /tpl:put --%>
<%@ include file="/theme/find/bodyStart2.jspf" %>
						<%-- tpl:put name="zoneMain" --%>
						<tr>
							<td>File di attesa</td>
							<%
							...
							<td>
								...
								<ct:select name="forQueueName" wantBlank="true" onchange="onChangeQueue()" defaultValue="<%=queueName%>">
									<%
									java.util.List queues = globaz.jade.job.client.JadeJobServerFacade.getQueues();
									if (queues != null) {
										for (java.util.Iterator iter = queues.iterator(); iter.hasNext(); ) {
											globaz.jade.job.message.JadeJobQueueInfo queue = (globaz.jade.job.message.JadeJobQueueInfo)iter.next();
									%>
...
<%@ include file="/theme/find/bodyButtons.jspf" %>
				...
				%>
					<ct:menuSetAllParams key="queueName" value="<%=queueName%>" menuId="optionsQueue"/>
					<%
					if (queueSelected) {
					%>
						<ct:menuSetAllParams key="queueSelected" value="true" menuId="optionsQueue"/>
						<ct:menuChange menuId="optionsQueue" displayId="options" showTab="options"/>
					<%
					} else {
					%>
						<ct:menuChange menuId="optionsQueue" displayId="options" showTab="menu"/>
					<%
					}
					%>
				<%
