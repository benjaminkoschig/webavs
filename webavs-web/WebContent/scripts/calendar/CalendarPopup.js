...
// Text for the word "Current month" appearing on the calendar
cal.setThisMonthText("Current month");
...
function CalendarPopup() {
	...
	c.setThisMonthText = CalendarPopup_setThisMonthText;
	...
	c.selectMonthCalendar = CalendarPopup_selectMonthCalendar;
	// Return the object
	return c;
	}
...
//replace y,m date by y,m,1 (first day of the month)
function CalendarPopup_tmpReturnMonthFunction(y,m) {	
	if (window.CalendarPopup_targetInput!=null) {
		var d = new Date(y,m-1,1,0,0,0);
		window.CalendarPopup_targetInput.value = formatDate(d,"MM.yyyy");
		 $(window.CalendarPopup_targetInput).change();
		}
	else {
...
// Set the text to use for the "Current month" link
function CalendarPopup_setThisMonthText(text) {
	this.thisMonthText = text;
	}
...
// Simple method to interface popup month calendar with a text-entry box
function CalendarPopup_selectMonthCalendar(inputobj, linkname, format) {
	if (!window.getDateFromFormat) {
		alert("calendar.select: To use this method you must also include 'date.js' for date formatting");
		return;
		}
...
function CalendarPopup_getCalendar() {
	...
	if (this.displayType=="month") {
		...
		result += '<TR>\n';
		result += '	<TD COLSPAN=7 ALIGN=CENTER>\n';
		result += '		<A CLASS="textlink" HREF="javascript:'+windowref+this.returnMonthFunction+'(\''+now.getFullYear()+'\',\''+(now.getMonth()+1)+'\');'+windowref+'CalendarPopup_hideCalendar(\''+this.index+'\');">'+this.thisMonthText+'</A>\n';
		result += '		<BR>\n';
		result += '	</TD></TR>';
