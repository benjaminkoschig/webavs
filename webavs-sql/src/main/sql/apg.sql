...
insert into SCHEMA.FWCOUP (pcosid,plaide,pcouid,pcolut, pspy) values ( 52015005, 'F', 'ACMALPHA2', 'ACM2', (replace(char(current date), '-', '') concat replace(char(current time), '.', '') concat user) ); 
insert into SCHEMA.FWCOUP (pcosid,plaide,pcouid,pcolut, pspy) values ( 52015005, 'D', 'ACMALPHA2', 'ACM2', (replace(char(current date), '-', '') concat replace(char(current time), '.', '') concat user) ); 
