...
--Drop de la colonne (ISOTYPEAVIS) impossible avec AS400, donc manipulation spécifique à faire
...
CREATE INDEX SCHEMA.CAORGRL1 ON SCHEMA.CAORGRP(IDJOURNAL);
CREATE INDEX SCHEMA.CAORGRL3 ON SCHEMA.CAORGRP(IDPOSTEJOURNAL);
CREATE INDEX SCHEMA.CAORGRL2 ON SCHEMA.CAORGRP(IDORGANEEXECUTION);
CREATE INDEX SCHEMA.CAORGRL4 ON SCHEMA.CAORGRP(IDLOG);

insert into SCHEMA.CAORGRP ( select 
   IDORDREGROUPE,
   IDJOURNAL,
   IDORGANEEXECUTION,
   IDPOSTEJOURNAL,
   ...
   ISOGEST,
   ISOHAUTEPRIO,
   ISONUMLIVR from SCHEMA.CAORGRP_OLD);
   
DROP TABLE SCHEMA.CAORGRP_OLD;
--FIN de l'exécution du drop de la colonnne (ISOTYPEAVIS)
