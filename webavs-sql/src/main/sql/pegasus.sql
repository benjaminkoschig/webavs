...
insert into SCHEMA.FWCOUP (pcosid,plaide,pcouid,pcolut, pspy) values ( 64001010, 'F', '', 'Annulé', (replace(char(current date), '-', '') concat replace(char(current time), '.', '') concat user) ); 
...
insert into SCHEMA.FWCOUP (pcosid,plaide,pcouid,pcolut, pspy) values ( 64003007, 'F', '', 'Annulé', (replace(char(current date), '-', '') concat replace(char(current time), '.', '') concat user) ); 
