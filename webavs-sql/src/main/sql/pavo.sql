...
UPDATE SCHEMA.CIECRIP SET KBTSPE = 312002 WHERE KBIECR IN
(
	SELECT cas.idEcr
	FROM (
		SELECT af.HTITIE idTiers, ci.KANAVS numAvs, ec.KBIECR idEcr
		FROM SCHEMA.CIECRIP ec 
		INNER JOIN SCHEMA.afaffip af on (ec.KBITIE = af.MAIAFF)
		INNER JOIN SCHEMA.CIINDIP ci on (ci.KAIIND = ec.KAIIND)
		WHERE  ec.KBTGEN = 310007 AND  (MATTAF = 804001 OR MATTAF = 804005) AND ec.KBTSPE <> 312002 AND  ci.KANAVS <> '' 
		) AS cas
	INNER JOIN SCHEMA.TIPAVSP ti on (cas.idTiers = ti.HTITIE AND REPLACE(ti.HXNAVS, '.', '') = cas.numAvs)
);
...
(
	SELECT cas.idEcr
	FROM (
		SELECT af.HTITIE idTiers, ci.KANAVA numAvs, ec.KBIECR idEcr
		FROM SCHEMA.CIECRIP ec 
		INNER JOIN SCHEMA.afaffip af on (ec.KBITIE = af.MAIAFF)
		INNER JOIN SCHEMA.CIINDIP ci on (ci.KAIIND = ec.KAIIND)
		WHERE  ec.KBTGEN = 310007 AND  (MATTAF = 804001 OR MATTAF = 804005) AND ec.KBTSPE <> 312002 AND  ci.KANAVA <> '' 
		) AS cas
	INNER JOIN SCHEMA.TIPAVSP ti on (cas.idTiers = ti.HTITIE AND REPLACE(ti.HXNAVS, '.', '') = cas.numAvs)
);
