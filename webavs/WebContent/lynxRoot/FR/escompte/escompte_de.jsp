...
function init() {
	top.document.title = "Détail d'un escompte - " + top.location.href;
		...
		<TR>
			...
			<TD>Taux d'escompte &nbsp;
				<% if(!JadeStringUtil.isBlankOrZero(viewBean.getIdOperationLiee())) { %>
					(<A href="<%=request.getContextPath()%>/lynx?userAction=lynx.paiement.paiement.afficher&idOperation=<%=viewBean.getIdOperationLiee()%>" class="link">Paiement associé</A>)
				<% } %>			
			</TD>
