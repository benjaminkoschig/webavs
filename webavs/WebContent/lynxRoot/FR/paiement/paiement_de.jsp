...
function init() {
	top.document.title = "Détail d'un paiement - " + top.location.href;
...
<TABLE border="0" cellspacing="0" cellpadding="0" width="100%">
	<TBODY>
		...
		<TR>
			...
			<TD>Taux d'escompte &nbsp;
				<% if(!JadeStringUtil.isBlankOrZero(viewBean.getIdOperationLiee())) { %>
					(<A href="<%=request.getContextPath()%>/lynx?userAction=lynx.escompte.escompte.afficher&idOperation=<%=viewBean.getIdOperationLiee()%>" class="link">Escompte associé</A>)
				<% } %>
			</TD>
		...
		<TR>
			<TD>N° de facture interne &nbsp;(<A href="<%=request.getContextPath()%>/lynx?userAction=lynx.facture.facture.afficher&idOperation=<%=viewBean.getIdOperationSrc()%>" class="link">Facture associée</A>)</TD>
