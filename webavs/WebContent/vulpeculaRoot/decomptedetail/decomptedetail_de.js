...
globazGlobal.cotisations = (function() {
	function load() {
		$.ajax({
			data: {idDecompte:globazGlobal.idDecompte,
...
$(function () {
	//Si on a soumis le décompte pour un contrôle, la variable est settée à true par l'Action (FWDefaultServlet) et on lance directement la phase de contrôle.
	if(globazGlobal.echecControler) {
		afficheDialogControle(globazGlobal.idDecompte);
	}
	//Sinon, si le décompte est contrôlable, on passe en mode édition
	...
	defaultTableAjax.init({
		...
		init : function() {
			...
			this.mainContainer.on('click','.supprimerSalaire',function() {
				var idLigneDecompte = $(this).closest('tr').attr('idEntity');
				that.ajaxDeleteEntity({idLigneDecompte:idLigneDecompte});
			});
...
function reaffichePage(){
	//On réaffiche la page dans un premier temps, sinon il faut juste mettre à jour l'état et l'historique
	document.forms[0].elements('userAction').value="vulpecula.decomptedetail.decomptedetail.afficher";
	document.forms[0].elements('selectedId').value=$('#idDecompte').val();
    document.forms[0].submit();
}
...
function afficheDialogControle(idDecompte){
	//On affiche un dialog qui permet à l'utilisateur de rectifier ou modifier
	...
	dialogConfirm.dialog({
		 ...
		 buttons: [ { text: globazGlobal.libelleBoutonModifier, click: function() { modifierControle(idDecompte); } },
		            { text: globazGlobal.libelleBoutonRectifier, click: function() { rectifierControle(idDecompte); } }
		          ] 
		 });
}
		 ...
		 var o_options= {
				 ...
				 callBack:function(data) {
					 if(data.isValid==false){
						 $("#dialogMontantControle").text(globazNotation.utilsFormatter.formatStringToAmout(data.montantControle,2));
						 $("#dialogTotalSalaires").text(globazNotation.utilsFormatter.formatStringToAmout(data.totalContributions, 2));
						 $("#dialogDifference").text(globazNotation.utilsFormatter.formatStringToAmout(data.difference, 2));
