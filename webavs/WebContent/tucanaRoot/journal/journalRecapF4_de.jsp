...
<%
	globaz.tucana.vb.journal.TUJournalRecapF4ViewBean viewBean = (globaz.tucana.vb.journal.TUJournalRecapF4ViewBean) session.getAttribute("viewBean");
	idEcran = "TU-252";
...
function onClickGenerer(){
    ...
    document.forms[0].elements('userAction').value="tucana.journal.journalRecapF4.generer";
	chargeUrl(state);
}
function onClickPdf(){
    state = validateFields();
    document.forms[0].elements('userAction').value="tucana.journal.journalRecapF4.pdf";
	chargeUrl(state);
}
...
<%@ include file="/theme/detail/bodyStart.jspf" %>
			...
			<ct:FWLabel key="TIT_JOURNAL_RECAP_F4"/>
			<%-- /tpl:put --%>
<%@ include file="/theme/detail/bodyStart2.jspf" %>
						<%-- tpl:put name="zoneMain" --%>
						<TR>
							<TD width="120" id="fmt01"><ct:FWLabel key="EMAIL"/></TD>
						...
						<TR>
							...
							<TD>
								<ct:inputText name="mois" styleClass="numeroCourt" style="text-align: right;" tabindex="2" id="mois"/>
							</TD>
						</TR>
						<TR>
							<TD><ct:FWLabel key="AGENCE"/></TD>
							
							<TD>
								<ct:select id="csAgence" name="csAgence" wantBlank="true" tabindex="3">
									<ct:optionsCodesSystems csFamille="TU_AGENCE"/>
								</ct:select>
							</TD>
						</TR>
						<TR>
							<TD valign="top"><ct:FWLabel key="RUBRIQUE"/></TD>
							
							<TD>
								<ct:select id="csRubriqueList" name="csRubriqueList" wantBlank="true" tabindex="3" style="overflow: scroll;">
									<ct:optionsCodesSystems csFamille="TU_RUBR"/>
								</ct:select>
							</TD>
							<script>
								document.getElementById("csRubriqueList").multiple=true;
								document.getElementById("csRubriqueList").size=20;
								document.getElementById("csRubriqueList").style.width="100%";
