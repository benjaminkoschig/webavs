...
function onLibelleFailure(event) {
	...
	if(event.keyCode != 8 && event.keyCode != 46) {
		alert(" Der Standardtext existiert nicht.");
	}
}
...
<%@ include file="/theme/detail/bodyStart.jspf" %>
			<%-- tpl:put name="zoneTitle" --%>Detail eines Journals<%-- /tpl:put --%>
...
<tr>
<td>Eingabe offen an anderen Benutzern</td>
<td>
<%
	if (viewBean.isNew()) {
%>
...
%>
	<A href="<%=request.getContextPath()%>/osiris?userAction=osiris.comptes.apercuJournal.afficher&selectedId=<%=viewBean.getReferenceExterne()%>" class="external_link">Verbundenes Hilfsbuchhaltung Journal</A>
<%
	} else if (viewBean.isReferenceLynx()) {
%>
	<A href="<%=request.getContextPath()%>/lynx?userAction=lynx.journal.journal.afficher&selectedId=<%=viewBean.getLynxReference()%>" class="external_link">Verbundenes Lieferantsbuchhaltung Journal</A>
<%
	} else {
%>
	Externe Referenz
