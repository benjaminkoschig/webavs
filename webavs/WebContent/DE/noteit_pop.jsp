...
function doDeletingNote(aId, name) {
	if (window.confirm("Bitte bestätigen Sie die Löschung der Notiz.\n" + name))
	{
...
<%if(viewBeanList.getSession().hasRight("framework.noteit", FWSecureConstants.ADD)){%>
	<button class="btnCtrl" name="btnNew" id="btnNew" onclick="doNewNote()">Neu</button>
<%} else  {%>&nbsp;<%} %>		
</div>
<div id="editNote" style="display:none;background-color:#B3C4DB;" align="left">
	<form id="mainForm">
		Beschreibung : <input size="40" maxlength="255" type="text" id="description"/><br/>
		Notiz : <br/>
		<textarea rows="14" cols="40" id="memo"></textarea>
		<br/>
		<button name="btnCancel" id="btnCancel" onclick="cancel();" type="button">Annullieren</button>
		<%if(viewBeanList.getSession().hasRight("framework.noteit", FWSecureConstants.UPDATE)){%>	
		<button name="btnSave" id="btnSave" onclick="saving()" type="button">Speichern</button>		
		<%}%>
...
<table id="tblNote" cellpadding="1" cellspacing="1" border="0" width="100%" style="background-color: #B3C4DB">
	<thead>
		<tr id="trTest" class="title">
			<th></th>
			<th>Datum<br/><select id="cbDate" style="vertical-align:top;" name="cbDate" onchange="return filtre(this, event);"></select> </th>
			<th>Benutzer<br/><select id="cbUser" style="vertical-align:top;" name="cbUser" onchange="return filtre(this, event);"></select></th>
			<th>Beschreibung</th>
		</tr>
	</thead>
	<tbody id="tblBody">
...
<body>
	<div>Ein Fehler ist aufgetreten : <%=viewBeanList.getSession().hasErrors()?viewBeanList.getSession().getErrors().toString():viewBeanList.getMessage()%></div>
<%} %>
</body>
</html>
