...
function getCompteALettrer(query) {
	...
	document.getElementById('work').innerHTML="<table width='100%' height='100%' ><tr><td width='40%' >&nbsp;</td><td><img src='<%=request.getContextPath()%>/images/loading_animated.gif'></td><td nowrap>Daten werden geladen...</td><td width='40%'>&nbsp;</td></tr></table>"
...
function onReady() {
	...
	if ((req1.status == 0)||(req1.status == 200)) {
		...
		document.getElementById('work').innerHTML="Laden beendet."
...
function onReadyActionPerformed() {
	...
	if ((req1.status == 0)||(req1.status == 200)) {
		...
		if (status=="NM") {
			...
			showStatus("Die Operation wurde erfolgreich ausgeführt.","OK");
		} else {
...
function doLettrage() {
	...
	query ="<%=request.getContextPath()%>/osiris?userAction=osiris.lettrage.main.doLettrage&" +queryParams+"&"+bsParams
		...
		document.getElementById('work').innerHTML="<table width='100%' height='100%' ><tr><td width='40%' >&nbsp;</td><td><img src='<%=request.getContextPath()%>/images/loading_animated.gif'></td><td nowrap>Beschriftung...</td><td width='40%'>&nbsp;</td></tr></table>"
		req1.send(null); 
...
<%@ include file="/theme/detail/bodyStart.jspf" %>
			<%-- tpl:put name="zoneTitle" --%>Massenbeschriftung<%-- /tpl:put  --%>
								...
								<td colspan=2>
									<div id="search" >
										...
										<select name="filter" id="filter">
											<option value=""></option>
											<option value="1">Vor Betreibungsbegehren</option>
											<option value="2">Ab Betreibungsbegehren</option>
											<option value="3">Mit Zahlungsvereinbarung</option>
											<option value="4">Ohne Zahlungsvereinbarung</option>
											<option value="5">Manueller Ausschluss</option>	
											
										</select>
										
										<input type="button" id="btSearch" value="Suchen"
									...
									<div style="display:none" id="dialog" title="Sektion">
