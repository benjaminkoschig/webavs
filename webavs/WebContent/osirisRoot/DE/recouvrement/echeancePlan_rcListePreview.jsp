...
<%@ include file="/theme/list/lineEnd.jspf" %>
	...
	<tr>
		<td class="mtd" nowrap="nowrap" >
			Anzahl Fälligkeiten : <%=vec.size()%>
		</td>
		<td class=mtd nowrap="nowrap" align=right>
			Gesamtbetrag : <%=globaz.globall.util.JANumberFormatter.format(total)%> CHF
		</td>
	</tr>
	<tr>
		<td class="mtd" nowrap="nowrap" colspan="2" align="center">
			<%if(objSession.hasRight("osiris.recouvrement.echeancePlan.calculerSave", FWSecureConstants.UPDATE)) {%>
				<input type="button" value="Zahlungsvereinbarung akzeptieren" onClick="parent.location.href='<%=servletContext + mainServletPath %>?userAction=osiris.recouvrement.echeancePlan.calculerSave&selectedId=<%=request.getParameter("selectedId")%>&mode=save';">
			<%}%>
		<td>
	</tr>
<%-- /tpl:put --%>
