...
<script type="text/javascript">
	...
	function delNext(idEcheancePlan, idPlanRecouvrement) {
		if (window.confirm("Vous êtes sur le point de supprimer l'objet sélectionné et les suivants ! Voulez-vous tout supprimer ?")) {
			if(b) {
				b=false;
	...
	function editNext(idEcheancePlan, idPlanRecouvrement) {
		if (window.confirm("Vous êtes sur le point de recalculer les échéances ! Voulez-vous continuer?")) {
			if(b) {
				b=false;
...
<%@ include file="/theme/list/lineStyle.jspf" %>
	...
	<td class="mtd" nowrap="nowrap">
		...
		<% if(!b){%><a href="javascript:del(<%=line.getIdEcheancePlan()%>, <%=line.getIdPlanRecouvrement()%>);"><%} %><img src="<%=servletContext%>/images/btnX.gif" alt="Supprimer l'élément" title="Supprimer l'élément" border=0 <% if(b){%> style="opacity:0.4;filter:alpha(opacity=40)"<%} %> /><% if(!b){%></a><%} %>
		<% if(!b){%><a href="javascript:delNext(<%=line.getIdEcheancePlan()%>, <%=line.getIdPlanRecouvrement()%>);"><%} %><img src="<%=servletContext%>/images/btnTdelete.gif" alt="Supprimer l'élément et les suivants" title="Supprimer l'élément et les suivants" height="15" border=0 <% if(b){%> style="opacity:0.4;filter:alpha(opacity=40)"<%} %> /><% if(!b){%></a><%} %>
			...
			|| !plan.getCumulSoldeSections().trim().equals(viewBean.getSum("MONTANT").toString())){%>
				<tr>
					<td align=center colspan=<%=viewBean.hasRappelSurPlan()?8:7 %>>
						<font color=red><b>La somme des échéances ne correspond pas au solde du plan !</b></font>
					</td>
				</tr>
	<%	}
	  }%>
