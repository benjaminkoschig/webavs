...
$(function () {
	<%if(!JadeStringUtil.isBlankOrZero(viewBean.getWarningVerifierCompensation())){%>
		globazNotation.utils.consoleWarn("<%=viewBean.getWarningVerifierCompensation()%>",'Avertissement',true);
	<%}%>
});
...
function del() {
	if (window.confirm("Vous êtes sur le point de supprimer l'écriture sélectionnée! Voulez-vous continuer?")) {
...
<%@ include file="/theme/detail/bodyStart.jspf" %>
			<%-- tpl:put name="zoneTitle" --%>Saisir une décision d'intérêt<%-- /tpl:put --%>
<%@ include file="/theme/detail/bodyStart2.jspf" %>
						<%-- tpl:put name="zoneMain" --%>
            				<tr class="row">
						    	<td width="20%">
						    		<%= (viewBean.isDomaineCA()?"<a href="+request.getContextPath()+"/osiris?userAction=osiris.comptes.apercuComptes.afficher&id="+viewBean.getIdCompteAnnexe()+">Compte</a>":"Débiteur") %>
						  	...
						  	<tr class="row">
						  	  	<td>
						  	  		<a href="<%=request.getContextPath()+(viewBean.isDomaineCA()?"/osiris?userAction=osiris.comptes.apercuParSection.afficher&id="+viewBean.getIdSection():"/musca?userAction=musca.facturation.enteteFacture.afficher&selectedId="+viewBean.getIdSectionFacture())%>" > <%=viewBean.isDomaineCA()?"Section":"Décompte"%></a>
						  	  	</td>
						    	<td>
						  			...
						  			<SELECT name="idPlan" id="idPlan" onChange="jsInitRecherchePlanId()" >
										...
										for(int i = 0; i < manager.size(); i++){
											planCalcul = (CAPlanCalculInteret)manager.getEntity(i);%>
						               	 	<option selected value="<%=planCalcul.getIdPlanCalculInteret()%>"><%=planCalcul.getLibelleFR()%></option>
						                <%}%>
						  			</SELECT>
						  		</td>
						  	<% } %>
