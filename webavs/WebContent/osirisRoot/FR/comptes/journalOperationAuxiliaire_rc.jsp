...
<%
	...
	if (viewBeanTemp instanceof CAAuxiliairePaiementViewBean) {
		element = (CAAuxiliairePaiementViewBean) viewBeanTemp;
	} else {
		element = (CAAuxiliaireViewBean) viewBeanTemp;
	}

	// Cacher le bouton nouveau si journal comptabilisé
	if ((element.getJournal() != null) && (!element.getJournal().isUpdatable())) {
	   bButtonNew = false;
   	}
...
usrAction = "<%=globaz.osiris.application.CAApplication.DEFAULT_OSIRIS_NAME %>.comptes.journalOperationAuxiliaire.lister";
bFind = true;
...
top.document.title = "Comptes - Recherche des op&eacute;rations auxiliaires du journal - " + top.location.href;
</script>
<%-- /tpl:put --%>
<%@ include file="/theme/find/bodyStart.jspf" %>
				<%-- tpl:put name="zoneTitle" --%>
        <div align="left">Recherche des op&eacute;rations auxiliaires du journal</div>
        <%-- /tpl:put --%>
...
<!--              <input type="text" name="fromDate" class="date" value="<%=viewBean.getFromDate()%>" onChange="jsFromDate()" >-->
          ...
          <TR>
			...
			String redirectUrl = "/osirisRoot/FR/comptes/journalOperationAuxiliaire_rc.jsp";
			%>
			<ct:FWSelectorTag
			...
			<input type="hidden" name="forIdTypeOperation" value=""/>
			<input type="hidden" name="likeIdTypeOperation" value="<%=CAOperation.CAAUXILIAIRE%>"/>
