...
// récupère le viewBean qui a ete stocke dans la session par l'action APSituationFamilialeMatAction
globaz.apg.vb.droits.APEnfantMatViewBean viewBean = (globaz.apg.vb.droits.APEnfantMatViewBean) request.getAttribute("viewBean");
bButtonNew = viewBean.isModifiable() && bButtonNew &&  viewBean.getSession().hasRight(IAPActions.ACTION_ENFANT_MAT, FWSecureConstants.UPDATE);
actionNew += "&" + globaz.apg.servlet.APAbstractDroitDTOAction.PARAM_ID_DROIT + "=" + viewBean.getIdDroitMaternite();
...
<SCRIPT>
	...
	bFind = true;
	usrAction = "<%=globaz.apg.servlet.IAPActions.ACTION_ENFANT_MAT%>.lister";
	detailLink = servlet + "?userAction=<%=globaz.apg.servlet.IAPActions.ACTION_ENFANT_MAT%>.afficher&_method=add&<%=globaz.apg.servlet.APAbstractDroitDTOAction.PARAM_ID_DROIT%>=<%=viewBean.getIdDroitMaternite()%>";
	...
	function versEcran3() {
		var destination = "?userAction=<%=globaz.apg.servlet.IAPActions.ACTION_PERE_MAT%>.afficher&<%=globaz.apg.servlet.APAbstractDroitDTOAction.PARAM_ID_DROIT%>=<%=viewBean.getIdDroitMaternite()%>";
	...
	function versEcran4() {
		var destination = "?userAction=<%=globaz.apg.servlet.IAPActions.ACTION_SITUATION_PROFESSIONNELLE%>.chercher&<%=globaz.apg.servlet.APAbstractDroitDTOAction.PARAM_ID_DROIT%>=<%=viewBean.getIdDroitMaternite()%>";
...
<%@ include file="/theme/capage/bodyStart.jspf" %>
				<%-- tpl:put name="zoneTitle" --%><ct:FWLabel key="JSP_TITRE_SAISIE_MAT_2"/><%-- /tpl:put --%>
<%@ include file="/theme/capage/bodyStart2.jspf" %>
						...
						<INPUT type="hidden" name="forIdDroitMaternite" value="<%=viewBean.getIdDroitMaternite()%>">
