...
<%
	...
	CELettreLibreCatalogueViewBean viewBean = (CELettreLibreCatalogueViewBean) session.getAttribute ("viewBean");

	//Définition de l'action pour le bouton valider
	userActionValue = "hercule.controleEmployeur.lettreLibreCatalogue.executer";
...
<%@page import="globaz.hercule.db.controleEmployeur.CELettreLibreCatalogueViewBean"%>
...
<%-- tpl:put name="zoneTitle" --%><ct:FWLabel key="TITRE_IMPRESSION_LETTRE_LIBRE_CATALOGUE"/><%-- /tpl:put --%>
...
<%-- tpl:put name="zoneMain" --%>
	...
	<TR> 
		<TD><ct:FWLabel key="MODELE_IMPRESSION_DEFAUT"/></TD>
		<td><ct:FWListSelectTag data="<%=viewBean.getDocumentsPossible()%>" name="idDocument" defaut="<%=viewBean.getIdDocumentDefault()%>" /></TD>
	</TR>	
    <TR><TD colspan="2">&nbsp;</TD></TR>
    <TR><TD colspan="2">&nbsp;</TD></TR>

    <input type="hidden" name="idDocumentDefault" id="idDocumentDefault" value="<%=viewBean.getIdDocumentDefault()%>"/>
