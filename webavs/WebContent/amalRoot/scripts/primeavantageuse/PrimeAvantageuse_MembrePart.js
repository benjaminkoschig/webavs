/*
 * Scripts pour les écrans paramètres Prime Avantageuses
 * DHI, 05.2011
 */
 ...
 * @return
 */
function PrimeAvantageusePart(container){	
	
	// variables
	var that=this;
	this.ACTION_AJAX=ACTION_AJAX_PRIME_AVANTAGEUSE;
	...
	this.getParametres=function($data){
		var o_map = {
			'simplePrimeAvantageuse.anneeSubside':this.detail.find('#anneeSubside').val(),
			'simplePrimeAvantageuse.montantPrimeAdulte':this.detail.find('#montantPrimeAdulte').val(),
			'simplePrimeAvantageuse.montantPrimeFormation':this.detail.find('#montantPrimeFormation').val(),
			'simplePrimeAvantageuse.montantPrimeEnfant':this.detail.find('#montantPrimeEnfant').val()
		};
	...
	this.getParametresForFind=function(){
		return {			
			'listePrimeAvantageuseAjaxListViewBean.simplePrimeAvantageuseSearch.forAnneeSubside':$('#s_annee').val()
		};
	};
...
PrimeAvantageusePart.prototype=AbstractScalableAJAXTableZone;
...
//	$("#s_anneeSubside").val(new Date().getFullYear());
	...
	$('.areaMembre').each(function(){
		var $that=$(this);
		var zone=new PrimeAvantageusePart($that);
		this.zone=zone;
		var oldContent;
