...
<%@ include file="/theme/process/bodyStart.jspf" %>
			<%-- tpl:put name="zoneTitle" --%>Verwaltung der Uneinbringlichen<%-- /tpl:put --%>
<%@ include file="/theme/process/bodyStart2.jspf" %>
						...
						<tr>
							<td>
								...
								<span><input type="checkbox" id="wantTraiterAmortissementCi" name="wantTraiterAmortissementCi" checked="checked" style="" /> Umbuchung durchführen</span>
								<span>
									<ct:ifhasright element="pavo.compte.ecriture.chercherEcriture" crud="r">	
										<a href="<%=request.getContextPath()%>/pavo?userAction=pavo.compte.ecriture.chercherEcriture&compteIndividuelId=<%= viewBean.getIdCompteIndividuelAffilie() %>">(IK-Auszug anschauen)</a>
									</ct:ifhasright>
								</span>
								<%}else if (viewBean.getAmortissementCiMap().isEmpty() && viewBean.hasRubriqueCotPers()){ %>
									<span class="rubriqueIndeterminee"> IK-Abschreibung unmöglich, Überprüfen Sie den Status des IK : <%if (viewBean.getTiers() != null) { %> <%=viewBean.getTiers().getNumAvsActuel() %> <%} %></span>
								<%}%>
								...
								<div class="amortissementCi_div" id="amortissementDiv">
									<table>
										<tr>
											<th colspan="2" width="1010px">IK Abschreibung</th>
											<th>IK Status</th>
											<th>Abschreibung</th>
											<th>Netto Saldo</th>
										</tr>
										...
										%>
											<tr>
												<td colspan="2"><b><%="Abschreibung "+amortissementCi.getAnnee() %></b></td>
