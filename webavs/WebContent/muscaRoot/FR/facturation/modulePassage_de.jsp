...
<%-- tpl:insert page="/theme/detail.jtpl" --%><%@page import="globaz.musca.translation.CodeSystem"%>
<%@page import="globaz.musca.db.facturation.FAPassage"%>
<%@page import="globaz.musca.db.facturation.FAModulePassageViewBean"%>
<%@page import="globaz.musca.util.FAUtil"%>
<%@page import="globaz.musca.db.facturation.FAModulePassage"%>
<%@page import="globaz.musca.db.facturation.FAModuleFacturation"%>
...
<%
  ...
  //Vérifie si les passage est verrouillé ou comptabilisé. si oui, n'affiche pas les boutons
  	...
  	if ( FAPassage.CS_ETAT_COMPTABILISE.equalsIgnoreCase(passageStatus)
	 	|| passageLocked
...
top.document.title = "Facturation - Détail module journal"
...
<%@ include file="/theme/detail/bodyStart.jspf" %>
			<%-- tpl:put name="zoneTitle" --%>Détail d'un module du journal<%-- /tpl:put --%>
<%@ include file="/theme/detail/bodyStart2.jspf" %>
						<%-- tpl:put name="zoneMain" --%> 
          <TR>
            <TD nowrap width="200">Passage</TD>
	...
	<TR>
		...
		<%if ((request.getParameter("_method")!=null)&&(request.getParameter("_method").equals("add"))) {%>
 		<ct:FWListSelectTag name="idModuleFacturation"
            		...
            		data="<%=FAUtil.getModuleList(session, \"ALL\")%>"/>
 		<%} else {%>
 		<input  type="text" readonly class="libelleLongDisabled" value="<%=FAUtil.getLibelleModule(session, viewBean.getIdModuleFacturation())%>" >
