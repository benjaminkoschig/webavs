...
public class APSummaryAPG implements ITISummarizable {
    String element = "";
    String icon = "";
    ...
    public TISummaryInfo[] getInfoForTiers(String idTiers, BSession userSession) throws Exception {
        ...
        boolean hasDroitAPGDroit = userSession.hasRight("apg.droits.droitLAPGJointDemande", FWSecureConstants.READ);
        boolean hasDroitAPGPrest = userSession.hasRight("apg.prestation.prestationJointLotTiersDroit",
                FWSecureConstants.READ);

        setTitle(userSession.getLabel("VG_APG_TITLE"));
        setUrlTitle("");

        // on cherche le dernier droit APG pour le tiers
        ...
        manager.setNotForGenreService(IAPDroitLAPG.CS_ALLOCATION_DE_MATERNITE);
        ...
        if ((manager.size() > 0)) {
            ...
            return handleDroit(lastDroit, userSession, hasDroitAPGDroit, hasDroitAPGPrest);
        } else {
            if (hasDroitAPGDroit) {
                setUrlTitle("apg?userAction=apg.droits.droitLAPGJointDemande.chercher&typePrestation=APG&idTiers="
                        + idTiers);
            }
            return null;
        }
    ...
    public TISummaryInfo[] handleDroit(APDroitLAPGJointDemande lastDroit, BSession userSession,
            boolean hasDroitAPGDroit, boolean hasDroitAPGPrest) throws Exception {
        ...
        if (!isProtected) {
            if (lastDroit != null) {
                ...
                if (hasDroitAPGPrest) {
                    titrePeriode = "<a href='#' onclick=\"callLocalUrl('/apg?userAction=apg.prestation.prestationJointLotTiersDroit.chercher&typePrestation=APG&forIdDroit="
                ...
                title.append("</td></tr><tr><td>" + lastDroit.getSession().getLabel("SUMMARY_GENRE") + " : "
                        + lastDroit.getSession().getCodeLibelle(lastDroit.getGenreService()));
                title.append("</td></tr>");
                ...
                if (hasDroitAPGDroit) {
                    setUrlTitle("apg?userAction=apg.droits.droitLAPGJointDemande.chercher&typePrestation=APG&"
                            + "idDroit=" + lastDroit.getIdDroit() + "&noAVS=" + lastDroit.getNoAVS() + "&idTiers="
