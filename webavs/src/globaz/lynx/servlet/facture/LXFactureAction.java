package globaz.lynx.servlet.facture;
...
public class LXFactureAction extends FWDefaultServletAction {

    private static final String USERACTION_CHERCHER_FACTURE = "lynx.facture.facture.chercher";

    public LXFactureAction(FWServlet servlet) {
        super(servlet);
    }
    ...
    protected String _getDestModifierSucces(HttpSession session, HttpServletRequest request,
            HttpServletResponse response, FWViewBeanInterface viewBean) {
        FWAction action = FWAction.newInstance(LXFactureAction.USERACTION_CHERCHER_FACTURE);
        ...
        try {
            ...
            if ((method != null) && (method.equalsIgnoreCase("ADD"))) {
                if (isRequestForceNew(request)) {
                    viewBean = new LXFactureViewBean();
                } else {
                    viewBean = (FWViewBeanInterface) session.getAttribute("viewBean");
                }

                action.changeActionPart(FWAction.ACTION_NOUVEAU);
                ((LXFactureViewBean) viewBean).setIdJournal(request.getParameter("idJournal"));
                ((LXFactureViewBean) viewBean).setIdSociete(request.getParameter("idSociete"));
            } else {
                viewBean = new LXFactureViewBean();
    ...
    protected void actionAjouter(HttpSession session, HttpServletRequest request, HttpServletResponse response,
        ...
        try {
            ...
            FWViewBeanInterface viewBean = new LXFactureViewBean();

            JSPUtils.setBeanProperties(request, viewBean);
            ((LXFactureViewBean) viewBean).setVentilations(LXVentilationActionUtils.getVentilationsFromRequest(request,
                    false, ((LXFactureViewBean) viewBean).getMaxRows()));
            ...
            if (!viewBean.getMsgType().equals(FWViewBeanInterface.ERROR)) {
                ((LXFactureViewBean) viewBean).setShowRows(((LXFactureViewBean) viewBean).getVentilations().size());
                setSessionAttribute(session, "viewBean", viewBean);
                ...
                ((LXFactureViewBean) viewBean).setShowRows(0);
                setSessionAttribute(session, "viewBean", viewBean);
    ...
    protected void actionModifier(HttpSession session, HttpServletRequest request, HttpServletResponse response,
        ...
        try {
            ...
            JSPUtils.setBeanProperties(request, viewBean);
            ((LXFactureViewBean) viewBean).setVentilations(LXVentilationActionUtils.getVentilationsFromRequest(request,
                    true, ((LXFactureViewBean) viewBean).getMaxRows()));
            ((LXFactureViewBean) viewBean).setShowRows(((LXFactureViewBean) viewBean).getVentilations().size());
    ...
    private boolean isRetourDepuisPyxis(FWViewBeanInterface viewBean) {
        return ((viewBean != null) && (viewBean instanceof LXFactureViewBean) && ((LXFactureViewBean) viewBean)
                .isRetourDepuisPyxis());
    }
}
