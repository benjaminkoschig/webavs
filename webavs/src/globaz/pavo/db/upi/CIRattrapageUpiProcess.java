...
public class CIRattrapageUpiProcess extends BProcess {
    ...
    private boolean compareCI() throws Exception {
        ...
        try {
            ...
            while (result.next() && isFirst) {
                ...
                if ((ci.getSexe().equals(CICompteIndividuel.CS_HOMME) && (result.getInt("SEXE") == CIRattrapageUpiProcess.HOMME_NSSRA))
                        || (ci.getSexe().equals(CICompteIndividuel.CS_FEMME) && (result.getInt("SEXE") == CIRattrapageUpiProcess.FEMME_NSSRA))) {
                    sexeOk = true;
                } else {
                    sexeOk = false;
                }
    ...
    public void corrigeEntete() throws Exception {
        ...
        // nom
        if (result.getString("NOM").trim().length() > CIRattrapageUpiProcess.NB_CHAR_CHAMPS_NOM) {
            corrigeCI.setString(1,
                    result.getString("NOM").trim().substring(0, CIRattrapageUpiProcess.NB_CHAR_CHAMPS_NOM));
        } else {
        ...
        // Sexe
        if (result.getInt("SEXE") == CIRattrapageUpiProcess.HOMME_NSSRA) {
