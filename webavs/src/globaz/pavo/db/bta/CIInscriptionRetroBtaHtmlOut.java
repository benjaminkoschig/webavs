...
public class CIInscriptionRetroBtaHtmlOut {
    ...
    private void write(FileOutputStream out) {
        ...
        try {
            ...
            ps.println("<span class=\"titre\"> " + getSession().getLabel("TITLE_INSCRIPTION_BTA_LOG") + " </span>");
            ...
            ps.println("<thead>");
            ps.println("<tr>");
            ps.println("<th>" + getSession().getLabel("COL5_INSCRIPTION_BTA_LOG") + " </th>");
            ...
            ps.println("<th>" + getSession().getLabel("COL4_INSCRIPTION_BTA_LOG") + " </th>");
            ps.println("</tr>");
            ps.println("</thead>");

            for (Iterator it = liste.iterator(); it.hasNext();) {
                ps.println("<tr class = \"errTd\">");
                log = (CIInscriptionRetroBtaLog) it.next();
                ps.println("<td>" + log.getAnnee() + "</td>");
                ps.println("<td>" + log.getNnssRequerant() + "</td>");
                ps.println("<td>" + log.getNomRequerant() + "  </td>");
                ps.println("<td>" + log.getPrenomRequerant() + "  </td>");
                ps.println("<td class = \"mgs\">" + "1/" + log.getFraction() + "  </td>");
