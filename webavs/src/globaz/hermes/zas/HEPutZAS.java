...
/**
 * Insérez la description du type ici. Date de création : (30.04.2003 15:25:08)
 * 
 * @author: Administrator
 * @deprecated
 */
@Deprecated
public class HEPutZAS {
    ...
    public static void main(String[] args) {
        if (args.length != 2) {
            ...
            System.exit(HEPutZAS.CODE_RETOUR_ERREUR);
        }
        System.out
                .println("**********  HEPut executing with user:" + args[0] + " password:" + args[1] + "  **********");
        /*
        ...
        try {
            ...
            System.out.println(DateUtils.getTimeStamp() + "Test if the file exists on FTP server");
            FTPUtils ftpUtils = new FTPUtils(un.getSession());
            try {
                if (ftpUtils.isFileExists(un.getSession().getApplication().getProperty("ftp.file.output"))) {
                    throw new Exception("the file exist already on FTP Server, HEPut don't generate a new file !");
                }
            } catch (Exception e) {
                System.err.println(DateUtils.getTimeStamp() + e.getMessage());
                System.err.println("TRANSACTION ROLLBACK !!!");
                un.emailResponsable.sendMail(un.getSession().getLabel("HERMES_10019"), e.getMessage());
                System.exit(HEPutZAS.CODE_RETOUR_ERREUR);
            }
            System.out.println(DateUtils.getTimeStamp()
                    + "Test ok, the file doesn't exist on FTP server, HEPut can generate a new file !");
            ...
            try {
                ...
                if (!transaction.isRollbackOnly()) {
                    ...
                    System.exit(HEPutZAS.CODE_RETOUR_OK);
                } else {
    ...
    public void confirmLots(BTransaction crtTransaction) throws Exception {
        for (int i = 0; i < vecLots.size(); i++) {
            ...
            // lot.setDateEnvoi(DateUtils.getCurrentDateAMJ());
            // lot.setHeureEnvoi(DateUtils.getCurrentTimeHMS());
    ...
    public File generateFile(BTransaction crtTransaction) throws Exception {
        ...
        if (isFileEBCDIC) {
            // j'écris en EBCDIC
            fos = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(fileName), HEPutZAS.ENCODING));
        } else {
    ...
    private void genereLotPtyBasse(BTransaction crtTransaction, BufferedWriter fos, boolean hasCarriageReturns)
        ...
        if (listLot.size() > 0) {
            traiterLot(crtTransaction, fos, hasCarriageReturns, (HELotViewBean) listLot.getFirstEntity());
        }
    }
