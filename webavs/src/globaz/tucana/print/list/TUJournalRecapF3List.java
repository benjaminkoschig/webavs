 ...
 * @version : version 1.0
 */
public class TUJournalRecapF3List extends FWIAbstractManagerDocumentList {
    /**
     * 
     */
     ...
     * @throws Exception
     */
    public TUJournalRecapF3List() throws Exception {
        ...
        super(new BSession(TUApplication.DEFAULT_APPLICATION_TUCANA), TUApplication.APPLICATION_PREFIX, "ALFA",
                "Liste recap F3", new TUTemporaireManager(), TUApplication.DEFAULT_APPLICATION_TUCANA);
    }

    /**
     * @param session
     */
    public TUJournalRecapF3List(BSession session) {
        super(session, TUApplication.APPLICATION_PREFIX, "ALFA", session.getLabel("LIST_F3"),
    ...
    public void _beforeExecuteReport() {
        ...
        try {
            journal = TUJournalGenerator.generate(getTransaction(), getAnnee(), getMois(), getCsAgence(),
                    ITUCSConstantes.CS_TY_CATEGORIE_RUBRIQUE_F002, TUJournal.TYPE_CHARGEMENT_AGENCE);
        } catch (Exception e) {
            // on ne fait rien, le journal sera vide
        }
        String title = "PAR MOIS / AGENCE - pour l'année " + getAnnee();
