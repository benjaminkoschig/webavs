 ...
 */
public class CGReleveAVSCptExpl_Bean extends CGReleveAVS_Bean {
    /**
     * Constructor for CGReleveAVSBilan_Bean.
     */
    public CGReleveAVSCptExpl_Bean() {
        super();
    }
    ...
    public boolean prepareValue(BEntity bEntity, BTransaction transaction, BSession session) {
        ...
        try {
            CGMontantHelper result = traitementReleveAvsHelper.getMontantComptesExploitation(transaction, session,
                    entity, getIdMandat(), getIdExerciceComptable(), getIdPeriodeComptable(), isProvisoire());

            // Column 2 - Infos solde actif
            if (result.actif != null && !result.actif.isZero()) {
            ...
            if (result.passif != null && !result.passif.isZero()) {
                result.passif.negate();
                col3 = new Double(result.passif.doubleValue());
            } else {
                col3 = null;
            }
            // Column 4 - Infos solde cumule
            if (result.soldeCumule != null && !result.soldeCumule.isZero()) {
                col4 = new Double(result.soldeCumule.doubleValue());
            } else {
                col4 = null;
            }
        } catch (Exception e) {
        ...
        return (col2 != null || col3 != null || col4 != null);
    }
}
