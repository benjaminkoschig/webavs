...
public class PRAffiliationHelper {
     ...
     * @return l'employeur (un tiers) si il est trouvé, sinon null
     * @throws Exception
     *             si le numéro d'affilié non unique
     */
     ...
     *            Lance une exception si définit à <code>true</code> et que plusieurs numéro d'affilié sont identique
     * @return l'employeur (un tiers) si il est trouvé, sinon null
     * @throws Exception
     *             si numéro d'affilié non unique
     */
    public static final IPRAffilie getEmployeurParNumAffilie(BISession session, String numAffilie, String date,
        ...
        // BZ_6560 : Erreur dans le formattage automatique du no d'affilié
        ...
        if ((result == null) || (result.length == 0)) {
            ...
            // plusieurs affliliation active
                ...
                if (failIfEmployeurNotUnique) {
                    throw new Exception(
                            "Plusieurs affiliations ont été trouvés pour la date de début du droit (numAffilie = "
                                    + numAffilie + ")");
                } else {
                    GlobazValueObject affilie = (GlobazValueObject) result[noAffiliation];
                    Object value = affilie.getProperty(IPRAffilieImpl.PROPERTY_TIERS_AF);
