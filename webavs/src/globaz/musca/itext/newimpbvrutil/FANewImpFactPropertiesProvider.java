 ...
 * @author VYJ
 */
public abstract class FANewImpFactPropertiesProvider {
    ...
    protected static void init(FAApplication application) {
        if (!FANewImpFactPropertiesProvider.IS_INITIALIZED) {
            ...
            if ((value != null) && JadeNumericUtil.isInteger(value)) {
                FANewImpFactPropertiesProvider.NEXT_DATASOURCE_WAITING_TIME = Long.parseLong(value);
            }

            // Initialise la taille maximale de la pile
            value = application.getProperty("musca.bvr.impression.pile.taille.max");
            if ((value != null) && JadeNumericUtil.isInteger(value)) {
                FANewImpFactPropertiesProvider.MAX_STACK_SIZE = Integer.parseInt(value);
            }
            ...
            if ((value != null) && JadeNumericUtil.isInteger(value)) {
                FANewImpFactPropertiesProvider.STACK_FULL_WAITING_TIME = Long.parseLong(value);
            }

            // Permet d'activer le mode débug pour les logs
            value = application.getProperty("musca.bvr.impression.debug.mode");
            if (value != null) {
                FANewImpFactPropertiesProvider.IS_DEBUG_MODE = Boolean.valueOf(value).booleanValue();
            }
            ...
            if (value != null) {
                FANewImpFactPropertiesProvider.IS_MODE_ASYNCHRONE = Boolean.valueOf(value).booleanValue();
            }

            // Défini comme quoi l'objet est maintenant initialisé avec les
            // valeurs des propriétés
            FANewImpFactPropertiesProvider.IS_INITIALIZED = true;
        }
    }
}
