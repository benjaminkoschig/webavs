 ...
 * @author VYJ
 */
public class FANewImpFactDataSourceASynchroneProvider implements IFANewImpFactDataSourceProvider {
     ...
     */
    private LinkedList<FANewImpFactDataSource> impressionFactureDataSources = null;
     ...
     */
    protected FANewImpFactDataSourceASynchroneProvider(int _nbEnteteFactures) {
        super();
        nbEnteteFactures = _nbEnteteFactures;
        impressionFactureDataSources = new LinkedList<FANewImpFactDataSource>();
    }
     ...
     */
    private synchronized Object doOperationOnList(int mode, FANewImpFactDataSource impressionFactureDataSource) {
    ...
    protected int getCurrentCacheSize() {
        return ((Integer) doOperationOnList(FANewImpFactDataSourceASynchroneProvider.SIZE_MODE, null)).intValue();
    }
    ...
    public FANewImpFactDataSource getNextDataSource() {
        while ((getCurrentCacheSize() == 0) && !abort) {
            try {
                Thread.sleep(FANewImpFactPropertiesProvider.NEXT_DATASOURCE_WAITING_TIME);
                if (FANewImpFactPropertiesProvider.IS_DEBUG_MODE) {
                    JadeLogger.info(this, "nextDatasource - empty stack, waiting "
                            + FANewImpFactPropertiesProvider.NEXT_DATASOURCE_WAITING_TIME + "mls");
                }
        ...
        return (FANewImpFactDataSource) doOperationOnList(FANewImpFactDataSourceASynchroneProvider.GET_MODE, null);
    }
     ...
     * @param impressionFactureDataSource
     */
    protected void setImpressionFactureDataSource(FANewImpFactDataSource impressionFactureDataSource) {
        doOperationOnList(FANewImpFactDataSourceASynchroneProvider.ADD_MODE, impressionFactureDataSource);
    }
}
