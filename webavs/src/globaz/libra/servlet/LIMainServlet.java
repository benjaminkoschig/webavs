...
import globaz.libra.stack.LINaviRules;
 ...
 * @author hpe
 */
public class LIMainServlet extends FWServlet {
    ...
    public FWController createController(BISession arg0) throws Exception {
        return new FWDispatcher(arg0, LIApplication.DEFAULT_APPLICATION_LIBRA.toLowerCase(),
                LIApplication.APPLICATION_PREFIX);
    }
    ...
    protected void customize(FWUrlsStack aStack) {
        ...
        aStack.addRule(new LINaviRules(aStack));
    }
        ...
        if (!hasRight) {
            ...
            try {
                if (JadeLogger.isDebugEnabled()) {
                    JadeLogger.debug(this, "LIBRA action " + act);
                }

                Class actionClass = LIMainServletAction.getActionClass(act);
                ...
                JadeLogger.warn(this, "* LIBRA : WARNING                                           ");
    ...
    protected void goHomePage(HttpSession httpSession, HttpServletRequest request, HttpServletResponse response)
            throws Exception {
        String path = "/" + LIApplication.APPLICATION_LIBRA_REP + "/";
        path = path.concat(HOMEPAGE + ".jsp");

        forward(path, request, response);

        FWMenuBlackBox menuBB = (FWMenuBlackBox) httpSession.getAttribute(FWServlet.OBJ_USER_MENU);
        menuBB.setCurrentMenu("li-menuprincipal", "menu");
        menuBB.setCurrentMenu("LI-OnlyDetail", "options");
    }
