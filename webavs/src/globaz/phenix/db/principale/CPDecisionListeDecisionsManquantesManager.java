...
public class CPDecisionListeDecisionsManquantesManager extends globaz.globall.db.BManager implements
    ...
    protected String _getFrom(BStatement statement) {
        ...
        if (!JadeStringUtil.isIntegerEmpty(fromAnneeDecision) && !JadeStringUtil.isIntegerEmpty(toAnneeDecision)) {
            ...
            String table1 = "(SELECT a.MAIAFF, b.MADDEB, b.MADFIN FROM " + _getCollection() + "CPDECIP a INNER JOIN "
                    ...
                    + this._dbWriteNumeric(statement.getTransaction(), getFromAnneeDecision()) + " AND "
                    + this._dbWriteNumeric(statement.getTransaction(), getToAnneeDecision());

            if (getForExceptEtatDecision().length() != 0) {
                table1 += " AND a.IATETA <>"
                        + this._dbWriteNumeric(statement.getTransaction(), getForExceptEtatDecision());
            }

            table1 += " GROUP BY a.IAANNE, a.MAIAFF, b.MADFIN, b.MADDEB) AS T1";
    ...
    protected String _getWhere(globaz.globall.db.BStatement statement) {
        ...
        if (!JadeStringUtil.isIntegerEmpty(fromAnneeDecision) && !JadeStringUtil.isIntegerEmpty(toAnneeDecision)) {
            ...
            sqlWhere += "(MADFIN >= " + this._dbWriteDateAMJ(statement.getTransaction(), dateFin)
                    + " OR MADFIN = 0) AND MABTRA <> 1 AND MADFIN <> MADDEB AND MADDEB<="
                    + this._dbWriteDateAMJ(statement.getTransaction(), toDateDebut) + " AND MATTAF IN ("
                    + CodeSystem.TYPE_AFFILI_INDEP + ", " + CodeSystem.TYPE_AFFILI_INDEP_EMPLOY + ", "
                    + CodeSystem.TYPE_AFFILI_NON_ACTIF + ", " + CodeSystem.TYPE_AFFILI_TSE_VOLONTAIRE + ", "
                    + CodeSystem.TYPE_AFFILI_TSE + ") AND T2.MAIAFF IS NULL";
        } else {
            _addError(statement.getTransaction(), getSession().getLabel("CP_MSG_0023"));
        }
