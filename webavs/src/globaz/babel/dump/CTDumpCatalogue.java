...
public class CTDumpCatalogue {
    ...
    public static void main(String[] args) {
        ...
        try {
            BSession session = (BSession) ((CTApplication) GlobazSystem
                    .getApplication(CTApplication.DEFAULT_APPLICATION_BABEL)).newSession("globazf", "globazf");
            file = new FileWriter(args[0]);
            new CTDumpCatalogue(session, args[1].equals(".") ? null : args[1], args[2].equals(".") ? null : args[2],
    ...
    // ---------------------------------------------------------------------------------------------------
    /**
     * Crée une nouvelle instance de la classe CTDumpCatalogue.
     * 
     * @param session
     ...
     */
    public CTDumpCatalogue(BSession session, String csDomaine, String csTypeDocument, String schema) {
    ...
    public void dumpSQL(Writer out) throws Exception {
        ...
        try {
            ...
            for (int idDoc = 0; idDoc < docs.size(); ++idDoc) {
                ...
                printDeleteCatalogueQuery(print, doc.getCsDomaine(), doc.getCsTypeDocument());
                printInsertQuery(doc, queryBuilder.reset(), doc.getIdTypeDocument(), 0, null, docCount,
                        CTDumpCatalogue.DOC_BASE, doc.getBorneInferieure(), print, transaction);
                ...
                for (int idElement = 0; idElement < elements.size(); ++idElement) {
                    ...
                    eleCount++;
                    print.println();
                    printInsertQuery(element, queryBuilder.reset(), null, docCount, CTDumpCatalogue.DOC_BASE, eleCount,
                            CTDumpCatalogue.ELE_BASE, doc.getBorneInferieure(), print, transaction);
                    ...
                    for (int idTexte = 0; idTexte < textes.size(); ++idTexte) {
                        ...
                        printInsertQuery((CTTexte) textes.get(idTexte), queryBuilder.reset(), null, eleCount,
                                CTDumpCatalogue.ELE_BASE, texCount, CTDumpCatalogue.TEX_BASE, doc.getBorneInferieure(),
                                print, transaction);
                    }
                }
            }
