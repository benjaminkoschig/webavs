...
/**
 * Servlet gérant les avances au niveaux cygnus (paiement unique et listes)
 * --> Mamdat INFOROM547
 * 
 * @author sce
 * 
 */
public class RFAvanceAction extends RFDefaultAction {

    public RFAvanceAction(FWServlet servlet) {
        super(servlet);
    }
    ...
    protected void actionAfficher(HttpSession session, HttpServletRequest request, HttpServletResponse response,
        ...
        try {
            ...
            if ((vvv != null) && (vvv instanceof RFAvanceViewBean) && ((RFAvanceViewBean) vvv).isRetourDepuisPyxis()) {
                isRetourDepuisPyxis = true;
            }

            if ((method != null) && (method.equalsIgnoreCase("ADD")) && !isRetourDepuisPyxis) {

                RFAvanceViewBean vb = new RFAvanceViewBean();
                ...
                if ((viewBean != null) && (viewBean instanceof RFAvanceViewBean)
                        && ((RFAvanceViewBean) viewBean).isRetourDepuisPyxis()) {
                    ((RFAvanceViewBean) viewBean).setRetourDepuisPyxis(false);
                    // On réinitialise l'adresse de pmt. Ainsi elle sera
                    // rechargée avec la nouvelle récupérée des tiers.
                    ((RFAvanceViewBean) viewBean).setAdpmt(null);
                    /*
                     * redirection vers la destination
                     */
                    session.setAttribute("viewBean", viewBean);
