/*
 * Crée le 22 juillet 2009
 */
package globaz.cygnus.servlet;

import globaz.cygnus.application.RFApplication;
import globaz.cygnus.stack.RFGereCaPages;
import globaz.cygnus.stack.RFNaviRules;
 ...
 * @author jje
 */
public class RFMainServlet extends FWServlet {
    ...
    public FWController createController(BISession arg0) throws Exception {
        return new FWDispatcher(arg0, RFApplication.DEFAULT_APPLICATION_CYGNUS.toLowerCase(),
                RFApplication.APPLICATION_PREFIX);
    }
    ...
    protected void customize(FWUrlsStack aStack) {
        ...
        aStack.addRule(new RFNaviRules(aStack));
        aStack.addRule(new RFGereCaPages());
        aStack.addRule(new FWRemoveActionsEndingWith(IRFActions.ACTION_JADE_WIDGET + "." + FWAction.ACTION_LISTER));
        aStack.addRule(new FWRemoveActionsEndingWith(IRFActions.ACTION_SAISIE_QD_AUGMENTATION + ".afficher"));
        aStack.addRule(new FWRemoveActionsEndingWith(IRFActions.ACTION_SAISIE_QD_SOLDE_CHARGE + ".afficher"));
        aStack.addRule(new FWRemoveActionsEndingWith(IRFActions.ACTION_SAISIE_QD_SOLDE_EXCEDENT_DE_REVENU + ".afficher"));
        aStack.addRule(new FWRemoveActionsEndingWith(IRFActions.ACTION_SAISIE_QD_PERIODE_VALIDITE_QD_PRINCIPALE
                + ".afficher"));
    }
        ...
        if (!hasRight) {
            ...
            try {
                if (JadeLogger.isDebugEnabled()) {
                    JadeLogger.debug(this, "CYGNUS action " + act);
                }

                Class actionClass = RFMainServletAction.getActionClass(act);
                ...
                JadeLogger.warn(this, "* CYGNUS : WARNING                                           ");
    ...
    protected void goHomePage(HttpSession httpSession, HttpServletRequest request, HttpServletResponse response)
            throws Exception {
        String path = "/" + RFApplication.APPLICATION_CYGNUS_REP + "/";
        ...
        menuBB.setCurrentMenu("cygnus-menuprincipal", "menu");
        menuBB.setCurrentMenu("cygnus-optionsempty", "options");
    }
