 ...
 * author fha
 */
public class RFDentistesEnvoiFactureEtrangerOO extends RFAbstractDocumentOO {

    public static final String FICHIER_MODELE_DOCUMENTS_RFM = "RF_LETTRE_TYPE_DENTISTE_ENVOI_FACTURE_ETRANGER";
    ...
    public void chargerCatalogueTexte() throws Exception {
        ...
        documentHelper.setCsTypeDocument(IRFCatalogueTexte.CS_MEDECIN_FACTURE_ETRANGER);
    ...
    public void chargerDonneesEnTete() throws Exception {
        ...
        try {
            ...
            caisseHelper = CaisseHelperFactory.getInstance().getCaisseReportHelperOO(getSession().getApplication(),
                    codeIsoLangue);
            caisseHelper.setTemplateName(RFDentistesEnvoiFactureEtrangerOO.FICHIER_MODELE_DOCUMENTS_RFM);
    ...
    public void generationLettre(RFCopieDecisionsValidationData... copie) throws Exception {
        ...
        data.addData("idProcess", "RFDentistesEnvoiFactureEtrangerOO");
        ...
        if (null != tiersWrapper) {
            ...
            throw new Exception("Erreur : Pas d'adresse tiers (RFDentistesEnvoiFactureEtrangerOO.generationLettre())");
        }
    ...
    public JadePrintDocumentContainer remplir(JadePrintDocumentContainer documentContainer, RFDocumentsProcess process,
        ...
        return super
                .remplir(
                        ...
                        process,
                        getIdTiers(),
                        IPRConstantesExternes.RFM_LETTRE_TYPE_DENTISTE_ENVOIS_FACTURE_MEDECIN_CONSEIL_DE_LA_FACTURE_TRAITEMENT_ETRANGER,
                        isSendToGed);
    }
    ...
    public void remplirDocument() throws Exception {
        ...
        try {
            ...
            // %=URGENT=%
            data.addData("URGENT", documentPrincipale.getTextes(4).getTexte(3).getDescription());
            // %=ENVOI=%
            ...
            data.addData("SALUTATIONS", PRStringUtils.replaceString(documentPrincipale.getTextes(4).getTexte(5)
                    .getDescription(), TITRE, sexe.equals("516001") ? "Monsieur" : "Madame"));
            // %=INFORME=%
            data.addData("INFORME", documentPrincipale.getTextes(5).getTexte(1).getDescription());
            // %=PAS_CONTACTE=%
            data.addData("PAS_CONTACTE", documentPrincipale.getTextes(5).getTexte(2).getDescription());
            // %=CONTACTE_DATE=%
            data.addData("CONTACTE_DATE", documentPrincipale.getTextes(5).getTexte(3).getDescription());
            // %=CONTACTE_SOLUTION=%
            data.addData("CONTACTE_SOLUTION", documentPrincipale.getTextes(5).getTexte(4).getDescription());
            // %=NOTE_CORRECTE=%
            data.addData("NOTE_CORRECTE", documentPrincipale.getTextes(6).getTexte(1).getDescription());
            // %=RAMENE=%
            data.addData("RAMENE", documentPrincipale.getTextes(6).getTexte(2).getDescription());
            // %=FR=%
            data.addData("FR", documentPrincipale.getTextes(6).getTexte(3).getDescription());
            // %=TRAIT=%
            data.addData("TRAIT", documentPrincipale.getTextes(6).getTexte(4).getDescription());
            // %=TRAITEMENT=%
            data.addData("TRAITEMENT", documentPrincipale.getTextes(6).getTexte(5).getDescription());
            // %=SEA=%
            data.addData("SEA", documentPrincipale.getTextes(6).getTexte(6).getDescription());
            // %=LOI=%
            data.addData("LOI", documentPrincipale.getTextes(6).getTexte(7).getDescription());
            // %=SA=%
            data.addData("SA", documentPrincipale.getTextes(6).getTexte(8).getDescription());
            // %=RESTE_MONTANT=%
            data.addData("RESTE_MONTANT", documentPrincipale.getTextes(6).getTexte(9).getDescription());
            // %=DETAILS=%
            data.addData("DETAILS", documentPrincipale.getTextes(6).getTexte(10).getDescription());
            // %=MONTANT_HONORAIRE=%
            data.addData("MONTANT_HONORAIRE", documentPrincipale.getTextes(7).getTexte(1).getDescription());
        } catch (Exception e) {
            throw new Exception(e.toString());
        }
    }
