...
import ch.ech.xmlns.ech_0104._3.ErrorPeriodType;
import ch.ech.xmlns.ech_0104._3.OverlapInformation;
...
public class AnnoncesRafamErrorBusinessServiceImpl extends ALAbstractBusinessServiceImpl implements
        AnnoncesRafamErrorBusinessService {
            ...
            if (!erreurAnnonce.isNew()) {
                ...
                if ((error.getOverlapInformation() != null) && !error.getOverlapInformation().isEmpty()) {
                    for (OverlapInformation overlapInfo : error.getOverlapInformation()) {
                        ...
                        overlapInfoModel.setOfficeIdentifier(overlapInfo.getDeliveryOfficeConflict()
                                .getOfficeIdentifier());
                        if (overlapInfo.getDeliveryOfficeConflict().getBranch() != null) {
                            overlapInfoModel.setBranch(Long.toString(overlapInfo.getDeliveryOfficeConflict()
                                    .getBranch()));
                        }
                        ...
                        overlapInfoModel.setOverlapPeriodeEnd(ALDateUtils.XMLGregorianCalendarToGlobazDate(overlapInfo
                                .getOverlapPeriod().getEnd()));
    ...
    public void validateInsignificance(int nbDays) throws JadeApplicationException, JadePersistenceException {
        ...
        searchInsignificance.setForDateReception(JadeDateUtil.getGlobazFormattedDate(ALDateUtils.addJourDate(nbDays,
                ALDateUtils.getCalendarDate(JadeDateUtil.getGlobazFormattedDate(new Date()))).getTime()));
        searchInsignificance.setWhereKey("errInsignificance");
        searchInsignificance = ALImplServiceLocator.getAnnonceRafamErrorComplexModelService().search(
        ...
        for (int i = 0; i < searchInsignificance.getSize(); i++) {
            ...
            if (0 == ALImplServiceLocator.getAnnonceRafamErrorComplexModelService().count(searchSignificance)) {
                ALImplServiceLocator.getAnnonceRafamBusinessService().validateForRecordNumber(
                        annonce.getAnnonceRafamModel().getRecordNumber(), false);
            }
        }
    }
}
