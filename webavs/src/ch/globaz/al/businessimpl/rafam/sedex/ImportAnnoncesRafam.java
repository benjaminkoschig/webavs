...
public class ImportAnnoncesRafam {
    ...
    private void addErrorProtocole(ArrayList<HashMap<RafamImportProtocolFields, String>> protocole,
            Object messageRafam, String messageErreur) throws JadeApplicationServiceNotAvailableException {
        ...
        if (messageRafam instanceof NoticeType) {
            errList = ALServiceLocator.getAnnoncesRafamErrorBusinessService().getErrorsFromList(
                    ((NoticeType) messageRafam).getError());
            ...
            errList = ALServiceLocator.getAnnoncesRafamErrorBusinessService().getErrorsFromList(
                    ((ReceiptType) messageRafam).getError());
            ...
            errList = ALServiceLocator.getAnnoncesRafamErrorBusinessService().getErrorsFromList(
                    ((RegisterStatusRecordType) messageRafam).getError());
            ...
            errList = ALServiceLocator.getAnnoncesRafamErrorBusinessService().getErrorsFromList(
                    ((UPISynchronizationRecordType) messageRafam).getError());
    ...
    private void createFichiersRetoursDelegue(HeaderType header, RafamTypeAnnonce typeAnnonces) throws Exception {
        ...
        if (annonceEmpl.size() > 0) {
            ...
            for (Map.Entry<Integer, ArrayList<Object>> en : annonceEmpl.entrySet()) {
                ...
                fillHeaderFichierEmployeur(header, childAllowances, (AnnonceRafamDelegueComplexModel) en.getValue()
                        .get(0));

                for (Object annonce : en.getValue()) {
                    try {
                        if (RafamTypeAnnonce._69B_SYNCHRO_UPI.equals(((AnnonceRafamDelegueComplexModel) annonce)
                                .getAnnonceRafamModel().getTypeAnnonce())) {
                            isUPI = true;
                        }
                        ...
                        errors.append(
                                "Une erreur s'est produite pendant la création des fichiers d'annonces de l'employeur délégué n°"
                                        ...
                                        + ", annonce n° "
                                        + ((AnnonceRafamDelegueComplexModel) annonce).getAnnonceRafamModel()
                                                .getRecordNumber()).append(e.getMessage()).append("\n");
                ...
                if (errors.length() > 0) {
                    try {
                        ...
                        ALRafamUtils.sendMail(new StringBuffer(
                                "Af-delegue :Erreurs survenues lors de génération du fichier retour"), errors,
                                new String[0]);
                    } catch (Exception e) {
                ...
                String uriFilesRetour = JadePropertiesService.getInstance().getProperty(
                        "al.rafam.delegue." + idEmployeurDelegue + ".filesRetour.uri");
                ...
                String file = ALServiceLocator.getAnnoncesRafamDelegueProtocoleService().createProtocole(
                        Integer.toString(idEmployeurDelegue));
                ...
                description = new StringBuffer("Protocole et fichiers de retour (" + typeAnnonces
                        + ") de vos annonces disponibles");
        ...
        if (annonce.getComplementDelegueModel() != null) {
            ...
            if (JadeStringUtil.isBlankOrZero(annonce.getComplementDelegueModel().getMessageMailResponsiblePerson())) {
                ...
                String email = JadePropertiesService.getInstance().getProperty(
                        "al.rafam.delegue." + annonce.getAnnonceRafamModel().getRecordNumber().substring(0, 2)
                                + "defaultContactMail");
                childAllowances.setMailResponsiblePerson(email);
            } else {
                childAllowances.setMailResponsiblePerson(annonce.getComplementDelegueModel()
                        .getMessageMailResponsiblePerson());
            }
            ...
            if (JadeStringUtil.isBlankOrZero(annonce.getComplementDelegueModel().getMessageNameResponsiblePerson())) {
                ...
                childAllowances.setNameResponsiblePerson(annonce.getComplementDelegueModel()
                        .getMessageNameResponsiblePerson());
            }
            ...
            if (JadeStringUtil.isBlankOrZero(annonce.getComplementDelegueModel().getMessageTelResponsiblePerson())) {
                ...
                childAllowances.setTelResponsiblePerson(annonce.getComplementDelegueModel()
                        .getMessageTelResponsiblePerson());
            }
    ...
    private String[] getMailsProtocole() {
        String contactTechnique = JadePropertiesService.getInstance().getProperty(
                ALConstRafam.RAFAM_CONTACT_TECHNIQUE_EMAIL);
    ...
    public BSession getSession() throws Exception {
        if (session == null) {
            session = (BSession) GlobazSystem.getApplication(ImportAnnoncesRafam.APPLICATION_NAME).newSession(
                    userSedex, passSedex);
        }
    ...
    private boolean isImportRegisterStatusDelegateEnabled() {
        return "1".equals(JadePropertiesService.getInstance().getProperty(
                ALConstRafam.IMPORT_REGISTER_STATUS_DELEGATE_IS_ENABLED));
    }
    ...
    private boolean isImportRegisterStatusEnabled() {
        return "1".equals(JadePropertiesService.getInstance().getProperty(
                ALConstRafam.IMPORT_REGISTER_STATUS_IS_ENABLED));
    }
    ...
    private boolean isRecipientIdValid(SedexMessage messageCentrale, List<String> recipientIdMessage)
        ...
        } else if (!recipientIdMessage.get(0).equals(
                JadeSedexService.getInstance().getSedexDirectory().getLocalEntry().getId())) {
            JadeLogger.error(
                    this,
        ...
        for (NoticeType notice : message.getContent().getNotice()) {
            try {
                ...
                if (JadeThread.logMessages() != null) {
                    addErrorProtocole(protocole, notice, new JadeBusinessMessageRendererDefaultStringAdapter().render(
                ...
                try {
                    ...
                    if (!RafamReturnCode.ANNULEE.equals(RafamReturnCode.getRafamReturnCode(String.valueOf(notice
                            .getReturnCode())))) {
    ...
    protected void processReceipts(ArrayList<HashMap<RafamImportProtocolFields, String>> protocole, Message message) {
        ...
        for (ReceiptType receipt : message.getContent().getReceipt()) {
            try {
                ...
                if (JadeThread.logMessages() != null) {
                    addErrorProtocole(protocole, receipt, new JadeBusinessMessageRendererDefaultStringAdapter().render(
                            JadeThread.logMessages(), JadeThread.currentLanguage()));
                    JadeThread.rollbackSession();
                    JadeThread.logClear();
                } else {
                ...
                try {
                    // si l'annonce est annulée à la centrale il n'est pas nécessaire de logguer l'erreur
                    if (!RafamReturnCode.ANNULEE.equals(RafamReturnCode.getRafamReturnCode(String.valueOf(receipt
                            .getReturnCode())))) {
    ...
    protected void processRegisterStatusRecords(ArrayList<HashMap<RafamImportProtocolFields, String>> protocole,
        ...
        if ((isImportRegisterStatusEnabled() || isImportRegisterStatusDelegateEnabled())
            ...
            for (RegisterStatusRecordType register : message.getContent().getRegisterStatus().getRegisterStatusRecord()) {
                ...
                try {
                    ...
                    if (isImportRegisterStatusDelegateEnabled()
                            && serviceAnnonceRafamSedex
                    ...
                    } else if (isImportRegisterStatusEnabled()
                            && !serviceAnnonceRafamSedex.isAnnonceEmployeurDelegue(register.getRecordNumber()
                                    .toString())) {
                    ...
                    if (JadeThread.logMessages() != null) {
                        addErrorProtocole(protocole, register,
                                new JadeBusinessMessageRendererDefaultStringAdapter().render(JadeThread.logMessages(),
    ...
    protected void processUPISynchronizationRecords(ArrayList<HashMap<RafamImportProtocolFields, String>> protocole,
        ...
        for (UPISynchronizationRecordType synchro : m.getContent().getUPISynchronizationRecord()) {
            try {
                ...
                if (JadeThread.logMessages() != null) {
                    addErrorProtocole(protocole, synchro, new JadeBusinessMessageRendererDefaultStringAdapter().render(
                            JadeThread.logMessages(), JadeThread.currentLanguage()));
                    JadeThread.rollbackSession();
                    JadeThread.logClear();
                ...
                try {
                    // si l'annonce est annulée à la centrale il n'est pas nécessaire de logguer l'erreur
                    if (!RafamReturnCode.ANNULEE.equals(RafamReturnCode.getRafamReturnCode(String.valueOf(synchro
                            .getReturnCode())))) {
